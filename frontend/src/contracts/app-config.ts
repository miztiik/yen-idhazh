...
export interface ObservabilityConfig {
	...
	/** The buffer each side of the memory-bandwidth probe allocates, so the probe holds twice this. Zero switches the probe off and leaves the bandwidth cell empty; every other cell of the fingerprint still gets written. The default beats the largest L3 this project has drawn, 480 MiB, because a buffer that fits in cache measures cache and reads as a memory figure four times too high. Raise it when a drawn machine reports an L3 at or above this. */
	host_fingerprint_bandwidth_mib?: number;
