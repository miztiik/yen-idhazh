...
function knobs(): { applied: number; maxDownStep: number; band: [number, number] } {
	...
	return {
		...
		band: [threshold.band_low, threshold.band_high]
	};
}
...
test('the margin draws at a width a person can see', () => {
	...
	// of the box - so a reader could not tell the dot from the rule. On this scale
	// it draws at 24 px, read 2026-09-19 off a 1033 px plot.
	const { applied, maxDownStep, band } = knobs();
	...
	// reader can see which side of the rule the dot is on. The gate is that width
	// and not the reading above it, because the reading moves with the fall cap.
...
test('the zone goes down from the line and never up', () => {
	// THE BITE. The clamp caps both directions, but only a fall folds a pair the
	// line refuses today - a rise folds fewer. A zone drawn either side of the
	// line would tint scores tomorrow cannot reach.
...
test('the zone is where one more day would fold a pair', () => {
	...
	// Three of the four clear the line today and would not after one day's fall.
	// The fourth, 0.9407, is already over it.
	expect(inside).toEqual([0.9374, 0.9352, 0.9343]);
});

test('the reach counts the marks today, tomorrow and the day after', () => {
	// One of the four is over the line as committed, and one day's legal fall of
	// 0.010 takes it to all four. The day after adds nothing, which is the whole
	// of what the headline has left to say once the fall cap got this wide.
	const { applied, maxDownStep } = knobs();
	const reach = holdoutReach(applied, maxDownStep, COMMITTED.map((score) => mark(score)));

	expect(reach).toEqual({ today: 1, tomorrow: 4, dayAfter: 4 });
});
...
test('the headline says what one more day costs, not just the distance', () => {
	...
	expect(said).toContain('0.010');
	expect(said).toContain('3 more marks');
