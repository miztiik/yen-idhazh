	...
	for (const band of BANDS) {
		test(`${band.name} (${band.width}px): the zone model, at two root font sizes`, async ({
			...
			for (const rootPx of ROOT_PX) {
				...
				// off the content box. Everywhere else the item IS the content box:
				// the day stream took a leading column for its time rail until
				// 2026-09-12, and the rail is gone.
