...
- **Hosted inference at runtime.** No API call to a model provider from the published site, and none from the pipeline without an explicit contract change (it reverses Holy Law #1).
...
| `evals/`             | planned    | The committed eval ledger (CSV). Appended by CI, read by the dashboard, never recomputed at runtime.         |
