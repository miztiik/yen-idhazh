...
12. **Nothing costs more as the repository grows.** A step whose work scales with what we have already published is a bill that arrives every day for an answer we already had. This binds `state/`, `frontend/public/digest/`, the vector and index files under `assist/`, `corpus/`, and every test, build and check that reads them. One good case proves the code. The archive is checked once, by the producer that wrote it. **Constant cost is the default and growing cost is the exception.** Design every check, test and build step to do the same work on day 1,000 that it did on day 1: take one item, one day, one shard, one header - a fixed parameter, never a loop over what the archive happens to hold. A step whose cost grows with the corpus needs a **named human approval recorded next to the code**, saying what it reads, how its cost grows, and why a bounded fixture cannot answer the question. No agent may grant that approval to itself, and no such step may be added because it was the quickest thing to write. Where one is unavoidable it is one step, run once, over a bounded window - never a rule re-checked per item on every later run. `backend/tests/test_archive_readers.py` is where the test-side exceptions live: the list may only shrink, a bare name is refused, and CI fails on a name that is not on it.
