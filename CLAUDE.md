...
- **A source file answers one question, and its first sentence says which one.** If what you add needs that sentence widened, the file now holds two answers - put yours in a new file beside it. A sentence that names a tier, a layer or a subsystem has named no question, so that file accepts everything anybody brings. Length decides nothing and no count separates the two cases, so this one is enforced by reading: a long file with one answer is correct, and a short file with two is not. **A router is not a worker** is the sharpest case - a file that chooses which unit of work runs holds the choice and nothing else, and the work lives in the unit's own module. A file every change opens is a file no change owns, so its growth is invisible to the change that causes it and paid by whoever reads it next.
