...
10. **Use measurements** when they can materially change the decision. Never invent precision: every measured number includes its hardware, date, and spread; unmeasured numbers are estimates. An estimate can guide the next step, but cannot settle the design. A lack of measurement is not a reason to reject a design, nor an excuse to avoid measuring. When a useful measurement is cheap, take it. When results contradict the design, change the design.
