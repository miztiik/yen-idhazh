...
Each `work` shard now commits its server's counters onto its row of
`state/host-fingerprint/<YYYY>/<MM>/<DD>.csv`, and
`backend/utilities/reconcile_prefill.py` opens the day file the run id names,
pools both sides of that run and prints
...
`work` shard against the 180-minute bar, read from `job_seconds` in
...
- **A rate that moved between two runs is not evidence of a change we made**
 ...
 committed cell on `state/host-fingerprint/` and on every census row, so that
 check costs a column read.
...
| Put the counter snapshot on `RunManifest` | Wrong grain (a manifest run record is one run, a snapshot is one shard), wrong producer (the manifest is written hours later in another job, so the numbers would have to travel in an artifact that expires and is never committed - how long each one lives is [../../reference/github-actions.md](../../reference/github-actions.md)), and wrong audience (`run.json` is a payload a reader fetches; this is measurement evidence and belongs under `state/`). |
...
- [`../contracts/schemas.md`](../contracts/schemas.md) - `HostFingerprintRow`, and why a job-grain reading is its own contract.
