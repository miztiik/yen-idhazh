...
| `StorySimilarityPair` | `story-similarity-pair` | one appended row of `state/story-similarity/scored-pairs/<YYYY>/<MM>/<DD>.csv` - one borderline pair, what it scored, what a judge said in both orders, and the instrument that said it. The same shape holds the day's draw under `backend/var/judge/<date>/draw.csv` before a judging leg reads it |
