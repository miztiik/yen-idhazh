...
| --- | --- | --- |
| Why this item ran | `selection_score`, `authority_score`, `tier_score`, `feed_weight`, `feed_reliability`, `lens_bonus`, `recency_bonus`, `carriage_step`, `watchlist_bonus`, `carried_by`, `watchlist_hit`, `on_front_page`, `tier`, `source_form`, `published_at`, `time_source` | which term carried it onto the page. The ranker computes each of these and then keeps only the total, so without them the only answer to "why did this publish" is "the score said so" |
