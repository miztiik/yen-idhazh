...
**Nine names joined the thirteen in the same commit, and each has a named renderer.** Measured 2026-08-31 on Intel Core i7-1265U / Windows 11 / node 24.12.0, 11 committed days and 3,596 items, `gzip -9` over the compact projection, one build an arm:

| Added | What draws it | Cost |
| --- | --- | ---: |
| `carried_by`, `watchlist_hit`, `on_front_page`, `rank_score` | the lead block, which needs a comparable score across the whole day | +3.21 B an item |
| `published_at`, `time_source` | the time rail, and the meta line today | +9.04 B an item |
| `introduced_by_run` | the divider at the seam between runs | +1.16 B an item |
| `lenses` | the topic chips | +1.07 B an item |
| `key_points` | the in-page filter, which reads them today | +92.91 B an item |

**`key_points` is nine tenths of that and it is the one worth defending.** `DigestList` filters on it now, so once a reading route fetches this file an absent `key_points` is a thrown `TypeError` rather than a narrower filter. The twelve prerendered documents it replaces carry the same words twelve times over, so on the wire it is cheaper here than it was there. Three names were refused: `events` and `entities` have no renderer and are out of scope as reader-facing chips (+1.57 and +1.71 B an item), and `source_form` has no reader at all (+1.20).

**What it cost, end to end.** The staged payloads went 361.98 to 468.51 gzipped bytes an item, 29.4 percent more, and the 178 rendered images are untouched. The projection is still 40.9 percent under the committed day, which compacts to 792.24 gzipped bytes an item. No prerendered page moved: the six routes the bundle gate names read -1 to +6 bytes across the two builds, because a prerendered document reads the committed day and not this file.

**The runway, re-derived rather than restated (Rule #10).** The built site went 152.9 to 155.2 MiB and the whole-tree rate went 44,578 to 45,267 bytes a published item. `idhazh site-weight` prints 126 published days to the 1 GiB cap against 128 before, and that figure divides the headroom by `run.safety_ceiling_per_run` items a day - a per-run ceiling of 160 spent as a per-day rate. **Over the 11 committed days a published day holds a median of 334 items and ranges from 4 to 731**, so at the measured rate the same headroom is 60.3 published days against 61.3 before: **this change costs about one published day of runway, and the cap arrives about 2026-10-30.** Both figures charge `assist/` and `_app/` - 65.6 MB, 42.3 percent of the site, neither of which grows with a day - to the items, so both are floors.

That is what this row spends. What it buys is the migration, and one day priced on this build says how much: **2026-08-30 is twelve prerendered documents totalling 8,822,134 bytes raw and 2,528,812 gzipped, against one served payload of 717,709 raw and 194,016 gzipped.** Twelve times the bytes, after this row grew the payload by 62 percent. The documents are the six HTML pages and their six `__data.json` twins, and every one of them carries the whole item list.
...
| Serving `DigestDay` whole and deleting the projection | It is the same objection one level on, with a bigger number behind it. Measured 2026-08-31 over 11 committed days and 3,596 items, `gzip -9`: the committed day is 792.24 bytes an item against the projection's 468.38, so serving it whole is 69.1 percent more on the wire - and 40.0 percent of that is a vector block no browser opens. Bytes are what this migration is for. |
