...
**The floor on that diagram is a number the run chooses, since 2026-09-18.** With `assemble.same_story.adaptive_dedup_threshold.enabled` off - which is how it ships - it is `assemble.same_story.floor_min` and the diagram reads exactly as it always has. With it on, `similarity.applied.effective_same_story` replaces that one field with the newest line a fit applied inside `applied_lookback_days`, and the rest of the block is untouched: same weights, same veto, same all-pairs rule. The run then writes the number it used onto its own record as `same_story_floor_applied`, because without it a reader of a committed `run.json` cannot tell a day grouped at 0.94 from a day grouped at 0.937 - and the grouping is what the line moves. A run that published before the column existed carries no value, which is every run before that date.
