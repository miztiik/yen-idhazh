...
**The runway, re-derived rather than restated (Rule #10).** Over the same pair the built site went 152.9 to 155.2 MiB and the whole-tree rate went 44,578 to 45,267 bytes a published item. `idhazh site-weight` printed 126 published days to the 1 GiB cap against 128 before, and that figure divides the headroom by `run.safety_ceiling_per_run` items a day - a per-run ceiling of 160 spent as a per-day rate. **Over the committed days a published day holds a median of 334 items and ranges from 4 to 731**, so at the measured rate the same headroom is 60.3 published days against 61.3 before: **this change costs about one published day of runway, and the cap arrives about 2026-10-30.** Both figures charge `assist/` and `_app/` - 65.6 MB, 42.3 percent of the site, neither of which grows with a day - to the items, so both are floors.
