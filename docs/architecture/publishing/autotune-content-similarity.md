...
**The bar is a weighted term rather than a bare cosine**, and it lives in `assemble.same_story`. The term runs 0 to 1, its weight is 1.0 - refused by a validator if it is anything else - and `floor_min` is on that same scale. That rule is what keeps the floor meaning something: let the weight be 1.3 and the floor gets easier to clear every time it moves, silently, and the day publishes a merge nobody chose.

| Knob | Ships at | What it scores |
| --- | ---: | --- |
| `same_story.cosine_weight` | 1.0 | Cosine between the two stored int8 vectors. The whole score, which is why 1.0 is its only legal value. |
| `same_story.floor_min` | 0.94 | What the weighted score has to reach, for **every** pair inside a group. |

**It shipped changing nothing, and that is measured rather than asserted.** All the weight is on the cosine, so the score is arithmetically what the single floor was. Replayed through `collapse_same_story` over every committed day, before and after, on one box: **26 days, 9,552 items carrying a vector, 99 of them in a group - and not one item's `also_covered_by` or `same_story_as` differs.** Measured 2026-09-16 on a developer machine / Python 3.14.2; the pass is deterministic and the counts have no spread.

**A second term sat in that table until 2026-09-24.** `same_story.key_point_weight` scored the words two items' key points had in common, reduced exactly the way a headline is. It was the only other term measured that separates the two populations - a different-story 99th percentile of 0.0962 against a same-story median of 0.2419, so the two barely touch - and it shipped computed, logged and weighted zero, waiting for a row of hand labels to raise it. That row never arrived and key points left the pipeline first, so the term went with them. The rule that the weights sum to 1.0 now says the same thing about one number.
...
**A second spelling of one reduction rule stood here until 2026-09-24, and it left with the term it served.** `key_point_overlap` needed the key points reduced to words, and `assemble._reduce` is a Python string function that no contract can be generated from - so `$lib/console/holdout.ts` carried a TypeScript copy, and a reading rather than a test held the two in step. Measured 2026-09-19 on a developer machine / Node 24.12.0 / Python 3.14.2, over every key-point block in the 29 committed days: **10,328 of 10,328 reduced to the same word set**. The first attempt kept combining marks and disagreed on 3 of them, all Devanagari - Python's `\w` is alphanumeric plus the underscore and a matra is neither, so a matra was a separator on one side and not the other. The term shipped at a weight of 0.0 and was never raised, so it never moved a published group. The cosine is the whole score now, the console reproduces one multiplication rather than two, and the drift this paragraph used to bound cannot happen.
