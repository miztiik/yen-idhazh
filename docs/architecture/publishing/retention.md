...
| `state/scores/<YYYY>/<MM>/<DD>/` | 2,700,019 | 34.6 percent | `observability.scores_full_grain_months` - **archived and deleted from 2026-09-03, and the deletion is in dry run** |
| `state/item-health/<YYYY>/<MM>/<DD>/` | 1,409,945 | 18.0 percent | `observability.item_health_full_grain_months` - folded a month at a time from that month's day files, and the fold is in dry run |
