...
Eight routes are generated at build time - `/`, `/archive/`, `/evals/`, `/console/` and its four tabs - and `adapter-static`'s fallback, `404.html`, answers everything else. SvelteKit with `adapter-static` and no `entries` reading the digest tree anywhere. **Seven files declare `prerender = true` and eight routes get a document.** `/console/judgement/` and `/console/voices/` declare nothing and inherit it from `console/+layout.ts`, which covers every route under `/console/` - so how many documents a build writes is counted in the build and never in the grep. This page said "six" until 2026-09-12, because the two tabs arrived with #616 and nobody counted again; the build writes eight `index.html` files plus `404.html` ([the 2026-09-12 record](../../reference/benchmarks/2026-09-12-prerender-on-and-off.md)). `/archive/`, `/404`, `/evals/` and the console inline everything they draw; `/` inlines a seed of the newest day and fetches the rest; a dated URL is the fallback document, and the page fetches the served day for the whole of it.

**Until 2026-09-09 every route was prerendered, and that cost six documents a published day.** 20 dated pages and 96 topic pages, each with a `__data.json` twin, all rebuilt on every run because a document holding a seed of a day changes when the day does. Measured on a real build, a developer machine, 2026-09-09, `BUILD_VERSION` pinned across both arms: 796 files and 116,050,183 bytes before, **572 files and 101,880,352 after** - 224 files and 14,169,831 bytes, 12.2 percent of the built site. On that build 123 documents became 7 and 121 `__data.json` files became 5; the twins are still 5 and the documents are 9, because the two console tabs added on 2026-09-12 carry no server load of their own. **The document count no longer moves when a day is added**, which is the whole of it - it moves when somebody adds a route, which is a line in a commit rather than a run of the pipeline.
...
**The reader's browser draws the chart and the pipeline never draws one** (owner ruling, 2026-09-13, [visuals.md](visuals.md)). A story arrives carrying a pointer at its marks; [ItemVisual.svelte](../../../frontend/src/lib/components/ItemVisual.svelte) fetches that file when the story comes near, [drawing.ts](../../../frontend/src/lib/payload/drawing.ts) refuses anything that does not fit the type it names, and [visual/bar.ts](../../../frontend/src/lib/visual/bar.ts) decides where every mark sits. Svelte writes them, so the marks are ordinary elements in the component's own subtree and take the page's tokens through plain scoped CSS.
...
**What arrives is checked, and it is checked in the reader's browser.** `refusedVisualData` in [../../../frontend/src/lib/payload/drawing.ts](../../../frontend/src/lib/payload/drawing.ts) is the guard: it refuses a path that is not a published visual, a document whose `renderer_version` this page does not hold, a type it cannot draw, and a shape that does not fit that type - four names against three figures, a channel naming a mark that is not carried, a bar with no figure. A refusal costs the story its picture and nothing else. **The build checks none of it**, because the build draws nothing: every chart on the site is checked on the device of the reader who opens that day. That is the same trade `validate-days` records above - the build stopped being the place a bad day is found, and it is written down rather than assumed.
...
**What it costs was measured rather than assumed, and the bytes turned out not to be the question.** One tree built both ways - as it stands, and with the seven declarations deleted and nothing else changed - three interleaved passes an arm at one pinned `BUILD_VERSION`, and zero spread on every quantity. With prerendering the built site is 106,316,899 B and without it 104,681,197 B, so prerendering costs **1,635,702 B: 1.54 percent of the published site, which is 1.6 MB of a 1,024 MB cap, or 18 published days of the 1,047 the runway has left.** The arm without the declarations writes no `frontend/build/index.html` at all and leaves `404.html` as the only HTML file in the tree, and `npm run build` exits 1 on it, because `frontend/scripts/build-state.ts` refuses to certify a build whose root document is missing - so every browser spec would stop collecting as well. The hardware, the method, the pin and every figure are in [../../reference/benchmarks/2026-09-12-prerender-on-and-off.md](../../reference/benchmarks/2026-09-12-prerender-on-and-off.md), which is the page to cite and the page a better number replaces.
