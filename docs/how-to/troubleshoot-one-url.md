...
| Summary `labels_truncated` | The labelling call exhausted ITS output budget, so the call that writes the summary was never sent. That budget is derived from the labelling grammar in `classify.calls.call_one_output_tokens`, not from `models.summarize.inference.max_answer_tokens`. |
