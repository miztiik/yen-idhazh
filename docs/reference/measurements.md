...
| Qwen3.5-9B prompt tokens, article-token spread and worst-case context | **unmeasured; Qwen3 counts do not transfer** | render every band with LF, tokenize with `Qwen3.5-9B-Q4_K_M.gguf`, tokenize the measured article buckets, then recompute prompt + capped article + output budget |
