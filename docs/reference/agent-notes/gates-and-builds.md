...
**A guard that enumerates hazards is wrong the day after it is written.** `test_archive_readers.py` held twelve approved paths and covered two collections out of nineteen; its own maintenance cost grew with the number of collections, which is the defect it existed to catch. It shipped and was deleted the same day, 2026-09-06 (`CLAUDE.md` Guardrail #12). State a rule as a property and enforce it in review.
...
**Two builds back to back can fail with `EPERM` on `.svelte-kit/output`** when the previous process still holds it. The message names a permission and reads like a broken checkout - and the second build then wrote nothing, so a script that hashes the output afterwards hashes the FIRST build's file and reports two cases that agree. Delete the directory before each build, retry until it is gone, and refuse to read the output when the exit code is not zero. Seen 2026-09-09.
...
**`TODO/STATUS.md` has one writer, and a pull request that carries it is refused.** The page is generated from every plan-doc's Status Reckoner, so the gate that used to regenerate it on the merge candidate gave the file one writer per open branch: any row anybody stamped `IN-FLIGHT` or `DONE` on `main` made your committed copy stale, and the diff it printed named plan rows you never opened, which reads as a contaminated branch. Merging and regenerating settled it, and the second round then conflicted. **Measured 2026-09-12: four of the six rows of plan 27 hit this, one of them twice** - each settled by regenerating, which is a band-aid rather than a fix (Guardrail #5). A `status-page` job on `main` regenerates and commits the page after a merge lands; `gates` fails any pull request that touches the file at all and names it. So a worker stamps its own Reckoner line and leaves the generated page alone. Where a branch picked it up anyway - a merge of `origin/main`, or habit - take the trunk's copy rather than re-deriving one, because the counts in its section headings are computed and a textual merge can produce a file that is internally inconsistent and still merges cleanly.
