...
flowchart TB
 ...
 subgraph MOD["Summarize - what the model is asked"]
  ...
  CALLS --> ROWS[("item health, eval rows,<br/>machine readings")]
 end
...
shard's own segment, which `assemble` folds into
...
The same row carries how long the job took. The `work` job's first step - ahead
of the checkout, so the clock covers the cache restore and the weight load -
writes an epoch second to `$GITHUB_ENV`, and the `What this job cost` step hands
it to `python -m idhazh job-clock`. Which processor the job drew is on that row
too, written before the model server starts by `python -m idhazh fingerprint`.
