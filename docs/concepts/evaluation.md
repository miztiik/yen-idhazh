...
**Four surfaces here were shaped by a ban that is gone, and none of them has been reconsidered (2026-09-24).** [../../CLAUDE.md](../../CLAUDE.md) section 1a makes LLM-as-judge primary evaluation: a model verdict may run in a production workflow, may score live content, and may determine publication. Four things in this repository were built while the opposite was the rule. None of them is dead and none is deletable - every one still runs and still produces something a person or a gate reads - but every one took the shape it has because a model was not allowed to judge, so every one can now be asked a question it could not be asked before.

| What it is | Why it took this shape | What section 1a changes |
| --- | --- | --- |
| [`../../backend/idhazh/visual_validator.py`](../../backend/idhazh/visual_validator.py) - nine checks over a visual plan, all deterministic code | a judge that shares the failure modes of the thing judged is not a measurement, so no check may call a model and a test reads the module's own imports to prove it | a model judge is permitted. The import guard is the module's own choice now, not a rule |
| [`../../backend/idhazh/evals/hhem.py`](../../backend/idhazh/evals/hhem.py) - a separate cross-encoder that scores faithfulness | an independent check was needed, because a summarizer grading itself agrees with itself for exactly the reasons you wanted the check | the summarizer's own model may judge. Keeping a separate instrument is an argument about measurement error now, not a rule |
| the human label ledger - [`../../backend/idhazh/contracts/label_row.py`](../../backend/idhazh/contracts/label_row.py), [`../../backend/idhazh/evals/labels.py`](../../backend/idhazh/evals/labels.py), [`../../backend/utilities/label_queue.py`](../../backend/utilities/label_queue.py) | `labeller` is a required name checked against `evaluation.labellers`, there is no `model_id` and no nullable author, and the CLI has no `--model`, no `--from-file` and no stdin path | a model could fill it. Doing so still needs a schema change, a changelog entry and a Level 5 consultation, and that is the control that survives |
| [`../../backend/idhazh/contracts/review_queue.py`](../../backend/idhazh/contracts/review_queue.py) - a day's visual decisions, laid out for a person | a model verdict could not reach publication, so the tree gates nothing and no publish decision reads it | a verdict can reach publication now |

**This is a finding and not a proposal.** Nothing in the table is scheduled, nothing here asks for a decision, and no current plan carries a row for any of it. Each of the four is its own product decision, and each needs the same thing before a human comes out of the loop: a measurement showing the model is at least as good as the person or the deterministic check it would replace. Section 1a removed the rule that said no. It did not supply that measurement, and removing a human on a permission is not the same as removing one on a number.
