...
- `models.summarize.inference.max_think_tokens` (256) is the other half of that pair and a **hard cap**. It bounds the thinking span, which only runs where `models.<role>.turns.thinking_close` declares a closing marker - the incumbent declares none, so the number is inert today. It is a cap rather than a hint because a model that never closes its reasoning block would otherwise eat the whole window, and the failure would be recorded as a truncated summary, which names the wrong cause. What it costs when it does run is 42.6 seconds an item a span at the measured decode rate ([../architecture/summarize/throughput.md](../architecture/summarize/throughput.md)).
