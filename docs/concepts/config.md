...
- `page_weight.ceilings_bytes` is a **limit**, and the only one on this list. It says how heavy each prerendered page may be on the wire, gzipped, and `frontend/scripts/bundle-gate.mjs` fails the build above it. Each number is the heaviest of three builds of one tree plus the 64-byte noise floor, and nothing beyond it: a ceiling above today's weight is a gate that never fires, and a ceiling inside its own noise floor is a coin toss. Only routes that render no day are bounded - a day page weighs what the day published, and a number over it would cap the news. Raising one is legitimate when the page earned the bytes, and the commit that raises it says what they buy. Two of them are countdowns rather than steady bounds: `/archive/` inlines every committed day to feed the on-device search and grows about 170 KB per published day, and `/console/` grows with the ledger its charts read. Both were watched firing on a re-run day, an hour after being set, and both were raised again on 2026-08-26 when the vector backfill filled the archive's corpus - the archive plan that would shard it is blocked on that backfill, so it could not land first ([../reference/measurements.md](../reference/measurements.md#the-prerendered-page-on-the-wire)).
