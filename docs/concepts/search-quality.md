...
the thing being retrieved, so letting it generate the query set would grade the
model with itself - which `CLAUDE.md` section 1a now permits and this eval still
...
| Use an item's own key points as a free query | The summarizer wrote them. A query generated by the model whose output is being retrieved shares its failure modes, so it cannot find them. `CLAUDE.md` section 1a would permit it; this eval refuses it on the measurement, not on the rule. Moot since 2026-09-24: the field left the pipeline. | Andre |
