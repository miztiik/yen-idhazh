...
- **Two faces at most**: one for reading, one for data (tabular numerals on the dashboard, so columns line up). A display face is a weight of the reading voice rather than a third face: it carries the wordmark and an item's title and nothing else, and `theme.display_face_enabled` puts both back on the reading stack.
