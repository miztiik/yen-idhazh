...
from two days' fall below the line to one day's fall above it - `max_down_step`
either side of `floor_min`, both off `config/idhazh.json`, so the window is the
same width every day and two days of this panel compare. Measured on the built
page at 1440: the margin drew at **5.8 px of a 1033 px plot** on the 0.88 to 1.00
band, and **48 px** on this one. At 5.8 px a dot 13 px across sits on top of the
rule it is measured against, so a reader cannot see which side of the line the
closest call is on - which is the whole of what the panel is for.

**It reaches two days down because the sentence counts two days.** One day's
legal fall of 0.005 moves the committed count on the wrong side from 1 to 3, and
a second day moves it to 4. An axis stopping at one day would print a count of
marks it had not drawn.

**The zone goes down and only down.** `fit.clamp` is one-sided -
`lowest_today = previous - max_down_step`, with no upward clamp - so a corridor
drawn either side of the line would draw a constraint that does not exist. The
tint says: every mark in here is a pair tomorrow could fold.
