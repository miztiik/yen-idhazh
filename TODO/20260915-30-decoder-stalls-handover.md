...
`fix/a-worker-gets-a-clock`, off `247e386a`. **Not pushed. Not a PR yet.**

`ruff` and `mypy` are clean. Six tests fail, all one mechanical class: a new
enum member needs its fixtures and its docs.
