...
- **Decisions:**
 ...
 | 6 | **A per-model build opens a hole this row closes in the same change.** A fork binary loads a stock model file happily, and `validate.yml` job `qualify` decides publication, so a verdict could be measured on one binary and reported under another (Guardrail #10). The refusal is one assertion in the gate that publishes: a qualification whose recorded build is not the repository pin does not produce a verdict. The benchmark arms keep the fork. `backend/idhazh/fingerprint.py:103` already records the build and falls back to `UNRECORDED_BUILD`, so the reading exists | Carmack and Fowler, agreed 2026-09-21 |
 | 7 | The repository-wide pin stays the default, so every current model resolves exactly the release it resolves today | Carmack |
