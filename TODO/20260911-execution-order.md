...
**The wave ran serially rather than seven at once, and that was a cost rather than a design.** The seven were dispatched one at a time in pole order and each pull request was merged before the next worker cut its worktree, because the contract then read as a wave and the owner waited for each report before starting the next. **Every file collision the wave was composed to avoid therefore could not arise**, and none did: seven pull requests, zero conflicts, every one green on its first push - which proves nothing about the disjointness, because nothing ever overlapped. Under the pool rule that replaced it ([`../docs/how-to/execute-a-plan.md`](../docs/how-to/execute-a-plan.md#parallel-fan-out)) four rows are in flight at once, a slot refills the moment a worker returns, and a pending merge never holds one. The disjointness proof above is load-bearing now rather than theoretical.
