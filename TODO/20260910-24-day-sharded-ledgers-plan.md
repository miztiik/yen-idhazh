...
- **Files touched:** `backend/idhazh/evals/writer.py`, `backend/idhazh/evals/archive.py`, `backend/idhazh/retention.py`, `backend/idhazh/publish_scores.py`, `backend/idhazh/drift.py`, `backend/idhazh/ledger.py`, `backend/idhazh/cli.py`, `backend/utilities/reband_scores.py`, `backend/utilities/label_queue.py`, `backend/utilities/data_wrangler.py`, `backend/utilities/migrate_score_ledger.py` (deleted), `backend/idhazh/contracts/{observation_index,public_eval,score_archive}.py`, `schemas/{observation-index-row,score-archive}.schema.json` (regenerated), `frontend/src/lib/server/payload.ts`, `frontend/src/routes/console/{,machine/,model/}+page.server.ts`, `backend/tests/{test_evals,test_ledger,test_drift,test_retention,test_pipeline,test_workflows}.py`, `tests/fixtures/evals/index-rebuild/` (moved to the day layout), `frontend/tests/console.spec.ts`, `state/scores/2026-08.csv` and `state/scores/2026-09.csv` (deleted) replaced by the day files the utility writes under `state/scores/2026/08/` and `state/scores/2026/09/` - **23 files on 2026-09-13**, not the 20 counted on 2026-09-11 - and `state/score-index/2026-08.csv` and `state/score-index/2026-09.csv` (deleted) replaced by the day files the rebuild writes under `state/score-index/2026/08/` and `state/score-index/2026/09/` - **23 files, one beside each ledger day; no other row in this plan writes in either directory** - `docs/concepts/partitions.md`, `docs/concepts/growing-reads.md`, `docs/concepts/evaluation.md`, `docs/concepts/config.md`, `docs/architecture/contracts/schemas.md`, `docs/architecture/publishing/{retention,layout,console-payloads,telemetry-series}.md`, `docs/how-to/run-the-pipeline.md`, `.github/workflows/digest.yml`
...
| 8 | Row #5's four dependencies and its file list were accurate | Three of the four were. **`backend/utilities/build_canary_day.py` and `backend/tests/test_console_payloads_producer.py` build no item-health path**, so neither needed an edit, and `backend/tests/test_pipeline.py` addresses the ledger only through `ledger.item_health_path` and was green unchanged. Fifteen files the row did not name were unavoidable; the row's own list carries them and says why. Found by row #5, 2026-09-13 |
