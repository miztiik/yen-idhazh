...
- **Files touched:** `backend/idhazh/day_partition.py` (new, created by this row and written by no other), `backend/idhazh/ledger.py`, `backend/idhazh/publish_telemetry.py`, `backend/tests/test_day_partition.py` (new), `backend/tests/test_ledger.py`, `backend/tests/test_marks.py`, `tests/fixtures/day-partition/` (created by this row; nothing else in the plan writes there), `backend/utilities/measure_day_window.py` (new), `docs/concepts/month-partitions.md` renamed to `docs/concepts/partitions.md`, `docs/architecture/contracts/schemas.md`, `docs/architecture/publishing/layout.md`, `docs/architecture/publishing/telemetry-series.md`, `docs/architecture/publishing/console-payloads.md`, `docs/concepts/growing-reads.md`, `docs/reference/measurements.md`, `TODO/20260906-constant-cost-reads-plan.md`, `TODO/20260907-growing-reads-window-plan.md`, `TODO/20260910-24-day-sharded-ledgers-plan.md`
...
- **Acceptance gates:** `GATE-PY` with `backend/tests/test_day_partition.py` and `backend/tests/test_ledger.py`, `GATE-SUITE`. Plus, in this row:
  ...
  - every relative link in the repository that named `month-partitions.md` names `partitions.md` and resolves - **14 links in 7 files, and 36 mentions in 11 files, re-measured 2026-09-11** - and a link whose visible text ends `.md` names the same basename as the file it opens. **An earlier draft said 17 links; it was counting mentions, and the two numbers matter differently**: a link that does not resolve is a broken page, while a mention in a docstring is prose. **Three of the eleven files are Python** - `backend/idhazh/ledger.py`, `backend/idhazh/publish_telemetry.py` and `backend/tests/test_ledger.py` - and their docstrings are corrected in this row too, because a module pointing a reader at a page that does not exist is the same defect one directory over;
...
| --- | --- | --- | --- |
| `docs/concepts/partitions.md` | as `docs/concepts/month-partitions.md`, 14 links in 7 files and 36 mentions in 11 | 1 renames; 3, 5, 6, 7, 8 extend | The day rule beside the month rule, the per-collection table re-derived a row at a time, and the section on changing a collection's grain |
...
- [`../docs/concepts/month-partitions.md`](../docs/concepts/month-partitions.md) - the page row #1 renames to `partitions.md`; the layout rule it carries is the one every row here obeys. **This link is one of the fourteen row #1 repoints**, and it stays at the name that resolves today rather than at the name it will have - a See-also pointing at a file that does not exist is a broken link in every clone until the row lands.
