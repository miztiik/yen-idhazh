...
| `backend/tests/test_publish_telemetry.py` walks a growing collection | `COMMITTED_SHARDS` at line 30 globs `frontend/public/telemetry/` at import, and `test_migrating_the_committed_shards_changes_no_byte` copies and migrates **every** shard it finds - 2 files and 1,488,767 bytes on 2026-09-12, gaining one file a month. [`../CLAUDE.md`](../CLAUDE.md) section 13 refuses it. Found by row #4 in its own file, 2026-09-12 | Bounding it to the newest shard trades the growing read for a coverage hole: `cli._console_payload_faults` (lines 4096-4105) names six producers and `publish_telemetry` is not one, so nothing else in CI reads a committed telemetry shard back through its contract. Closing that needs `cli.py`, which row #5 owns. **It is a row, not a gap** - Fowler, 2026-09-12 - and the orchestrator is asked to schedule it after row #5 |
