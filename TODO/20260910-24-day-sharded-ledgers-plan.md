...
| `state/telemetry-aggregate/`, `state/score-archive/` | month | each summarises a month; a day file of a month's totals is a shape nothing consumes |
| `state/fingerprints.csv`, `state/runtime-counters.csv`, `state/feed-retirements.csv`, `state/day-validations.csv` | **one file, unsharded** | see below |
...
| --- | --- | --- | --- | --- |
| A | 1, 2 | `backend/idhazh/day_partition.py`, `backend/idhazh/ledger.py`, `backend/tests/{test_day_partition,test_ledger,test_marks}.py`, `tests/fixtures/day-partition/`, `docs/concepts/partitions.md`, `docs/concepts/growing-reads.md`, `docs/architecture/contracts/schemas.md`, `docs/architecture/publishing/{layout,telemetry-series,console-payloads}.md`, `docs/reference/measurements.md`, the two plan-docs that link the renamed page | `backend/idhazh/evals/writer.py`, `backend/idhazh/cli.py`, `backend/tests/test_evals.py`, `tests/fixtures/evals/index-rebuild/`, `docs/concepts/evaluation.md` | Both write a module under `backend/idhazh/` and a page under `docs/concepts/`. Different modules, different pages. Row #2 writes `cli.py`; row #1 does not |
...
- **Scope:** `backend/idhazh/day_partition.py` becomes the peer of `backend/idhazh/month_partition.py`. It owns what a `<YYYY>/<MM>/<DD>.csv` tree is, which names it refuses, which days a window of `n` days names, and which day an age in months keeps. `ledger._day_files`, `ledger._refuse_stray` and `ledger._days_in_window` move into it unchanged. **No collection changes grain in this row**, and the one measurement this plan is priced against is taken here.
- **Files touched:** `backend/idhazh/day_partition.py` (new, created by this row and written by no other), `backend/idhazh/ledger.py`, `backend/tests/test_day_partition.py` (new), `backend/tests/test_ledger.py`, `backend/tests/test_marks.py`, `tests/fixtures/day-partition/` (created by this row; nothing else in the plan writes there), `backend/utilities/measure_day_window.py` (new), `docs/concepts/month-partitions.md` renamed to `docs/concepts/partitions.md`, `docs/architecture/contracts/schemas.md`, `docs/architecture/publishing/layout.md`, `docs/architecture/publishing/telemetry-series.md`, `docs/architecture/publishing/console-payloads.md`, `docs/concepts/growing-reads.md`, `docs/reference/measurements.md`, `TODO/20260906-constant-cost-reads-plan.md`, `TODO/20260907-growing-reads-window-plan.md`
- **Acceptance gates:** `GATE-PY` with `backend/tests/test_day_partition.py` and `backend/tests/test_ledger.py`, `GATE-SUITE`. Plus, in this row:
  ...
  - every relative link in the repository that named `month-partitions.md` names `partitions.md` and resolves - **17 links in 7 files on 2026-09-11**, and a link whose visible text ends `.md` names the same basename as the file it opens;
  - `docs/reference/measurements.md` carries the day-window reading with its hardware, its date and its spread.
- **Oracle:** **One function decides what a day file is, and a tree it cannot place stops the read rather than being skipped.** **Driven from `tests/fixtures/day-partition/`** - one built tree holding `2026/09/07.csv`, `2026/13/01.csv`, `2026/09/32.csv`, a day stem written in Arabic-Indic digits, and a `notes.txt` - and every day-tree reader in the repository is driven over it, asserting all of them name the same one file and raise on the same four. **Behaviour rather than identity**, because two names can point at one object and still be called from a third place that reimplemented the check. It is the day-side twin of `test_the_month_readers_all_agree_on_what_a_month_is`, which caught three disagreeing month readers on 2026-09-08 and one file that was left alone in one store and deleted in another.
...
| 2a | **Seven of the 17 links are outside `docs/`, and two of those are in another plan's doc.** `TODO/20260907-growing-reads-window-plan.md` carries seven of them and is open work, so a worker executing that plan concurrently collides with this row. Read `git worktree list` before staging, and if the other plan has a row in flight this one waits rather than resolving by hand | Section 0.1 |
...
| 3 | Keep the page named `month-partitions.md` and extend it | 17 links stay correct and the title stays wrong for eight of seventeen partitioned collections. The link repoint is mechanical and a checker proves it; a title nobody trusts is not repaired by anything | `CLAUDE.md` section 5 |
...
| --- | --- | --- | --- |
| `docs/concepts/partitions.md` | as `docs/concepts/month-partitions.md`, 17 links in 7 files | 1 renames; 3, 5, 6, 7, 8 extend | The day rule beside the month rule, the per-collection table re-derived a row at a time, and the section on changing a collection's grain |
