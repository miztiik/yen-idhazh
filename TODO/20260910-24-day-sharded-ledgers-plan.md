...
| Rows, and the days they fall on | item-health **11,223 rows / 20 days, re-measured 2026-09-13 by row #5** (it read 10,423 / 18 on 2026-09-11, and the archive gains a day and about 400 rows a day); feed-health 13,516 / 21, re-measured by row #6; scores **9,584 / 23, re-measured 2026-09-13 by row #8 after that day's digest run landed** (8,784 / 20 on 2026-09-11); seen 69,842 / 21, re-measured by row #7 | the committed shards |
...
| CSV header bytes an ledger row | item-health **546, re-measured 2026-09-13 by row #5** (316 on 2026-09-11, before the two-call columns landed); feed-health 144, scores 465, seen 45, score-index 27 | the committed headers |
...
| 8 | `state/scores/` and `state/score-index/` file by day | 1, 2, 3 | F | DONE #669 | p24-8 | #669 | worker |
...
**Four singletons, and here the cause is a file conflict rather than a dependency.** Rows #5, #6, #7 and #8 have no ordering constraint between them - each moves a different ledger - and all four write `backend/idhazh/retention.py`, `backend/tests/retention/`, `docs/concepts/growing-reads.md` and `docs/architecture/contracts/schemas.md`. Three of the four also write `backend/idhazh/ledger.py` and `frontend/src/lib/server/payload.ts`. **This plan counts that rather than dressing it up.** Splitting the modules was considered and refused; the rejected-alternatives table under row #1 carries the count that decided it.
...
- **Files touched:** `backend/idhazh/day_partition.py` (new, created by this row; row #7 later folds `shards_in_window` into it), `backend/idhazh/ledger.py`, `backend/idhazh/publish_telemetry.py`, `backend/tests/test_day_partition.py` (new), `backend/tests/test_ledger.py`, `backend/tests/test_marks.py`, `backend/utilities/measure_day_window.py` (new), `docs/concepts/month-partitions.md` renamed to `docs/concepts/partitions.md`, `docs/reference/benchmarks/day-window-read.md` (new; this row creates the directory), `docs/architecture/contracts/schemas.md`, `docs/architecture/publishing/layout.md`, `docs/architecture/publishing/telemetry-series.md`, `docs/architecture/publishing/console-payloads.md`, `docs/concepts/growing-reads.md`, `docs/reference/pipeline-cost.md`, `TODO/20260906-constant-cost-reads-plan.md`, `TODO/20260907-growing-reads-window-plan.md`, `TODO/20260910-24-day-sharded-ledgers-plan.md`, `TODO/20260911-execution-order.md`
- **`tests/fixtures/day-partition/` was in this list and was dropped on 2026-09-11.** The oracle's tree is built under `tmp_path` from constants in `backend/tests/test_day_partition.py`, which is exactly what the month twin already does - `backend/tests/retention/` builds its trees from `NOT_MONTHS` and `OTHER_STRAYS`, Arabic-Indic stem included. Guardrail #7 is satisfied by real directories and a real walk, and no non-ASCII filename is committed ([`../CLAUDE.md`](../CLAUDE.md) section 5). Fowler, 2026-09-11.
- **`TODO/20260911-execution-order.md` joined the list on 2026-09-11**, because it names the renamed page twice and carried a wrong link count. `docs/reference/benchmarks/` joined it because [`../docs/reference/pipeline-cost.md`](../docs/reference/pipeline-cost.md) and [`../docs/reference/documentation-structure.md`](../docs/reference/documentation-structure.md) both refuse a two-arm race as an append to the instrument log; the log carries the figure in force and links to the record.
...
- **Acceptance gates:** `GATE-PY` with `backend/tests/test_day_partition.py` and `backend/tests/test_ledger.py`, `GATE-SUITE`. Plus, in this row:
  ...
  - `docs/reference/pipeline-cost.md` carries the day-window reading with its hardware, its date and its spread.
...
- **`publish_telemetry.migrate` was declared too, 2026-09-12.** It globs every committed shard under `frontend/public/telemetry/`, which is the same class of read one function down, and a module that declares one of its two growing reads and not the other is the "looked finished" failure this page's own design rationale warns about. It takes `-1`: rewriting every shard is the job, and a person runs it once on a contract change. No new file - the row's list is unchanged.
...
| 2 | What bounds it is the **store**, not the read: `observability.item_health_full_grain_months` is 14, so the ledger holds at most fourteen partitions once the fold has a candidate. **Corrected 2026-09-12 while executing this row**: this decision said the fold was switched off by `retention.dry_run`. It is not - `config.dry_run` is read at exactly one place, [`../backend/idhazh/retention.py`](../backend/idhazh/retention.py) line 548, and it gates the **visuals** prune. The telemetry fold is switched off by `--dry-run` on the workflow step ([`../.github/workflows/digest.yml`](../.github/workflows/digest.yml) line 1416), so a sentence citing the config knob would stop being true with nobody editing it. **Neither flag matters yet**: the fold has never had a candidate, because the oldest partition is `2026-08` and `month_partition.oldest_month_kept` first reaches it on **2027-10-01**. The declaration says that instead, because no flag can falsify it | Section 0.3; [`../CLAUDE.md`](../CLAUDE.md) Guardrail #12; Carmack, 2026-09-12 |
...
- **Files touched:** `backend/idhazh/ledger.py`, `backend/idhazh/retention.py`, `backend/idhazh/publish_telemetry.py`, `backend/idhazh/publish_source_health.py`, `backend/idhazh/publish_day_metrics.py`, `backend/idhazh/day_partition.py`, `backend/idhazh/cli.py`, `backend/idhazh/contracts/{item_health,public_telemetry,telemetry_aggregate}.py`, `schemas/telemetry-aggregate-row.schema.json`, `.github/workflows/digest.yml`, `backend/utilities/measure_ledgers.py`, `backend/utilities/migrate_item_health.py` (deleted), `frontend/src/lib/server/payload.ts`, `frontend/src/lib/server/runtime-counters.ts`, `frontend/src/routes/console/+page.server.ts`, `frontend/src/routes/console/machine/+page.server.ts`, `frontend/src/routes/console/model/+page.server.ts`, `frontend/scripts/build-canary.mjs`, ``backend/tests/test_ledger.py`, `backend/tests/retention/`, `backend/tests/test_publish_telemetry.py`, `backend/tests/test_publish_source_health.py`, `backend/tests/test_telemetry.py`, `backend/tests/test_measure_ledgers.py``, `tests/fixtures/state/measure-ledgers/item-health/2026/01/01.csv` (replacing `2026-01.csv`), `frontend/tests/item-health-day.spec.ts`, `state/item-health/2026-08.csv` and `state/item-health/2026-09.csv` (deleted) replaced by the day files the utility writes under `state/item-health/2026/08/` and `state/item-health/2026/09/` - **20 files on 2026-09-13, one a day the ledger holds a row for, and no other row in this plan writes in that directory** - `docs/concepts/partitions.md`, `docs/concepts/growing-reads.md`, `docs/concepts/pipeline-loop.md`, `docs/concepts/evaluation.md`, `docs/architecture/contracts/schemas.md`, `docs/architecture/sources/item-health.md`, `docs/architecture/sources/freshness.md`, `docs/architecture/publishing/telemetry-series.md`, `docs/architecture/publishing/retention.md`, `docs/architecture/publishing/console.md`, `docs/how-to/run-the-pipeline.md`
- **The list moved in execution, 2026-09-13, and this is what moved.** **Two names came off**: `backend/utilities/build_canary_day.py` imports the item-health contract and builds no path under `state/item-health/`, and `backend/tests/test_console_payloads_producer.py` names no item-health surface at all - neither needed an edit. **`backend/tests/pipeline/` came off too**: every site in it goes through `ledger.item_health_path`, which is the function that moved, so the module was green unchanged. **Fifteen names went on and none was avoidable.** `day_partition.py` gains `days_by_month`, the one bridge a monthly consumer of a day-filed store needs, because the fold and the publisher would otherwise hold a copy each and rows #6 and #8 a third. `publish_day_metrics.py`, `cli.py`, the three contracts and `digest.yml` each carried a sentence that became false. `schemas/telemetry-aggregate-row.schema.json` is regenerated because a class docstring IS a schema description; no field moved, so no `version` is stamped. The three console routes are `itemHealthRows`' callers and its cover changed unit. `test_publish_source_health.py` asserts which partitions the census opens, by name. And six `docs/` pages stated the old path as current.
- **Acceptance gates:** `GATE-PY` with the seven modules above, `GATE-SUITE`, `GATE-WEB`, `GATE-BROWSER`, `GATE-DAYS`, and the [`../CLAUDE.md`](../CLAUDE.md) section 12 smoke on `/console/` and `/console/machine/`. Plus, in this row:
  ...
  - `git diff --stat -- state/item-health` names two deletions and **twenty** additions and nothing else. **The plan said eighteen, measured 2026-09-11**; the archive gained two published days before the row ran, and the count is re-derived rather than trusted.
...
| 7 | **The growing read gets worse and this row says so rather than hiding it.** `publish_telemetry.publish` on its `months=None` arm opens every partition, so this row turns **2 opens into 20, measured 2026-09-13, and about 365 a year**. It is declared under Guardrail #12's escape hatch on the line that declares the read, with the count named and the store bound that answers it - `observability.item_health_full_grain_months`, 14 - because the alternative, a cover in months on the read, would leave a fresh clone permanently short of a mirror it never published. The count is asserted rather than described: `test_the_cover_is_the_months_the_caller_names` counts the handles both arms open | `CLAUDE.md` Guardrail #12; `docs/concepts/growing-reads.md` |
| 8 | `day_partition` gains `days_by_month`, and the fold and the publisher both take it. They are the two places a monthly consumer meets a day-filed store, and rows #6 and #8 add two more - four copies of one grouping is four places for the month boundary to stop agreeing. One function per grain is what row #1 exists to establish | Row #1 decision 3 |
| 9 | `frontend/src/lib/server/payload.ts` gains `shardDays` and `readDayShards` beside `shardMonths` and `readShards`, rather than teaching `readShards` two layouts. Three ledgers still file by month and read through it; one function answering two questions is the shape decision 3 refuses one directory over. `shardDays(n)` is `n + 1` with no rounding at all, because a day tree needs none - which is the arithmetic `day_partition.days_in_window` already uses | Row #1 decision 1 |
...
- **Files touched:** `backend/idhazh/ledger.py`, `backend/idhazh/retention.py`, `backend/idhazh/publish_feed_health.py`, `backend/idhazh/publish_console_band.py`, `backend/idhazh/month_partition.py`, `backend/idhazh/cli.py`, `backend/idhazh/contracts/{feed_health,public_feed_health,app_config}.py`, `schemas/app-config.schema.json`, `.github/workflows/digest.yml`, `backend/utilities/migrate_feed_health.py` (deleted), `frontend/src/lib/server/payload.ts`, `frontend/src/lib/feed-health.ts`, `frontend/src/routes/console/+page.server.ts`, ``backend/tests/test_ledger.py`, `backend/tests/retention/`, `backend/tests/contracts/``, `frontend/tests/{console-feeds,console,console-window-claims}.spec.ts`, `state/feed-health/2026-08.csv` and `state/feed-health/2026-09.csv` (deleted) replaced by the day files the utility writes under `state/feed-health/2026/08/` and `state/feed-health/2026/09/` - **21 files on 2026-09-13, one a day the ledger holds a row for, and no other row in this plan writes in that directory** - `docs/concepts/partitions.md`, `docs/concepts/growing-reads.md`, `docs/concepts/config.md`, `docs/concepts/pipeline-loop.md`, `docs/architecture/contracts/schemas.md`, `docs/architecture/sources/health.md`, `docs/architecture/publishing/retention.md`, `docs/architecture/publishing/console-payloads.md`, `docs/how-to/run-the-pipeline.md`
- **The list moved in execution, 2026-09-13, and this is what moved.** **Five names came off.** `measure_ledgers.py` and its test read `state/item-health/` alone and name no feed-health path; `probe_feeds.py` imports the feed-health enums and builds no path; `build_canary_day.py` writes through `ledger.append_health`, which is the function that moved; `publish_console.py` owns `FEED_HEALTH_DIRNAME` for the PUBLISHED tree, which stays monthly. `test_discover.py` and `backend/tests/pipeline/` came off too - every site in them goes through `ledger.health_path` or `ledger.load_health`. **Fourteen names went on and none was avoidable.** `month_partition.py` opened on a sentence naming `state/item-health/` and `state/feed-health/` as month-pruned, which row #5 left half true and this row makes false; `publish_console_band.py` states this read's cover; three contracts and a workflow comment each carried a sentence that became false; `app_config.py`'s `feed_health_keep_months` description said "fourteen month shards", so `schemas/app-config.schema.json` is regenerated (**no `version` stamp - a field description moved, no field did**); `+page.server.ts` is `feedResults`' caller and its cover changed unit; `backend/tests/contracts/` synthesised a `<month>-28` filename the day grain makes a lie; two more browser specs walked the ledger root for `*.csv`; and four more `docs/` pages stated the old path as current.
- **Acceptance gates:** `GATE-PY` with the five modules above, `GATE-SUITE`, `GATE-WEB`, `GATE-BROWSER`, and the section 12 smoke on `/console/`. Plus, in this row:
  ...
  - `git diff --stat -- state/feed-health` names two deletions and **twenty-one** additions and nothing else. **The plan said nineteen, measured 2026-09-11**; the archive gained two days and 1,510 rows before the row ran, and the count is re-derived rather than trusted.
...
| 5 | **The three tests that import that utility go with it**, and nothing replaces them. Two of the three assert what a deleted function does. The third walked the committed ledger to check the committed header, which is `require_matching_header`'s job and is proved fixture-driven one screen up - and its other clause counted rows still lacking a field, which is the fused shape `CLAUDE.md` section 13 rules out. What is lost is one command an operator could run to parse every committed day; if it is ever wanted, its home is `backend/utilities/measure_ledgers.py`, which pytest does not run | Fowler, 2026-09-13; `CLAUDE.md` section 13 |
| 6 | **The `FeedHealthRow` changelog entry naming the deleted utility is left exactly as it is**, in the contract and in the generated schema. It is a dated statement about what happened on 2026-09-02 and it was true; a changelog is appended to, never back-edited, and the schema is generated so the only way to change it is the back-edit. The two `docs/` pages that described the tool in the present tense are rewritten to the past instead - the passages stay, because what they teach is still true | `CLAUDE.md` section 11, section 10; Fowler, 2026-09-13 |
...
- **Verified in execution, 2026-09-13, and the sentence above was wrong in both halves.** `ledger._days_in_window` had already gone: **row #1 moved it to `day_partition.days_in_window`**, so there was no second function here to fold. And `ledger.shards_in_window` **stays**, because it is not the day helper under another name - it returns month stems, and `drift.read_windows` names the `state/scores/` month shards with it. That is the last month-grained ledger a window reaches and **row #8 is the row that can retire it**. What this row did instead is move `load_seen` and `prune_seen` onto `day_partition.days_in_window`, leaving `shards_in_window` one live caller and a docstring that says so. Its old docstring - "Public because the pruner keeps exactly what this returns" - became false in this commit and was corrected in it.
- **Files touched:** `backend/idhazh/ledger.py`, `backend/idhazh/day_partition.py`, `backend/idhazh/retention.py`, `backend/idhazh/cli.py`, `backend/utilities/measure_ledgers.py`, ``backend/tests/test_ledger.py`, `backend/tests/retention/`, `backend/tests/test_discover.py`, `backend/tests/pipeline/``, `state/seen/2026-08.csv` and `state/seen/2026-09.csv` (deleted) replaced by the day files the utility writes under `state/seen/2026/08/` and `state/seen/2026/09/` - **19 files on 2026-09-11, and no other row in this plan writes in that directory** - `docs/concepts/partitions.md`, `docs/concepts/growing-reads.md`, `docs/architecture/contracts/schemas.md`, `docs/architecture/sources/freshness.md`, `docs/architecture/publishing/retention.md`
- **The list moved in execution, 2026-09-13, and this is what moved.** **Four names came off.** `measure_ledgers.py` names no seen path at all - it reads `state/item-health/` - and `test_ledger.py`, `test_discover.py` and `backend/tests/pipeline/` each reach the ledger only through `ledger.seen_path`, `append_seen` or `load_seen`, which are the functions that moved, so all four were green unchanged. **Sixteen names went on and none was avoidable.** `day_partition.py` gains `date_of`, the peer of `month_of`, because this is the first prune whose boundary is a day rather than a month and the alternative was spelling the path layout a second time inside `retention.py`; `test_day_partition.py` covers it and drops the hand-spelled copy it already held. `month_partition.py` opened on a sentence naming `state/seen/` as month-pruned, which rows #5 and #6 left one third true and this row makes false. `contracts/seen.py` described the row as one of `state/seen/<YYYY-MM>.csv`, and a class docstring IS a schema description, so `schemas/seen-row.schema.json` regenerates; `contracts/app_config.py`'s `seen_window_days` description said "older shards", so `schemas/app-config.schema.json` regenerates too - **no `version` stamp on either, because a field description moved and no field did**. `contracts/base.py` named `state/seen/` as a directory that files by the `<YYYY>-<MM>` stem, which it no longer does. `measure_day_window.py` is row #1's benchmark instrument and calls `append_seen` and `load_seen` for its **month** arm, so this row inverts which arm is the ledger's own code and which is written out; `docs/reference/benchmarks/day-window-read.md` takes one dated line saying so, and its reading is untouched. `digest.yml` forecast the first file this store would lose. And seven more `docs/` pages stated the old path or the old margin as current.
- **`git diff --stat -- state/seen` names two deletions and twenty-one additions, not nineteen.** The plan said nineteen, measured 2026-09-11; the archive gained two days and 5,527 rows before the row ran, and the count is re-derived rather than trusted. Migrated 2026-09-13: **2 month shards to 21 day files, 69,842 rows in and 69,842 rows out**, filed by `first_seen_run[:10]`, read back through `day_partition.day_files` before either month shard was unlinked.
...
| 2a | **Corrected in execution, 2026-09-13: there were not two helpers to fold.** `day_partition.days_in_window` was already the one day helper - row #1 moved it out of `ledger` - and `ledger.shards_in_window` answers a different question at a different grain, with one live caller in `drift.read_windows` over the still-monthly `state/scores/`. The decision's reason survives its premise and is what this row honoured: `prune_seen` takes its keep-set from **the reader's own helper**, which is now `days_in_window`, so the two cannot drift. Retiring `shards_in_window` is row #8's, with the ledger that needs it | Row #1; verified against the tree |
...
| 4 | **`SeenPruneResult.deleted` and `kept` both become day paths, and no `days_removed` field joins them.** Rows #5 and #6 each added one, because their knobs are months: `deleted` had to stay a month to match `feed_health_keep_months`, so the files had to be named somewhere else or a caller would synthesise a `<month>-01` the ledger may never have held. `collect.seen_window_days` is counted in days, so there is no month in this prune at all and one unit answers both. `cli._prune_seen_shards` returned exactly that synthesised `<month>-01` and now returns the list | Row #6 decision, inverted by the unit; Fowler |
| 5 | **The prune's log line counts what it kept rather than naming it.** At month grain `kept` was at most four stems and the line named every one; a 90-day window makes it up to 91 paths, and a line that joined them would be a wall nobody reads. The paths are still printed one to a line by `cli._report_removals`, which is the deliverable of a dry run - so nothing is lost and the line gains the oldest file kept, which is the number that says whether the boundary landed where the reader needs it | `CLAUDE.md` section 0b; Guardrail #10 |
...
- **Files touched:** `backend/idhazh/evals/writer.py`, `backend/idhazh/evals/archive.py`, `backend/idhazh/retention.py`, `backend/idhazh/publish_scores.py`, `backend/idhazh/drift.py`, `backend/idhazh/ledger.py`, `backend/idhazh/cli.py`, `backend/utilities/reband_scores.py`, `backend/utilities/label_queue.py`, `backend/utilities/data_wrangler.py`, `backend/utilities/migrate_score_ledger.py` (deleted), `backend/idhazh/contracts/{observation_index,public_eval,score_archive}.py`, `schemas/{observation-index-row,score-archive}.schema.json` (regenerated), `frontend/src/lib/server/payload.ts`, `frontend/src/routes/console/{,machine/,model/}+page.server.ts`, ``backend/tests/test_evals.py`, `backend/tests/test_ledger.py`, `backend/tests/test_drift.py`, `backend/tests/retention/`, `backend/tests/pipeline/`, `backend/tests/workflows/``, `tests/fixtures/evals/index-rebuild/` (moved to the day layout), `frontend/tests/console.spec.ts`, `state/scores/2026-08.csv` and `state/scores/2026-09.csv` (deleted) replaced by the day files the utility writes under `state/scores/2026/08/` and `state/scores/2026/09/` - **23 files on 2026-09-13**, not the 20 counted on 2026-09-11 - and `state/score-index/2026-08.csv` and `state/score-index/2026-09.csv` (deleted) replaced by the day files the rebuild writes under `state/score-index/2026/08/` and `state/score-index/2026/09/` - **23 files, one beside each ledger day; no other row in this plan writes in either directory** - `docs/concepts/partitions.md`, `docs/concepts/growing-reads.md`, `docs/concepts/evaluation.md`, `docs/concepts/config.md`, `docs/architecture/contracts/schemas.md`, `docs/architecture/publishing/{retention,layout,console-payloads,telemetry-series}.md`, `docs/how-to/run-the-pipeline.md`, `.github/workflows/digest.yml`
- **Acceptance gates:** `GATE-PY` with the six modules above, `GATE-SUITE`, `GATE-WEB`, `GATE-BROWSER`, and the section 12 smoke on `/console/` and `/evals/`. Plus, in this row:
  ...
  - **the growing read `recorded_observations` creates is declared with its new count.** It opens one file a partition, and this row turns "one a month for ever" into "one a day for ever". The declaration names the store bound that answers it - `observability.scores_full_grain_months` is 14, so once the prune is switched on the live index holds at most fourteen months of days and everything older is one `score-archive` document a month. It also says plainly that `--dry-run` is on the workflow step today, so nothing prunes and the count grows until it is flipped;
  - `git diff --stat -- state/scores state/score-index` names four deletions and **forty-six** additions and nothing else. Forty-six rather than the forty this row was written with, because the archive gained three days between 2026-09-11 and the row landing.
...
- **What this row does not do:** it does not move `state/score-archive/`, which stays monthly because it summarises a month, and it does not change the grain of `frontend/public/scores/`. **It changes one schema and not the two the row expected.** `EvalRow` and `ObservationIndexRow` keep their shapes exactly. `ScoreArchive` did not: `source_sha256` used to digest one month shard's bytes and now digests that month's day files in day order, which is a shifted meaning and so a `version` stamp and a changelog entry (`CLAUDE.md` section 11). **No read side migrates**, and that is checked rather than assumed - `state/score-archive/` has never been written, because the prune that writes it takes `--dry-run` from the workflow step.
...
| 5 | **The growing read gets worse and this row says so rather than hiding it.** `indexed_observations` opens every index partition, so this row turns 2 opens into 23, and about 365 a year. It is declared under Guardrail #12's escape hatch with the count named and the store bound that answers it, because the alternative - a bounded index - is a dedupe that would let a January measurement come back in February | `CLAUDE.md` Guardrail #12; `docs/concepts/growing-reads.md` |
...
| 7 | **Corrected in execution, 2026-09-13: `ledger.shards_in_window` is NOT retired.** Row #7 recorded that this row could retire it, on the premise that `drift.read_windows` was its one live caller. `drift` moved to `days_in_window` here, and the premise was still wrong: four callers remain - `test_contracts`, `test_retention`, `test_ledger` and `backend/utilities/measure_day_window.py` - and they ask a month question that survives a grain change, which is how many month-shaped `keep_months` buckets a day-counted window reaches. That is the reason `item_health_full_grain_months` is 14 and not 13. Deleting it would put one calendar rule in three places; its docstring is corrected instead | Guardrail #5; verified against the tree |
| 8 | **`rebuild_index` takes days and `idhazh rebuild-score-index --month` keeps its flag**, expanding a month to that month's committed days. The partition it rewrites is a day, so the function takes days; the thing an operator knows is that a month looks wrong, and naming thirty-one days to say so is a worse command. It is the same split the prunes already use, where a `keep_months` knob deletes day files | Row #6 decision; Fowler's split of knob unit from file unit |
...
| `docs/reference/pipeline-cost.md` | yes | 1 | The day-window reading with its hardware, its date and its spread |
...
**One item in section 11 was not a finding and was never scheduled.** `backend/tests/test_publish_telemetry.py` globbed `frontend/public/telemetry/` at import and migrated every shard it found. Fowler ruled it a row rather than a gap on 2026-09-12 and asked for it after row #5; no row was ever cut, so this plan closed with it outstanding. It is **defect 21** in [`20260823-known-defects-plan.md`](20260823-known-defects-plan.md) and closed there on 2026-09-13 - the read-back moved into `cli._console_payload_faults`, where section 13 says it belongs, and the test now builds its own projection.

---

## 11. Facts the repository contradicted, and gaps nobody owns

**Six claims this plan was briefed with or wrote for itself were wrong on the tree.** The first four were re-measured 2026-09-11 in a worktree cut from `origin/main` at `3c880832`; claims 5 and 6 were found by row #4 on 2026-09-12 at `242a701c`; claims 7 and 8 by row #5 on 2026-09-13. They are recorded here because the standing rule says a row that finds a wrong fact fixes it where it was carried.
...
| 5 | `retention.dry_run` is `true`, so every prune in this repository is a no-op | **Two errors in one sentence, found by row #4 on 2026-09-12.** `config.dry_run` is read at exactly one place - `retention.py` line 548 - and it gates the **visuals** prune alone. The five prunes in `cli.stage_prune_state` are switched off by `--dry-run` on the workflow step (`digest.yml` line 1416) instead. And the console-payload prunes are switched off by nothing at all: `publish_console.prune_months` takes no `dry_run` parameter and `publish_series` calls it on every assemble, so **six of the seven mirrors in claim 1 really are capped** at their `keep_months`. `frontend/public/telemetry/` is the one exception, because its deletion lives in `retention.prune_telemetry` inside the dry-run step. This table's own gap row below said the wrong thing and is corrected with it |
| 6 | Row #4's `publish_telemetry.publish` would be bounded in fact by the fourteen-partition cap | The cap has **never had a candidate**. `oldest_month_kept(2026-09-12, 14)` is `2025-08` and the oldest partition on disk is `2026-08`, so the fold skips both shards even with `dry_run=False`. The first partition it can take is `2026-08` on **2027-10-01**. A declaration resting on the cap would have been describing something that has not happened |
| 7 | Row #5's dispatch brief named `indexed_observations` as the growing read this row makes worse | **That read is row #8's**, not this one's: `evals.writer.indexed_observations` opens `state/score-index/`, which this row does not touch. The numbers in the brief - 2 opens to about 20, about 365 a year - are right for **this** row's equivalent read, `publish_telemetry.publish` on its `months=None` arm, and the declaration was written there. Found by row #5, 2026-09-13 |
| 8 | Row #5's four dependencies and its file list were accurate | Three of the four were. **`backend/utilities/build_canary_day.py` and `backend/tests/test_console_payloads_producer.py` build no item-health path**, so neither needed an edit, and `backend/tests/pipeline/` addresses the ledger only through `ledger.item_health_path` and was green unchanged. Fifteen files the row did not name were unavoidable; the row's own list carries them and says why. Found by row #5, 2026-09-13 |
| 9 | Row #8's file list, and row #7's note that row #8 could retire `ledger.shards_in_window` | **Both wrong, found by row #8 on 2026-09-13.** `measure_ledgers.py` names no score path and `build_canary_day.py` writes through `writer.append` and `writer.ledger_path`, which are the functions that moved - neither needed an edit, and nor did `test_labels`, `test_reband_scores` or `test_measure_ledgers`. Eleven names the row did not list went on instead: `drift.py` and `ledger.py`, three contracts and two regenerated schemas, `data_wrangler.py`, three console routes, `test_ledger`, `test_drift`, `test_workflows`, the moved `tests/fixtures/evals/index-rebuild/` tree and six more `docs/` pages. And `shards_in_window` **stays** - decision 7 above says why |
| 10 | "`git diff --stat -- state/scores state/score-index` names four deletions and forty additions" | **Four deletions and forty-six additions.** 23 day files a store rather than 20, because the archive gained three days between the plan being written and the row landing - the last of them arriving on `main` while this row was in flight, which is why the migration was taken twice. **9,584 rows in, 9,584 rows out**, and the regenerated index holds the identical 9,584 digests the two month files held |
...
| The five state prunes are switched off | `cli.stage_prune_state` - seen, traces, feed-health, scores and the telemetry fold - takes `--dry-run` from the workflow step and deletes nothing. **Corrected 2026-09-12 by row #4**: this gap named `retention.dry_run` as the cause and "every prune in this repository" as the scope, and both were wrong. See claim 5 | Flipping it is a decision about the whole repository, not about this plan. Plan 23 section 26 names it too |
| `backend/tests/test_publish_telemetry.py` walked a growing collection | `COMMITTED_SHARDS` at line 30 globbed `frontend/public/telemetry/` at import, and `test_migrating_the_committed_shards_changes_no_byte` copied and migrated **every** shard it found - 2 files and 1,488,767 bytes on 2026-09-12, gaining one file a month. [`../CLAUDE.md`](../CLAUDE.md) section 13 refused it. Found by row #4 in its own file, 2026-09-12 | Bounding it to the newest shard traded the growing read for a coverage hole: `cli._console_payload_faults` named six producers and `publish_telemetry` was not one, so nothing else in CI read a committed telemetry shard back through its contract. **It was a row, not a gap** - Fowler, 2026-09-12 - and the orchestrator was asked to schedule it after row #5. **No row was cut and this plan closed with it open.** It is defect 21 in [`20260823-known-defects-plan.md`](20260823-known-defects-plan.md), closed 2026-09-13: the read-back moved into the producer gate and the test now builds its own projection |
