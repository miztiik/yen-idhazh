...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | A new engine behind the same seam | - | A | BLOCKED | - | - | - |
...
- **BLOCKED 2026-09-13, before any code was written, and it needs an owner ruling.** The row says the d3 emitters sit behind the same render call site. That call site is Python, in the sharded `work` job of `.github/workflows/digest.yml`, which installs Python and no Node - and d3 is JavaScript. Three answers are defensible and each writes different code: Python shells into Node against `frontend/node_modules`; the renderer moves into the SvelteKit prerender and the backend stops writing SVG; or the backend emits SVG from its own scale maths and d3 appears only in the browser. This row's own wording points at the first, the next four rows' oracles - browser-measured width, computed font size, per-mark computed style - point at the second, and nothing in the plan says which. `CLAUDE.md` section 4 calls the backend-frontend boundary a structural invariant with one correct side, so a Python process spawning Node is a section 6 level 5 call and no agent makes it. Cost worth knowing before the ruling: rendering happens per item inside the sharded `work` job, so Node and an `npm ci` would be installed in **every shard**.
- **Four things in the file list above are wrong, found in the same pass.** `frontend/src/lib/charts/**` is the **console's** engine, which this row's own rejected alternative 3 says is not migrated - only the "or a new digest-visual module" half is right. `.github/workflows/digest.yml` is missing and the render job needs it unless the ruling removes the need. The plan's module matrix gives `bar` the axis module while decision 4 makes the selection module hydrated-path-only, and those conflict, because the axis renders only by being called on a selection. And decision 3 says exact-pin every module, but the two d3 modules already present are caret-ranged, so repinning them moves the console's lockfile too. **The list is left as it stands rather than corrected, because the ruling decides most of it.**
- **The recommendation the scoping worker made, for the owner to accept or refuse:** split this row in two. A contract row that mints `renderer_version` and restates `backend/idhazh/contracts/visual_decision.py`'s spec field, which today is declared as the Vega-Lite JSON the renderer draws and becomes false the moment the engine changes - a shifted meaning, which `CLAUDE.md` section 11 calls breaking. Then the engine row, dispatched once the build-time host question above is answered.
