...
| 7a | The classification code gets its own package | - | E | IN-FLIGHT | p23-7a | - | worker |
...
- **What moves, named by the banners already in the file.** `visual_planner.py` is **2,223 lines** and carries three sections its own comments separate (re-measured 2026-09-12 at `origin/main` 60a05fa4; the row was written against 1,824 lines with the banners at 641 and 1457, and the module has grown since): lines 1 to 665 are the visual planner proper - facts, chart reachability, the spec compiler; line 666 opens `# --- Call 1: the model reads the article and points at it`; line 1482 opens `# --- Call 2: the summary and the plan, over the prefix call 1 already paid for`. **Find the two banners rather than trusting these numbers, which move whenever the module does.** **The two call sections move to `backend/idhazh/classify/calls.py`. The visual planner stays where it is**, because it is not classification and moving it would make this row a rename of the module rather than an extraction from it.
...
- **Oracle:** **The request payloads are byte-identical before and after.** Driven from `tests/fixtures/planner/recorded-call-payloads.json` - the request bodies the current code builds for one fixture article, captured in the commit before the move. **Neither that file nor `tests/fixtures/planner/` exists yet (checked 2026-09-12), so capturing it from the pre-move code is the row's first step**, and it is captured before a line is moved or it records the new behaviour rather than the old. The test builds them again through `classify.calls` and asserts equality on the bytes. **A pure move proves itself by producing the same bytes; anything else is a behaviour change wearing a refactor's name**, and this is the one oracle in the plan that goes red on an improvement.
