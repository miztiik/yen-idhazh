...
| Prerender | The two dated reading routes render in the browser now. What is still prerendered is `/`, `/archive/`, `/evals/` and the three console routes - six routes, named in `frontend/prerender-guard.js` and re-verified 2026-09-11. **Prerender is legacy** | **Its removal belongs to the UI shell plan** and this plan does not scope it. Two things follow and are recorded rather than acted on: no row here prerenders anything new, and **no row here asserts that prerender output is byte-identical between builds**. It is not - `kit.version.name` defaults to `Date.now`, so two builds of one unchanged tree disagree on about 20 percent of `build/` by filename |
