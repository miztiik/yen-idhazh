...
| `docs/reference/benchmarks/<YYYY-MM-DD>-label-logprob-mode.md` | **no** - and neither does the directory | P5 | Which distribution the pinned runtime reports at a masked token, the two replies side by side, the build it was taken on. **A record in `docs/reference/benchmarks/`**, which `CLAUDE.md` section 5 prescribes and nothing has yet created. **It is not necessarily the first one**: [`20260911-26-retire-prerender-plan.md`](20260911-26-retire-prerender-plan.md) row #1 writes a different record into the same directory and is unblocked today, so either may create it, and whichever lands second names the row that did ([`20260911-execution-order.md`](20260911-execution-order.md) section 6). Corrected 2026-09-11; this table said "the first record" |
