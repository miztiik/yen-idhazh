...
| 4 | An event gets a lifecycle | 3 | F | IN-FLIGHT | p23-4 | - | worker |
...
| --- | --- | --- | --- | --- |
| A | 5, P1, 20 | `backend/idhazh/rank.py`, `backend/idhazh/contracts/base.py`, 13 contract models and the 13 schemas they generate, `backend/tests/{test_contracts,test_discover,test_rank}.py`, `backend/idhazh/render/write.py`, `docs/architecture/publishing/{layout,visuals}.md`, `docs/architecture/sources/{freshness,item-health}.md`, `docs/how-to/run-the-gates.md` | `CLAUDE.md`, `AGENTS.md`, `docs/agents/guardrails.md` | Both write `docs/`. Different tiers, no shared page. **The group's third row, #20, writes one docs page - `docs/concepts/placement.md` - plus `backend/tests/{test_order_of_the_day,test_marks}.py` and one fixture, none of which the other two open.** Row #5 names `backend/tests/test_contracts.py` and row #20 names `test_marks.py`; they are different modules. **Re-diffed 2026-09-12 after row #5 widened by four files and narrowed from 44 schemas to 13: still disjoint** |
...
| E | 3, 7a | `backend/idhazh/contracts/{taxonomy,article,digest_day,digest_view,__init__}.py`, four named schemas, `backend/idhazh/tag.py`, `backend/utilities/build_canary_day.py`, `frontend/src/lib/payload/{lenses,project}.ts`, `frontend/src/lib/components/LensChips.svelte`, `backend/tests/{test_contracts,test_tag}.py`, `frontend/tests/lenses.spec.ts`, `tests/fixtures/digest/retired-lens-item.json`, `docs/concepts/taxonomy.md` | `backend/idhazh/classify/{__init__,calls}.py`, `backend/idhazh/{visual_planner,elements}.py`, `backend/idhazh/contracts/visual.py`, `backend/utilities/measure_two_calls.py`, `backend/tests/{test_classify,test_visual_planner,test_elements,test_marks,conftest}.py`, `tests/fixtures/planner/recorded-call-payloads.json`, `docs/architecture/{summarize/prompt,extraction/elements}.md` | Both touch `backend/tests/`, and they name different modules. **Row #3's list was re-derived from the tree on 2026-09-12 and grew by five files**, one of which - `frontend/src/lib/bands.ts` - it lost, because that file holds no lens copy. **Row #7a's grew by seven on 2026-09-12 when it ran, and the pair was re-diffed afterwards: still disjoint.** The nearest approach is now `backend/idhazh/contracts/`, where #3 writes five models and #7a writes one - `visual.py`, and only a docstring line in it, so no schema regenerates and the drift gate sees one diff and not two |
...
**The nine singletons the earlier table carried had three named causes and a fourth nobody had counted.** `backend/idhazh/visual_planner.py` was written by rows 7, 8, 9, 10, 11, 12 and 16 - seven rows against a module that was **2,223 lines** when row #7a opened it (1,824 when this paragraph was first written on 2026-09-11, and 985 after the move landed). `docs/concepts/classification.md` was written by six. `schemas/` regenerates whole under row #5. And the fourth: **`backend/tests/test_contracts.py` was named or globbed by every row but four**, which no paragraph in the plan mentioned.
...
- **Five files joined the list on 2026-09-13 when the row ran, and the reason is Guardrail #6.** The row's own oracle says "The three floors are config, not literals", and this repository's config is `config/idhazh.json` behind `backend/idhazh/contracts/app_config.py` - so the floors land as a `reference_dataset` block, which regenerates `schemas/app-config.schema.json` and takes today's `version` with it (`CLAUDE.md` section 11). `backend/idhazh/contracts/export.py` carries an explicit contract list, so a new contract that is not named there generates no schema at all. And `pyproject.toml` declares `courlan`, which is what reads the public suffix list: it is already installed as a trafilatura dependency, so the install cost is zero, but a direct import of a transitive dependency breaks the day the package above it drops it. **Group B's other row is #P4 and it landed on 2026-09-12**, so the widening collides with nothing.
- **The article text is fetched, because there is nowhere else it could come from** (established 2026-09-13). The committed archive carries `source_url`, a title and our own summary, and no article body - a reader-facing page never carries the source text (`CLAUDE.md` section 0a). The one place article text lives in this repository is `corpus/corpus.jsonl`, and the oracle's third arm forbids drawing from it. So `build_reference_dataset.py fetch` takes each article once, by hand, honouring `robots.txt`, through the same trafilatura-plus-sanitizer path the pipeline uses. **Measured 2026-09-13** on an Intel Core i7-1265U over the live web: about 9.5 seconds an article including the robots read and the politeness delay, and 87 percent of attempts came back usable.
- **`TODO/REFERENCE_SET_1_URLS.txt` is not this row's input, and the name is the whole reason to say so.** It is 756 URLs over 27 hosts, gathered in PRs #593, #595 and #599 for [`20260829-reference-set-handover.md`](20260829-reference-set-handover.md) - the **summariser** reference set, which is 500 hand-written summaries under `tests/fixtures/reference/` and a different artefact with a different purpose. Nothing in the repository reads the file. It could not build this set either: 27 hosts collapse to about 10 registrable domains, against a floor of 20 on **each** side.
...
- **Oracle:** **No source domain appears on both sides of the split, and neither side is empty.** **Driven from `tests/fixtures/reference-dataset/`** - a six-article, two-domain fixture the test builds a split from - so it goes red on a builder change without opening the committed dataset, and it costs the same next year as today (Guardrail #12). The same assertions then run once over the two committed lists, which are fixed in size.
  ...
  **"The test asserts the intersection is empty" is the one sentence of this oracle that had to change, and section 13 is why** (corrected 2026-09-13 by the row's own worker). `CLAUDE.md` section 13 names **the corpus** in its list of collections a test may not walk, and it gives a check over committed data three legal fates and no fourth: delete it where a fixture covers the rule, move it into the producer, or make it an operator surface under `backend/utilities/`. All three are taken here. The rule is proved on `tests/fixtures/reference-dataset/overlapping-corpus/`, a real one-row window and one-key holdout driven through `corpus.read_rows`; the **builder refuses** at build time; and `build_reference_dataset.py verify` is the operator surface that asks the question of the committed set. The same correction applies to "the same assertions then run once over the two committed lists" - `verify` runs them, pytest does not. A pull request went red on 2026-09-13 on exactly the shape this avoids.
- **The row shape does not carry its own split, and that is a deliberate subtraction** (2026-09-13). Decision 3 makes `splits/dev.txt` and `splits/test.txt` the split. A `split` field on the row would be a second copy of that fact, and `CorpusRow`'s own changelog already states the rule this project follows - "a stored copy of a derivable value is a second thing that can disagree with the first". Leaving it off also gives the oracle something real to check: the two committed lists must **partition** `dataset.jsonl` exactly, which is a cross-file property rather than a self-check.
...
| 3 | Splits are **committed as lists**, never recomputed from a seed. A recomputed split moves when the row order moves, and then last month's number and this month's number were taken on different sets | `CLAUDE.md` Guardrail #10 |
...
| # | Option | Why rejected | Authority |
| --- | --- | --- | --- |
| 1 | Measure against the committed archive instead of a frozen set | The archive is not labelled, it grows every four hours, and reading it once an item is a Guardrail #12 breach. A number taken over a moving set cannot be compared with itself | `CLAUDE.md` Guardrail #12 |
...
| # | Decision | Authority |
| --- | --- | --- |
| 1 | This row is a prerequisite, not a nicety. **Two figures on one line of section 0.3's running-total table are estimates derived by scaling a `llama-bench` ratio, and this row replaces them with a measurement.** They are `+9.7` and `+20.8`, both on the "Plan 11 row 6" line - one line, two columns, and this decision said "two lines" until 2026-09-12. `measurements.md` says of its own bench table that those figures "are not the prompt-cache cost in the live digest path", which is exactly the cost every labelling row spends | `CLAUDE.md` Guardrail #10 |
...
- **Files touched:** `backend/idhazh/fingerprint.py`, `backend/idhazh/contracts/{fingerprint,run_manifest,day_metrics,eval_row,label_row,observation_index,public_eval,score_archive,summary,evidence,qualification,app_config}.py`, `backend/idhazh/evals/{archive,evidence,labels,score,writer}.py`, `backend/idhazh/{assemble,cli,corpus,drift,fetch,summarize,publish_day_metrics}.py`, `schemas/{fingerprint-row,run-manifest,day-metrics,eval-row,label-row,public-eval,score-archive,summary,evidence-item,qualification-report,qualification-shard,app-config}.schema.json`, `state/fingerprints.csv`, **`frontend/src/lib/console/eval-instruments.ts`, `frontend/src/lib/server/model-work.ts`, `frontend/src/routes/console/+page.server.ts`, `frontend/src/routes/console/machine/+page.server.ts`, `frontend/src/routes/console/model/+page.server.ts`, `frontend/tests/console-model-instruments.spec.ts`, `frontend/tests/console-model-rule.spec.ts`, `frontend/tests/support/reduction-input.ts`**, `backend/utilities/{build_canary_day,label_queue,measure_ledgers}.py`, **the fifteen backend test modules that name the field** - `backend/tests/{test_classify,test_console_payloads_producer,test_contracts,test_corpus_harvest,test_day_metrics_producer,test_drift,test_evals,test_extraction_health,test_fingerprint,test_grader_length_bias,test_labels,test_pipeline,test_qualify,test_summarize,test_telemetry}.py` - `tests/fixtures/evals/prompt-changed-window.csv` (**new - this row creates it**), `CLAUDE.md`, and the nine docs pages that name it - `docs/architecture/contracts/{determinism,schemas}.md`, `docs/architecture/publishing/{console-charts,retention}.md`, `docs/architecture/summarize/prompt.md`, `docs/archive/measurements-2026-08.md`, `docs/concepts/{config,evaluation}.md`, `docs/reference/github-actions.md`.
  ...
  **The count is re-measured, not inherited.** `git grep -l pipeline_fingerprint` returns **152** files on 2026-09-12 - five more than the 147 this row carried on 2026-09-11, because the archive gains about two a day - of which 15 are backend tests and 9 are docs. **The fifteenth backend test module is `test_classify.py`, not `test_visual_planner.py`**: re-measured 2026-09-12 before dispatch, `test_visual_planner.py` no longer names the field and `test_classify.py` does. Corrected by the orchestrator, 2026-09-12. The rest are frozen published days, committed day-metrics files, fixtures, 12 contract models, 12 generated schemas, 4 score mirrors and the fingerprint ledger, and **this row opens none of them, because none of them is a reader.**
...
**It did neither, and the measurement is why. Ruled 2026-09-12 by Fowler, on a measurement this row took before choosing.** `run_id` is `<date>-<n>` and therefore an execution identity: measured over the committed `frontend/public/scores/*.csv`, 9,435 rows across 22 dates, it makes **21 of the 21 day transitions a boundary** - against 9 for the stamp and 1 for `model_id`. A panel repointed at it draws a model-swap rule every single day, which is a wrong answer rather than a missing one and is exactly what rejected alternative 3 already refuses. **The arm taken instead is the recorded inputs themselves**: `RunRecord.inputs` carries the same seventeen values by name, the console already opens every day's `run.json` through `payload.ts loadManifests`, and `pipelineChanges` compares the named set. **Fowler's rule, which is the sentence that keeps it a record rather than the gate renamed**: do not publish a digest - one opaque token affords exactly one operation, equality, which is a gate's only operation; a set of named values affords reading, and there is nothing there to gate on. The days committed before the cutover are still read from the score ledger's column, behind a dated `RECORDED_INPUTS_FROM` constant carrying its removal condition on the declaring line, because a chart with no rules over a window holding 9 real boundaries says "nothing moved" - and the cutover day itself is never a boundary, because the store changed there and the pipeline did not. **What it cost beyond this row's file list: one file, `frontend/src/lib/server/payload.ts`**, which surfaces `inputs` on the run record it already reads.

**It did neither, and the measurement is why. Ruled 2026-09-12 by Fowler, on a measurement row #1a took before choosing.** `run_id` is `<date>-<n>` and therefore an execution identity: measured over the committed `frontend/public/scores/*.csv`, 9,435 rows across 22 dates, it makes **21 of the 21 day transitions a boundary** - against 9 for the stamp and 1 for `model_id`. A panel repointed at it draws a model-swap rule every single day, which is a wrong answer rather than a missing one and is what rejected alternative 3 already refuses. **The arm taken instead is the recorded inputs themselves**: `RunRecord.inputs` carries the same seventeen values by name, the console already opens every day's `run.json` through `payload.ts loadManifests`, and `pipelineChanges` compares the named set. **Fowler's rule, which is the sentence that keeps it a record**: do not publish a digest - one opaque token affords exactly one operation, equality, which is the gate's operation and the only one; a set of named values affords reading, and there is nothing there to gate on. The days committed before the cutover are still read from the score ledger's column, behind a dated `RECORDED_INPUTS_FROM` constant with its removal condition on the declaring line, because a chart with no rules over a window holding 9 real boundaries says "nothing moved" - and the cutover day itself is never a boundary, because the store changed there and the pipeline did not. **What it cost beyond the row's file list: one file, `frontend/src/lib/server/payload.ts`**, which surfaces `inputs` on the run record it already reads.
...
| 4 | **The field stays in this row; its readers go.** `pipeline_fingerprint` is named in **152 files, 15 of them backend tests and 9 of them docs** (re-measured 2026-09-12 with `git grep -l`, superseding the 147 of 2026-09-11, the 146 and 144 earlier drafts carried and the 93 of the one before that). Of those, the frozen published days, the committed day-metrics files and the fixtures are **not touched by this row, because none of them is a reader.** "No prisoners" applies to every reader and every gate, which is what section 0.1 is about | Section 0.1; decision 5 |
...
| 2 | **One popper, not twelve.** A `model_validator(mode="before")` per model is twelve copies of one rule, and the twelfth is the one somebody forgets. One helper function in `base.py`, imported by the twelve, and a test that asserts every model declaring the popper is in the same list the test reads | `CLAUDE.md` Guardrail #5 |
...
- **Scope:** `LensId` and `EventType` stop being closed `StrEnum`s in Python and become `Slug`. The vocabulary stays closed by `config/taxonomy.json` rather than by the type: `tag.tags` returns keys of the mapping that file builds and nothing else, so nothing can invent a label (Guardrail #11). A retired id keeps rendering in the days that already carry it.
...
- **Files touched:** `backend/idhazh/rank.py`, `backend/idhazh/contracts/base.py`, **the 13 contracts that carry an item id and the 13 schemas they generate** - `article`, `digest_day`, `digest_view`, `element`, `eval_row`, `evidence`, `item_health`, `qualification`, `run_plan`, `search_index`, `seen`, `summary` and `visual_decision` - `backend/tests/{test_contracts,test_discover,test_rank}.py`, `tests/fixtures/discover/two-candidate-pools.json`, `docs/architecture/publishing/layout.md`, `docs/architecture/publishing/visuals.md`, `docs/architecture/sources/freshness.md`. **Widened on 2026-09-12 when the row ran**, by three files the list did not name and whose sentences the row makes false: `backend/idhazh/render/write.py` (one docstring line), `docs/architecture/sources/item-health.md` (three passages naming `assign_ids` and the ten digits) and `docs/how-to/run-the-gates.md` (one clause describing the grammar). None is written by any other row of this plan.
- **Why 13 schemas and not one, and not 44 either:** `ITEM_ID_PATTERN` is what `ItemId` is built from in `backend/idhazh/contracts/base.py` (verified 2026-09-12), and `ItemId` is used across the contracts - so widening the pattern regenerates every schema that carries an item id, and **measurement says that is 13 of the 44**, not all of them. The earlier `every file under schemas/` was never measured. Nothing that regenerates one of those 13 may run beside this row, and its group-A partners touch neither a contract nor a schema.
...
- **Acceptance gates:** `GATE-PY` with `backend/tests/test_label_logprobs.py`, `GATE-SUITE`. **No `GATE-WEB`, no `GATE-BROWSER` and no dispatch** - nothing renders and nothing runs in CI. This row starts a local server on a developer machine and commits its finding, the way the prompt loop already does (`CLAUDE.md` Guardrail #2). Plus:
...
- **What moves, named by the banners already in the file.** `visual_planner.py` was **2,223 lines** and carried **five** banner sections, not the three an earlier draft of this row named (corrected 2026-09-12 by the row itself; the row was first written against 1,824 lines with the banners at 641 and 1457, and the module has grown since): lines 1 to 665 are the visual planner proper - facts, chart reachability, the spec compiler; line 666 opens `# --- Call 1: the model reads the article and points at it`; line 1482 opens `# --- Call 2: the summary and the plan, over the prefix call 1 already paid for`; line 1926 opens `# --- The gate that refuses before the plan is drafted`; line 2092 opens `# --- The ladder that steps down`. **Find the banners by their text rather than trusting these numbers, which move whenever the module does.** **The two call sections move to `backend/idhazh/classify/calls.py`. The visual planner, the gate and the ladder stay where they are**, because none of the three is classification and moving them would make this row a rename of the module rather than an extraction from it. It landed at 1,274 lines moved out and 985 left behind.
- **One function inside the call-2 banner stays, and it is the row's one deviation from its own line range.** `plan_lost_to_the_budget` builds no request and parses no reply: it is a `VisualDecision` factory whose three siblings - `suppressed_by_the_gate`, `declined_by_the_model` and `refused_by_the_validator` - all sit in the gate section and all stay. None of the five has a production caller yet, so they are one set waiting on one row; moving one would leave row #7b importing three `none`-builders from one module and a fourth from another, which is the two-homes problem decision 2 refuses. It is also the only name that would have crossed the new boundary at all, so leaving it behind means the package imports no private name out of the module it came from. **What that costs, said plainly: `recovered_completion` moves and `plan_lost_to_the_budget` does not, and the two are halves of one story** - the reply hit the budget, recover the summary, record the lost plan. Ruled by Fowler, 2026-09-12.
- **Files touched:** `backend/idhazh/classify/__init__.py`, `backend/idhazh/classify/calls.py`, `backend/idhazh/visual_planner.py`, `backend/idhazh/elements.py`, `backend/idhazh/contracts/visual.py`, `backend/utilities/measure_two_calls.py`, `backend/tests/test_classify.py`, `backend/tests/test_visual_planner.py`, `backend/tests/test_elements.py`, `backend/tests/test_marks.py`, `backend/tests/conftest.py`, `tests/fixtures/planner/recorded-call-payloads.json`, `docs/architecture/summarize/prompt.md`, `docs/architecture/extraction/elements.md`. **The list grew by seven when the row ran and lost two** (2026-09-12). The seven: the oracle fixture the row's own Oracle asks for; `measure_two_calls.py`, the one caller outside the tests; `elements.py`, `contracts/visual.py`, `test_elements.py` and `docs/architecture/extraction/elements.md`, each of which names a moved function in prose or a link; and `conftest.py`, which takes `RecordedEndpoint`, `a_table`, `call_one_payload` and the two reply-fixture paths, because the split leaves each of them serving two test modules and that file already carries a section for exactly that. The two it lost are `backend/idhazh/cli.py` and `backend/idhazh/summarize.py` - see decision 2.
...
- **Oracle:** **The request payloads are byte-identical before and after.** Driven from `tests/fixtures/planner/recorded-call-payloads.json` - the request bodies the current code builds for one fixture article, captured in the commit before the move. **Neither that file nor `tests/fixtures/planner/` existed (checked 2026-09-12), so capturing it from the pre-move code was the row's first step**, and it was captured before a line moved or it records the new behaviour rather than the old. The test builds them again through `classify.calls` and asserts equality on the bytes. **A pure move proves itself by producing the same bytes; anything else is a behaviour change wearing a refactor's name**, and this is the one oracle in the plan that goes red on an improvement. **Result: 42,874 bytes, SHA-256 `c021aee7f705...`, zero differing bytes.** It caught one real fault on the way: `Path(__file__).parent` resolves one directory deeper inside a package, so all three prompt files were unreadable from the new home until the walk-up was corrected.
...
| 2 | **No re-export shim is left in `visual_planner.py`.** A module that forwards to its own replacement is the band-aid section 0.1 forbids, and it means the next reader finds two homes for one thing. Every caller is repointed in this commit. **The row named `backend/idhazh/{cli,summarize}.py` as the two callers and both were wrong** (checked 2026-09-12): `summarize.py` does not name `visual_planner` anywhere, and `cli.py` names only `numeric_facts`, `reachable_kinds`, `decided_without_the_model`, `build_request` and `to_decision`, every one of which stays. **The one caller outside the tests is `backend/utilities/measure_two_calls.py`**, which the row did not name. The full suite is what proves there is no third | Section 0.1 |
...
| 6 | **`test_classify.py` is named in `test_marks.UNMARKED_MODULES` rather than carrying `pytest.mark.visual`.** `pyproject.toml` defines `visual` as the visual planner, its validator, the spec compiler and the two renderers, and a call builder is none of the four - so marking it `visual` would make the mark's own definition false on the day of a pure move, and rows #8, #9, #10, #11 and #16 would then grow `-m visual` into a classification suite nobody chose. **What it costs, and it is real: 90 tests leave `-m visual`, and they include the call-2 tests, which is the call that builds the visual plan's request.** Somebody editing the plan half of that prompt and running only `-m visual` will not run them. CI runs the whole suite, so the cost is one missed local re-run and not a missed regression | Fowler, 2026-09-12 |
| 7 | **`classify/calls.py` keeps its own copy of the two reply-cleaning regexes.** `summarize.py` already holds a byte-identical second copy, so a per-module cleaner is the convention this tree already shows twice - and it is what keeps the new package from importing a private name out of the module it came from, which no module under `backend/idhazh/` does. **The cost is a third copy of two lines**: the day the `<think>` tag or the fence syntax changes, three files change and nothing fails if one is missed. The right home is `backend/idhazh/llm/server.py` beside `parse_completion`, and that is a follow-up row rather than this one - it edits two modules this row does not scope, and **this row's oracle cannot see it**, because reply parsing does not touch request bytes | Fowler, 2026-09-12 |
...
- **`docs/concepts/growing-reads.md` is in the list because the refusal count reads a window of `state/item-health/`**, and a read over a collection a run appends to is declared whether or not it is bounded (Guardrail #12).
...
| 6 | It is **not** placed under `frontend/public/` unless a browser fetches it. A file in the served tree is a file in the page weight budget | `CLAUDE.md` Guardrail #2 |
...
| # | Decision | Authority |
| --- | --- | --- |
| 1 | Windowed fetch only. **No mass fetch, no prerender**, lazy fetch for everything except the first panel, and it **reuses the existing shared window control** rather than adding a second one | Carmack; `CLAUDE.md` Guardrail #12 |
...
4. **The proposal channel never feeds a prompt.** Untrusted text that re-enters the model is `CLAUDE.md` Guardrail #11 read backwards. **The control is the structural assertion in the oracle, not this sentence.** A control stated only in prose is a control nobody can fail.
...
| # | Decision | Authority |
| --- | --- | --- |
| 1 | Starting values to `config/`: **N=12 items, M=5 domains, D=7 days, cap 2 a domain.** All four are **estimates** and are config precisely because they are | Owner, 2026-09-10; `CLAUDE.md` Guardrail #6 |
...
| 3 | It reads a **bounded window** of the counterfactual ledger - the trailing `lens_weights.window_days` from config - and declares that read in `growing-reads.md` | `CLAUDE.md` Guardrail #12 |
| 4 | **It adapts on the counterfactual, not the outcome.** What the loop compares is what the ranker **would have** selected at a candidate weight against what it **did** select at the committed one, over the same window. Adapting on the outcome converges on whatever the loop already preferred, and it converges quietly | Andre; `CLAUDE.md` Guardrail #10 |
...
| 6 | **A thin window produces a refusal, not a smaller adjustment.** Below `lens_weights.min_items_per_lens`, the run proposes nothing for that lens and records why. A multiplier computed from four items is not a smaller multiplier; it is a different kind of thing | `CLAUDE.md` Guardrail #10 |
...
| 2 | **The proposed margin is 10 points**, and it is a proposal until this row is run. It is written down before the measurement so it cannot be chosen afterwards to fit the result | `CLAUDE.md` Guardrail #10 |
| 3 | The result is recorded with the date, the split, the definition-text version and the spread. Not a single percentage | `CLAUDE.md` Guardrail #10 |
...
- **Settled 2026-09-13: plan 25 row #2 landed first, as pull request #653, so this row EXTENDS `docs/concepts/placement.md` and creates nothing.** `docs/concepts/the-order-of-the-day.md` was never written and must not be; a reference to it anywhere is stale.
...
**Every term named, with its config address, and the notation CITED rather than restated.** `docs/architecture/sources/discovery.md` already carries the stage-1 and stage-2 formula in a code block, and plan 25 row #3's acceptance gate checks that block term for term against the code - so a second copy on `placement.md` would be the copy with no gate on it (rejected alternative 1b). What the page owes is the part `discovery.md` does not carry: each term's config address, in a table a test resolves through `idhazh.config.load()`. Stage 1 is computed in `plan`, **before the article is read**:
...
**The research citations - four, not seven, and the page carries only these four.**
...
**The three machinery papers left this page on 2026-09-13, and each went to the row that owns its question.** They are about structured output, constrained decoding and the log-likelihood convention - which answer "is the number the model printed a number", not "why is this story above that one". Carrying them on `placement.md` would make one page answer two questions, which decision 1 below and `CLAUDE.md` section 5 both refuse. Ruled by Fowler, 2026-09-13, on row #20's own dispatch; nothing was deleted, and the three sentences that used to sit here are now in section 25 against the rows that write them.

| Paper | Now written by | On |
| --- | --- | --- |
| The Format Tax (arXiv 2604.03616) | Row #7b | `docs/architecture/summarize/prompt.md` |
| Grammar-Aligned Decoding (arXiv 2405.21047) | Row #P5 for the method, row #9 for the rule | the `logprob_mode` benchmark record, then `docs/concepts/classification.md` |
| Summed against mean-token log-likelihood (arXiv 2608.03854) | Row #P5 for the method, row #9 for the rule | the same two |
...
**The three machinery papers moved to the rows that own their question on 2026-09-13** (above). The finding each one carries is recorded in section 25 against the row that writes it, so none of the three is lost when this plan is deleted.
...
| 1a | Keep this on its own page, `docs/concepts/the-order-of-the-day.md`, beside `docs/concepts/placement.md` | Two pages answering one question, distinguished only by which one a reader happened to open. Plan 25 row #14 already puts four of this page's seven citations on `placement.md`. **Settled 2026-09-13: plan 25 row #2 landed first, so `placement.md` is the page and the second one was never written** |
| 1b | Restate the exact stage-1 formula on `placement.md` as well as on `docs/architecture/sources/discovery.md` | Two copies of one definition, and only one of them has a gate pointing at it - plan 25 row #3's gate checks `discovery.md` term for term, so a copy on `placement.md` would go quietly wrong the day row #3 or row #4 lands. The page cites the formula and walks a worked example instead, which is what "every term named and its config address" actually needed. Fowler, 2026-09-13 |
| 2 | Learn the whole selection score end to end | There is no click, no dwell and no reader signal of any kind - `CLAUDE.md` Guardrail #1 - so there is no target to learn against. The four ranking papers are what this repository has instead |
...
| 4 | **The worked example is a test, not an illustration.** A page with a formula and no executable check is stale the first time a default moves, and nothing says so | `CLAUDE.md` Guardrail #10 |
...
| `docs/concepts/classification.md` | **yes** - created by row #2 on 2026-09-12 | 2 creates; 8, 9, 10, 11, 13 extend | Every label, its definition text, what renders and what does not, and the confidence rule. **Row #2 creates it so five rows are not each writing its opening paragraph**; the five extenders land in groups I, K, L, M and F, so no group has two of them. **Row #9 writes the two rules the confidence figure rests on, each with its citation** - a confidence read off a constrained decode is not the model's confidence (Grammar-Aligned Decoding, arXiv 2405.21047), and summed against mean-token log-likelihood **reverses** which model looks better calibrated (arXiv 2608.03854), which is why row #9 decision 1 takes one token position rather than a sum. Both citations moved here from row #20 on 2026-09-13 |
...
| `docs/concepts/placement.md` | **yes** - created by plan 25 row #2 on 2026-09-13 (pull request #653), and extended by row #20 the same day | 20 extends; 17 extends | Why one story is above another: every term's config address in a table a test resolves, a worked example driven from a fixture, a mermaid diagram of the signals and where the feedback loop closes, one plain-English paragraph, **four** citations and the rejected alternatives. The exact formula is cited from `discovery.md`, never copied. **Row #17 extends it with the learned lens multiplier**, which is the loop the diagram closes. **Plan 25 row #2 wrote the frame, the caps and the floors on the same page, and its row #14 the target distribution - which extends row #20's synthesis paragraph and does not re-list the four papers** - one page, one question, two plans |
...
| `docs/architecture/summarize/prompt.md` | yes | 1a, 7a, 7b | Row #1a strips the fingerprint from it; row #7a records that the call builders moved to `backend/idhazh/classify/`; row #7b records the two-node DAG and where the 30 definition sentences sit, **and cites the Format Tax (arXiv 2604.03616) beside the section "The decoder holds the shape, the prompt does not": most of the quality loss from structured output enters AT THE PROMPT, before any decoder constraint, so a row that blames a grammar for a worse answer is usually blaming the wrong half.** That citation moved here from row #20 on 2026-09-13 |
...
| `docs/reference/benchmarks/<YYYY-MM-DD>-label-logprob-mode.md` | **no** - though the directory now exists, holding `2026-09-11-day-window-read.md` (verified 2026-09-12) | P5 | Which distribution the pinned runtime reports at a masked token, the two replies side by side, the build it was taken on. **A record in `docs/reference/benchmarks/`**, which `CLAUDE.md` section 5 prescribes. **It is not the first one**: the directory was created before this row, so this record joins it rather than opening it. Corrected 2026-09-11 and again 2026-09-12; this table said "the first record", then "neither does the directory". **It also cites the two papers that make the run have two arms at all** - Grammar-Aligned Decoding (arXiv 2405.21047) for constrained decoding distorting the distribution rather than merely restricting it, and arXiv 2608.03854 for the summed-against-mean convention reversing the answer. A benchmark record carries the method, never the rule the figures justify (`docs/reference/documentation-structure.md`), so the rule half goes to row #9. Both citations moved here from row #20 on 2026-09-13 |
