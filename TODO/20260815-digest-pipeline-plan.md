...
| 9 | `run.json` recorded `runner: "local"` for a run on `ubuntu-latest` | `stage_assemble` pinned the label. Guardrail #10 asks what hardware produced a number; the manifest could not answer. |
...
| 10 | Sanitize before the model sees it: strip zero-width and control characters, HTML comments, base64 blobs. A feed title is data, never instruction (Guardrail #11). | Carmack |
...
| 12 | No title-derived slug in any URL. Titles come from fetched text, and fetched text never becomes a URL (Guardrail #11). | Jony |
...
| 10 | `transformers.js` | Runtime inference. Guardrail #1 and section 0a forbid it outright. | Carmack |
...
| 9 | `site_bytes` and `site_files` are recorded into the run manifest **on every daily run from day one**, and an issue opens above `site_budget_mb`. Measure the ceiling long before the policy exists (Guardrail #10 applied to storage). | Carmack |
...
| 2 | Reusing the render-failure state for a prune | A second meaning bolted onto an existing field, which is the band-aid Guardrail #5 forbids. Costs one enum member to do properly. | Fowler |
...
| # | Decision | Authority |
| --- | --- | --- |
| 1 | Guardrail #1 is amended in the same commit as this row, per CLAUDE.md section 0. The amendment bans egress and third parties, not compute: on-device work over bytes we committed and serve ourselves is permitted, and never sits on the digest's render path. | Fowler |
...
| # | Option | Why rejected | Authority |
| --- | --- | --- | --- |
| 1 | Loading weights from a third-party CDN | A runtime fetch to another origin, which is the half of Guardrail #1 that did not change. | Fowler |
...
| # | Decision | Authority |
| --- | --- | --- |
| 1 | Guardrail #11 applies inside the tab. Our summary is derived from a stranger's page, so feeding it to a second model is the same boundary a second time - with no CI standing behind it. | Andre |
...
| 8 | Shards are assigned round-robin over plan position, so the day waits on the slowest. Measured 2026-08-23: 3, 10, 11 and 13 minutes for one run - a 4.3x spread. `shard_of()` cannot balance by cost because article length is unknown until extraction, and its docstring claims a spread it does not achieve. Since 2026-08-2x `stage_work` fetches every item before it summarizes any, so the length IS known before the model runs - but the split has already happened one job earlier. | ~15% of run wall-clock, the largest single lever left | agent, then **OWNER ratifies** if it moves extraction into `plan` | Measure first: read the four `work` job durations across three runs and record the spread. If it holds, the cheap fix is to shard on a cost estimate the `plan` job can already make (feed tier and historical median words per source) rather than moving extraction. Moving extraction across the job boundary is a trust-boundary change and Level 5. |
