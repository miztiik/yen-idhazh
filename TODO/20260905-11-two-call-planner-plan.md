...
| --- | --- | --- |
| call 1's prompt on a cap-length article | 14,306 | rendered and tokenised by the server, 2026-09-12 |
| plus call 1's own output budget | 20,797 | `call_one_output_tokens()` is 6,491 since row #3g |
| call 2's prompt, carrying call 1's reply | 21,495 | rendered and tokenised the same way, with the stand-in replaced by the real budget |
| plus the reply call 2's grammar may write | **26,189** | `call_two_output_tokens()` is 4,694 |
| `models.summarize.inference.n_ctx` | 16,384 | `config/idhazh.json` |
| **over by** | **9,805** | 60 percent more than the window holds |

**Three numbers in that table moved on 2026-09-13 and the direction is worse, not better** (row #3g, in execution). It used to size call 2 with a **400-token stand-in** for call 1's reply, which was not call 1's budget at the time either: at the old 900-token budget the real overflow was **4,214** rather than the 3,714 printed here for weeks. Row #3g then derived call 1's budget from its own grammar and it went 900 -> 6,491, and **call 1's reply is paid twice** - once as its own decode, once inside call 2's prompt - so the pair went from 4,214 over to 9,805 over. **At the authorised 32,768 the worst sequence is 26,189 tokens, 80 percent of the window, 6,579 spare.** Nobody may read row #3g as a reason not to take this row; it is the reason to take it at 32,768 rather than at 24,576.
