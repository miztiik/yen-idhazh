...
- **Oracle:** The dispatched run completes with **no** job between `work` and `assemble`, the repo cache falls from **85.0 percent of the 10 GiB ceiling to 62.3**, and the worst shard stays under 180 minutes. Three independent numbers, because a retirement that only removes a config key has not retired anything. **The first and third are read from the dispatched run and from `state/runtime-counters.csv` over the seven days after it**, not from the table in any plan. **The second is readable today and has been read.** **The job is named `visuals`** - it declares `needs: [plan, work]` and `timeout-minutes: 50`, and it sits between `work` and `assemble` in `digest.yml` today (checked 2026-09-12 at `c16801f0`).
- **The cache reading, measured 2026-09-12** from `gh api repos/miztiik/yen-idhazh/actions/caches`, 7 entries: **9,130,060,863 bytes held, which is 85.0 percent of the 10 GiB ceiling.** The small model's entry `llm-Qwen3-4B-Q4_K_M.gguf-bc640142c66e1fdd12af0bd68f40445458f3869b-b10598-v4` is **2,438,761,672 bytes, 22.7 percent of the ceiling**, so retiring it leaves 6,691,299,191 bytes, **62.3 percent**. **The row's "roughly 82 to roughly 57" were estimates and both were wrong** - by 3.0 points, which understated today's pressure, and by 5.3, which overstated the saving (Rule #10). One point worth carrying: **85 percent is not a reason to hurry.** Passing the ceiling evicts the least-recently-used entry, and that will never be the 9B, which every work shard of every run touches; what sits at the front of that queue is pip, npm and playwright, which refill in seconds. **The reason to retire the model is one model to qualify, pin, download and measure instead of two** - the cache is a consequence, not the argument. Carmack, 2026-09-12.
- **There are two identical `playwright-Linux-1.62.1` cache entries**, 281,926,151 and 281,922,538 bytes. One of them is 2.6 percent of the ceiling for nothing. Not this row's to fix and not worth a row on its own; noted here because this is where the cache was last read.
- **This row cannot close in one sitting, and that is scheduling rather than a defect.** The third number needs seven days of runs that have not happened yet, so a worker that waits for it never reports. **Land the row when the first two numbers are in, and record the seven-day reading as owed** - the same shape plan 23 row #P4 used on 2026-09-12 for its own confirming dispatch. What is owed is one sentence: the worst shard in `state/runtime-counters.csv` over the seven days after the merge, against the 180-minute bar, with the merge date named. **A row left open waiting for a clock blocks the critical path** - eleven rows wait on plan 23 row #7b, which waits on this one. Ruled 2026-09-12.
