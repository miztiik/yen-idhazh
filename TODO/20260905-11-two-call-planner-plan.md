...
| 5 | **The projection publishes the first call and the total; the second is the remainder.** Fowler ruled twelve published cells and labelled his own byte figure an estimate, asking for the measurement first. Taken 2026-09-12 on the two committed shards with every timed row populated, twelve cells cost 72.9 and 80.1 percent more gzipped against 35.3 and 40.8 for the seven that ship, so the estimate was refused by the measurement (Rule #10). Nothing is lost: the totals are the sum, the arithmetic is integers, and `model_calls` stops a third call being absorbed in silence | Carmack, 2026-09-12, on a measurement taken in execution |
