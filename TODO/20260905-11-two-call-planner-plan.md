...
| 3e | The instructions move in front of the article | 3c | C5 | IN-FLIGHT | p11-3e | - | worker |
...
**The chain was re-ordered on 2026-09-12 and it is now one line: 3b -> 3d -> 3c -> 3e -> 5b -> 6.** Row #3b's first per-call reading showed that the two-call design's whole extra cost over a single call is prompt layout - tokens that call 2 reads and nobody asked for. Wiring the calls into the pipeline first would mean reading that decision through an instrument that cannot see three quarters of what it costs. So the layout is fixed before the wiring: row #3d makes the instrument report where every re-read token went, row #3c removes the template break, row #3e removes call 2's question, and only then does row #5b wire it up. **Measured on the configured weights 2026-09-12, the overhead is 797 tokens an item - 100 for the template break and 697 for the trailing turn - and those two rows remove 787 of it.** That is **80.9 seconds an item today, about 27 minutes of a 20-item shard, against about 1 second afterwards**, at the measured 9.85 tokens a second. **The 896, the 209 and the 670 this paragraph carried until then were the retired 8B's numbers and an estimate**, and they are withdrawn ([`../docs/reference/benchmarks/2026-09-12-two-call-re-read.md`](../docs/reference/benchmarks/2026-09-12-two-call-re-read.md)). Owner decision, 2026-09-12, on Andre's ruling; the full argument is in each row.
...
**It frees no context, and that is not a defect of this row.** Moving 687 tokens from behind the article to in front of it changes **where** they sit, never **how many** there are, so the sequence is the same length and row #3f's finding stands untouched. What it does widen is the prompt ceiling the harness computes, by 597 tokens - 5.9 percent, which closes 14 percent of row #3f's gap. Real, and not a fix.
...
- **What it saves, priced at the measured rate:** **687 tokens an item is 69.7 seconds an item, about 23.2 minutes of a 20-item shard**, at 9.85 tokens a second - **three times what the template gap costs.** The 687 is measured: `call_two_user_turn` renders to 2,555 characters and 687 tokens on the configured weights, 2026-09-12 (row #3d's reading). **The "about 670" this row carried until then was an estimate and is withdrawn**, along with the 68 seconds computed from it.
...
- **Acceptance gates:** `GATE-PY` over `test_classify.py` and `test_summarize.py`; `GATE-SCHEMA`, which must produce an empty diff because this row edits no contract.
- **Oracle:** **Three numbers and a conservation law, and the conservation law is what makes this row safe.** First, **conservation**: the tokens removed from the trailing turn equal the tokens added to the system turn, and the chat-template header delta is printed rather than absorbed - a row that "saves" 600 tokens by dropping an instruction instead of moving it passes any threshold and fails this at once. Second, **the floor is measured before anything moves**: render the trailing turn with the movable bulk already gone - the chat-template headers plus the band-number substitution decision 1 keeps - and tokenise it through the same server. Call it F; no effort by this row goes below it. Third, **the bar is stated against F**: row #3d's harness already prints the trailing-turn cause and the question's own tokens per item, so the row passes when the residual `after - F` is the pointer sentence and nothing else, and the pointer's own token count is reported rather than aimed at. A reader then reads `687 -> 94, floor 71, pointer 23` and knows both that the row worked and that it got everything there was. **"Under 100 tokens an item" replaced a measurement with a round number** and could not say either; it is withdrawn. Fourth and unchanged, over a fixed set of real articles run through both layouts, **the per-kind element counts and the anchoring survival rate hold** - deterministic counts over a model's output, not a model grading a model. Andre, 2026-09-12.
...
| 3 | Row #3e widens the window it can reach, and that is worth recording so nobody later reads it as the fix: shrinking the trailing turn from 687 tokens to about 90 moves the harness's own ceiling from 10,103 to about 10,700 - **597 tokens wider, 5.9 percent, which closes 14 percent of the gap** | Andre, 2026-09-12 |
