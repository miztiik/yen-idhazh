...
| 3h | A two-call item that failed on call 1 reports no cost at all | 3g | C8 | DONE #PR | p11-r6 | #PR | worker |
...
| 6 | The small model, its job and its cache go | 5b | F | DONE #PR | p11-r6 | #PR | worker |
...
- **Acceptance gates:** `GATE-PY` over `test_classify.py`, `test_visual_planner.py` and `test_contracts.py`, which holds the working-tree ASCII and LF check the re-recorded fixtures have to pass; `GATE-SCHEMA`, which must produce an empty diff because this row edits no contract.
