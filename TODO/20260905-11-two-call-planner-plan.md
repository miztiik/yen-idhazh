...
| 5b | Call 1 and call 2 run in the pipeline | 5, 3b, 3e | E2 | DONE | p11-5b | - | worker |
...
| plus call 1's own output budget | 15,206 | `models.summarize.inference.max_output_tokens` is 900 |
| call 2's prompt, with a 400-token stand-in reply | 15,404 | rendered and tokenised the same way |
...
- **A cut call-1 reply loses the item, loudly, and that is row #3g made visible rather than fixed here.** Call 2's prompt replays call 1's reply verbatim, and the reason that is safe is that the reply has already been held to a closed schema - so a reply `parse_call_one` cannot read is not sent on. The item gets a failed `Summary` with `bad_shape` and a detail naming the labelling call, which is a cell in the census rather than a silence. Row #3g found this on one article of three, so it is not rare; when that row lands it turns the loss into a recovery and this paragraph goes with it.
