...
**Memory is what a window costs, and it is why 65,536 was affordable.** Three arms, measured the same day: KV runs 32 KiB a token over 8 attention layers of 32 - the other 24 are recurrent and cost a fixed 50.25 MiB whatever the window is - so 512.00 MiB at 16,384, 1,024.00 at 32,768 and 2,048.00 at 65,536, which is **1,584 MiB more all told than today** against the runner's measured low-water free of 6.84 GiB and a 1.0 GiB bar. `n_ctx_train` is 262,144, so nothing is scaled. **49,152 would also have cleared the sizing**; 65,536 was picked because the sizing is a sizing rather than a ceiling - the worst build tokenized at 1.952 tokens a word against a recorded `WORST_TOKENS_A_WORD` of 1.585 - and 512 MiB is cheap insurance against the next article that tokenizes harder.
...
| 2 | **A cap-length prompt may not be affordable at all.** The one 8,741-token prompt the pipeline has actually sent cost 927 s of prefill on the runner. A 28,041-token call 1 prompt is 3.2 times that | Guardrail #2 against row #5b's wiring and the cap, not against the window. The window costs memory; the tokens cost time, and the same tokens cost the same time at any window |
| 3 | **The menu costs 34 tokens a row and there are up to 256 of them.** 8,733 tokens - 22 percent of the sequence - go to a candidate table | `elements.max_per_article` is a labelling-quality decision (Andre, Editor). Lowering it to 64 would save about 6,500 tokens and halve the candidates a label may cite |
...
| 4 | **The window took 65,536 and not the authorised 32,768.** Shipping 32,768 would have shipped an assertion that fails, which is the one thing the dispatch forbade in either direction: a quiet pass. The three moves `CLAUDE.md` section 0d allows are do it, price it, or name the measurement - the measurement was taken, it refused 32,768, and the direction the owner authorised was "increase n_ctx, memory is not a problem". **The owner may still rule 49,152, or rule that `elements.max_per_article` comes down instead**; both are priced above and either is a one-line change on top of this | row #3f's worker, 2026-09-13, under `CLAUDE.md` section 0d, against the owner's 2026-09-12 authorisation |
...
| 2 | Raise `n_ctx` from 16,384 to 24,576 inside this row | **Overturned by the owner on 2026-09-12, at 32,768 rather than 24,576, and overtaken on 2026-09-13 at 65,536.** The two objections stood on their own terms: memory on a 16 GB runner that nobody has measured, and a window that is fingerprint-digested, so every summary written before it is stamped apart from every summary after. **Neither was an argument against the raise; both were work the raise had to carry**, and the owner bought it. Both are discharged: the memory reading is above, and plan 23 row #1a merged as #647. Guardrail #2 is still surfaced and never adapted by an agent - it was surfaced, and a person answered | Andre, 2026-09-12; overturned by the owner, 2026-09-12, `CLAUDE.md` sections 0 and 0d |
