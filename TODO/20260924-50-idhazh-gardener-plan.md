...
Execute per docs/how-to/execute-a-plan.md: one owner carries the plan and delegates a row where delegation pays; **keep a running pool of parallel N = 2, refilling a slot the moment a worker returns its report and never waiting on a merge** - rows 3 and 4 are the only disjoint pair, and rows 5 to 9 all append to `config/idhazh_gardener.json` and `backend/idhazh/gardener/tasks/__init__.py`, so they are serial whatever their letters say; consult a persona only where two answers would lead to different code; AUTO-merge on green gates where no ESCALATE trigger fired; honor the ESCALATE triggers in section 0. AUTHOR-AND-STOP until the user authorizes.

## 0. Operating contract

| Field | Value |
| --- | --- |
| **Blocked by** | **[`20260926-53-one-door-into-state-plan.md`](20260926-53-one-door-into-state-plan.md) rows 2, 3, 5 and 6 - which is the whole of that plan's critical path.** Row 2 of this plan needs a package to live in (row 2 there), a `LedgerName` to type its first argument (row 3), `ledger/paths.py` to add its builders to (row 5, which creates it with the `LedgerPath` table) and `ledger/filenames.py` to add `unit_id`, `file_id` and `_pack_v8` to (row 6, which creates it when the module splits). **There is no `naming.py` in this plan and every mention of one is that file.** Plan 53 row 6 is its last row, so **rows 2 to 9 here start after all six of it land**. Row 1 here has no blocker and runs beside plan 53 from the first day; plan 53 row 4, the `path_classes.py` rename, blocks nothing here |
...
| An `enabled` flag per task | Nothing. **`state: paused` is the off-switch**, and it does more than an `enabled: false` would: a paused task keeps its registry entry and its `owns` claim, so its paths stay reserved and no sibling can quietly take them | Nothing. It would be a second spelling of `state: paused`, and two spellings mean two places to look when a task did not run. **`dry_run` is not the off-switch** - a dry run still runs, still costs a task slot and still writes a record (section 5.2) |
...
| --- | --- | --- |
| Row 1 ships without a blocker | Row 1 waits on nothing; row 2 waits on plan 53 rows 2 and 3 | Merged, row 1 waits for a plan it does not need |
...
flowchart TB
  ...
  subgraph OPS["Idhazh Gardener - idhazh-gardener.yml"]
    ...
    TEND["run-tasks<br/>5 shards, 4-5 due tasks each<br/>fail-fast false, max-parallel 5"]
...
| `tasks.<name>.window` | `{unit: days\|months, value: int ge=1}` or `{unit: forever}`, discriminated on `unit` | What the task keeps. Two bounded units so a ledger counted in months keeps a month window, and **`forever` for a tree this project has chosen not to age-bound**. `forever` carries no `value`, and `extra="forbid"` refuses one for free. **Three committed windows are null today** - `observability.item_health_aggregate_keep_months`, `observability.score_archive_keep_months` and `observability.visual_aggregate_keep_months`, read from `config/idhazh.json` on 2026-09-26 - and null there means never delete. **They transcribe to `{unit: forever}` and to nothing else.** A third union member rather than a nullable `value`, because `null` is also what an absent key looks like, and this contract makes `state` and `dry_run` required with no default precisely so that an omission cannot be read as an intent |
...
- **a `monthly_window` that, with `daily_keep_days`, reaches less far back than that ledger's own retention window.** Once a ledger goes through the door the two periods *are* its retention, and a shorter pair silently cuts it. **A ledger's own retention window is the `window` of the retention task that owns its tree, or `{unit: forever}` when no retention task owns it** - which is the case the drafted refusal could not answer and is why `feed-retirements` could not load. Four cases, and the refusal names which one it hit: bounded against bounded compares and refuses when shorter; bounded against `{unit: forever}` passes; **`{unit: forever}` against bounded refuses**, naming the ledger; `forever` against `forever` passes.
...
### 5.2.1 The eleven retention blocks, transcribed
...
**`state` is `active` and `kind` is `retention` on all eleven.** **`dry_run` is `true` on all eleven, and that is a finding rather than a choice** - see below.
...
| `day-validations` | `months: 14` | `retention.day_validation_keep_months` | `state/day-validations` | `null` |
...
| `telemetry-aggregate` | three series, below | three `observability` keys | `state/item-health`, `state/telemetry-aggregate`, `frontend/public/telemetry` | `null` |

`telemetry-aggregate.series`, and the third is the reason `forever` exists:
...
**Every one of the eleven is report-only today, and the plan believed otherwise until 2026-09-26.** `.github/workflows/digest.yml` passes `--dry-run` to the whole `prune-state` invocation and `config/idhazh.json` carries `retention.dry_run: true`; one flag fans out to all eleven passes in `prune_state.py`. **So nothing under `state/` is being deleted by this program at all**, and eleven blocks at `dry_run: true` is what "each task ships whatever that pass does today" actually means. **The gardener therefore deletes nothing on the day it ships**, and switching any task on is a separate decision with its own record to read first (section 5.2's onboarding stage).
...
**Two of the eleven read a key this file does not hold**, so each needs the same cross-file refusal `seen` already has: `gardener.tasks.seen.window.value` shorter than `collect.seen_window_days`, and `gardener.tasks.counterfactual-scores.window.value` shorter than `lens_weights.window_days`. Both are stated in section 5.2's refusal list.

**`retention.image_months` is transcribed twice**, by `digest-fragments` and by `visual-prune`. That is one key bounding two trees today; the blocks carry the same source name and diverge the moment somebody gives either its own number.
...
| --- | --- | --- |
| **Windowed** | The eleven retention tasks, and both GitHub collection tasks | Nothing. They run **at every wake**, as every task in the matrix does, and the window decides what qualifies. A wake with nothing old enough is a listing that finds nothing and a record that says `deleted: 0`. No last-run state exists because none is needed |
| **Compaction** | `compact-<ledger>-daily` and `compact-<ledger>-monthly`, one pair per ledger | Nothing. The task runs at every wake and **its own watermark decides the work**, read inside `run-tasks` where that ledger is already in the cone. A wake with no eligible period writes a record saying so, which is the answer the windowed tasks give too. **The daily task also writes that ledger's raw day indexes before it reads them** - it was a separate task until 2026-09-26, and merging it removed an intersection its `owns` prefixes could not express: an index task owning `state/raw/<ledger>/index` and a daily compaction owning `state/raw/<ledger>` is one prefix inside the other, which section 5.2 refuses by name, so the pair could never have loaded |
...
| `CompactionPolicy` only | `period: Period` (required), `max_periods_per_run`, and by `period`: **`daily_keep_days` and `raw_index_keep_days` on `daily`**, `monthly_window` on `monthly`. A key on the wrong period is refused by name at load |
...
| `monthly_window` | `{unit: months, value: int ge=1}` or `{unit: forever}`, discriminated on `unit`. Default `{unit: months, value: 13}` | monthly compaction | How long a month file survives. With `daily_keep_days` this is the ledger's real retention floor once it goes through the door: about 14 months at the defaults. **The same union as a task's `window`, and for the same reason** - two knobs answering "how long do I keep this" answer it in one spelling. **The name carries no unit because the value may be `forever`**, and a field named `_months` holding `forever` says the wrong thing (CLAUDE.md section 0b). A ledger with no retention task owning its tree is permanent, so its monthly window is `{unit: forever}` and a bounded one is refused |
...
**Row 9 takes the registry to 32 and the matrix to 31, all 31 due**, because each of the three ledgers it migrates gains an index task and two compactions. **That is 6.2 tasks a shard and the fullest shard holds 7** - thirty-one over five shards is four shards of six and one of seven. **Row 8's timeout derivation is stated against the fullest shard, not the average, because a timeout binds a job.**

**Two changes on 2026-09-26 cancel almost exactly.** Row 7 gained a third ledger, `feed-retirements`, which adds an index task and two compactions; row 10 collapsed, which removes the same three for `span-rollup`. The post-row-9 total is unchanged at 31; only the pre-row-9 total moved, from 19 to 22.
...
| `timeout-minutes` | `plan: 5`, `run-tasks: 20`, `history: 30` | 30 is what `prune.yml` uses for the same history work, and it now also covers the dueness read that exits early. 5 for a job that reads one committed file. **20 is derived and here is the derivation**: sparse checkout 30 s, `setup-python` 20 s, a cold `pip install -e .[parquet]` on `ubuntu-latest` at **the figure row 2's named measurement returns** - carried by reference, not by a number here, per decision 8 - **seven tasks at 45 s each**, which is section 5.9.10's fullest shard after row 9 and the one term that is an estimate, and a push loop bounded near 30 s by `attempts: 5` and the declared backoff. **At every candidate install figure the sum lands between about 6 and 13 minutes against a 20-minute timeout, so `timeout-minutes: 20` does not move.** Row 8's named observation replaces the task term with the first scheduled run's readings |
...
**`state/raw/` and `state/compact/` are bounded by the compaction, not by a retention task.** Row 7 gives every ledger this plan creates an index task and two compactions: the daily compaction deletes a day's raw files, `raw_index_keep_days` bounds the raw index tree, and `daily_keep_days` with `monthly_keep_months` bounds the two compact periods. **A second task windowing a tree a compaction already owns is forbidden** - row 7 decision 7 - because it would have to own a root that task already owns. **This plan creates three ledgers, so row 7 ships nine blocks and not six**: `gardener`, `visual-prune` and `feed-retirements`. the two this plan creates. What this row registers them against is narrower: the `trials` task sweeps any directory under `state/` that no task claims, and without the registration it would read the two roots as strays and delete the gardener's own records. "Registered" means "not a stray", never "not pruned".
- **Files touched:**
...
- **Acceptance gates:** local `ruff check .`, `mypy backend`, `pytest backend/tests/ledger backend/tests/retention backend/tests/contracts -q`. CI runs the full suite.
  - **Named measurement before merge:** re-take pyarrow's installed size and install time on `ubuntu-latest` (throwaway workflow off `main`, n=3, installed bytes before and after, cold and warm cache), and correct section 4. The Windows figure is 96.9 MiB; the estimate is 120-135 MiB. **This plan's row titled The three ledgers the console's routes read become parquet reads this figure before it merges**, because it puts pyarrow into the `digest.yml` jobs that write those ledgers.
...
- **Files touched:**
  ...
  - `config/gardener/` (**six declarations: a daily and a monthly compaction for each of three ledgers** - `gardener`, `visual-prune` and `feed-retirements` - carrying `period`, `max_periods_per_run`, and by period `raw_index_keep_days` with `daily_keep_days`, or `monthly_window`; each monthly declaration carries `after: compact-<ledger>-daily` per section 5.9.5). **No registry file is edited**: `compaction.py` builds its `TASKS` from the loaded policies of its kind, so a fourth ledger is two declarations and no Python
...
- **Decisions:**
  ...
  | 13 | **`compact-<ledger>-daily` and `compact-<ledger>-monthly` run in the same shard, daily first**, expressed as `after: compact-<ledger>-daily` on the monthly declaration rather than as a rule in code. They are two tasks writing one file - `state/compact/<ledger>/index/daily.json`, which the daily rewrites at every wake and the monthly rewrites on the roughly twelve days a year it absorbs a month - and the matrix has no ordering. Split across shards, the second to push resets to the new tip and re-adds the copy it built before the reset, **dropping every entry its sibling had just written, at exit 0 and invisible in both logs**. The dependency the index task used to carry was removed by merging it into the daily compaction; this one cannot be merged away, because two grains is what a reader needs | Carmack, 2026-09-25, re-aimed 2026-09-26 |
...
- **Decisions:**
  ...
  | 2 | **The matrix is five shards of 3-4 tasks, not one job per task.** One job per task means one record row per file, so the per-file charge never amortises: **measured 2026-09-26 with pyarrow 25.0.1, nineteen one-row record files weigh 132.5 KiB where five four-row files carrying the same rows weigh 36.5 KiB** - 3.6 times, and an empty 19-column table with its envelope is already 4,690 bytes before a single row. Each task keeps its own verb, its own config block and its own `dry_run`; a shard is a container, not a task | Carmack, on section 4, re-measured 2026-09-26. `digest.yml`'s own header already makes this argument: a worker takes a shard of several items rather than one item per machine |
...
- **The escalation, settled 2026-09-26. It is no longer a blocker, and it needs no new module.** Three build-time readers under `frontend/src/lib/server/` open these ledgers as CSV: `host-fingerprint.ts` reads `host-fingerprint`, `machine-counters.ts` reads both `host-fingerprint` and `item-health`, and `model-work.ts` reads `scores`. **Measured 2026-09-26 by the `state/` paths each module actually opens** - an earlier count said four and then seven, and both were grep artefacts: `payload.ts` reads `day-metrics` and `feed-health` and touches none of these, `run-timeline.ts` opens no `state/` path at all, and `similarity-ledger.ts` matched only because it names `host-fingerprint.ts` in a comment.
  ...
  **The cost is an ordering constraint and it is the only thing this settles.** `ledger.ts` is built by plan 51's row titled **The query door module and its two entry points**, which depends on nothing in either plan, so **this row cannot merge before that module exists** - and that ordering is now in this row's `Depends-on` cell rather than in prose, because the dispatcher reads the cell. That row creates the module and installs the engine; plan 51's row titled **One panel end to end** extends it. **An earlier draft pointed at One panel end to end, which produced a cycle**: that row waits on plan 51's publishing row, which waited on this one.ow cannot merge before that module exists**. That is recorded in plan 51's dependent-plans section as well, because a constraint written on one side only is a constraint somebody discovers.
...
- **Does `span-rollup` survive at all?** Settled: it does not. Row 10 is collapsed and carries the measurements and Susan's two rulings; the deletion is plan 52's first row.ow, not a migration one.
