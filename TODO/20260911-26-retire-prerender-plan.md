...
| Hard scope - out | **Removing `export const prerender = true` from any route.** Section 0.2 rules against it and section 6 prices the reversal; a row may not do it and no gate here permits it. **Adding a prerendered route.** **Any change to what a document contains** - the seam, the seed, `ui.shell_seed_items` and every payload are untouched, so every page ceiling in `page_weight.ceilings_bytes` must still hold to the byte after row #2. **Any page under `docs/reference/` that names the guard only in passing** - a reference page mentioning it is a documentation question with its own owner, and settling it inside a code plan hides that decision where nobody looks for it. **`docs/concepts/ui-shell.md`**, whose prerender sentence is true today and is rewritten by plan 25 row #11 |
...
**A claim is corrected; a mention is left alone.** 23 pages under `docs/` name prerendering and this plan edits three of them. The test is what the sentence would make a reader do: a page that says "every route is prerendered" sends somebody to design against a constraint that does not exist, and a page that says "the page-weight gate measures prerendered HTML at gzip -5" is a true statement of method. A page that names the guard only in passing makes nobody do anything, so it is not this plan's to open.
...
**Four rows, two groups, no singletons.** The plan is small because three of the four candidate rows a brief would expect are refused rather than deferred: removing a `prerender = true` is section 6, a sweep of all 23 documentation pages is refused by the claim-against-mention rule in section 0.1, and editing a reference page that names the guard in passing is refused by section 0's scope. **Nothing here is invented to fill a document.**
...
- `docs/reference/site-weight.md` - one line and a link to the record, per `CLAUDE.md` section 5: the log carries the figure now in force and links to the record rather than absorbing it.
...
| 3 | **A reference page that names the guard in passing is not this plan's to edit.** The reason is scope, not the revision it was written against: no row here opens those pages, and a documentation decision settled inside a code plan is a decision nobody can find. A later docs pass owns them | `CLAUDE.md` section 5 |
...
It removes no `export const prerender = true`, changes no route, moves no byte in any document, and opens no reference page that names the guard only in passing. It does not touch `docs/how-to/run-the-gates.md` - the false sentence there is about routes, not about the guard, and belongs to row #3.
...
| 1 | [`../docs/reference/site-weight.md`](../docs/reference/site-weight.md) | One line and a link to the record |
