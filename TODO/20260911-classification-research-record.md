...
| Prefill rate | median **9.84 tok/s** over 277 rows | `prompt_tokens_total / prompt_seconds_total` per row. Corroborates the 9.85 median over 4,117 timed items already on [`../docs/reference/pipeline-cost.md`](../docs/reference/pipeline-cost.md) | Measurement |
| A summarize call | median **114.6 s**, p95 **312.7 s**, longest **800.9 s**, over 4,117 rows | The figure [`../docs/reference/pipeline-cost.md`](../docs/reference/pipeline-cost.md) carries today, cited rather than restated. A whole-ledger recount over all 8,975 timed item-health rows gives a median of 117.7 s, which is a different denominator and not a correction | Measurement |
...
| The worst article, two calls | **15,889 tokens against a 16,384 window - 97 percent, a margin of 1.03x** | On record in [`../docs/reference/pipeline-cost.md`](../docs/reference/pipeline-cost.md), from plan 11 row 5. Re-read 2026-09-11 and unchanged | Measurement |
...
| Qwen3-4B decode on the runner | **13.00 +/- 0.03 tok/s** | `llama-bench`, `ubuntu-latest`, 2026-08-22, on record in [`../docs/reference/pipeline-cost.md`](../docs/reference/pipeline-cost.md). It is a bench figure and not the prompt-cache cost in the live digest path, which that page says of itself | Measurement, with its own stated limit |
...
- [`../docs/reference/pipeline-cost.md`](../docs/reference/pipeline-cost.md) - the instrument log section 3 cites rather than restates.
