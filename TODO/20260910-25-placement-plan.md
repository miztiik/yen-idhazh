...
| --- | --- | --- |
| Committed archive | **8,550 items over 21 days**, 2026-08-21 to 2026-09-10. **Re-measured 2026-09-12 by row #5: 8,922 items over 22 days**, 2026-08-21 to 2026-09-11 | `frontend/public/digest/**/digest.json` |
...
| `time_source` absent | **3,733 items** - every day published before the field landed, and the count is frozen. It was 79.2 percent of the archive when `layout.md` recorded it on 2026-09-02 and is **41.8 percent** of 2026-09-12's 8,922. **Re-measured 2026-09-12 by row #5: the count is right and the share was wrong.** This row read 43.7 percent against 8,550 on 2026-09-11, and both figures moved because the archive grew by a day - which is the point the row makes about this number | same |
| `time_source: unknown` | **0. It has never happened**, re-confirmed 2026-09-12 over 8,922 items, so only the canary day can drive it | same |
...
| The score's terms | `repetition_weight` 1.0, `recency_weight` 0.6, `recency_half_life_hours` 18.0, `watchlist_bonus` 0.5, `front_page_bonus` 0.4; the heaviest lens weight in `config/taxonomy.json` is 0.3. **Corrected 2026-09-13 by row #3 on execution: `front_page_bonus` no longer exists.** It was removed with the term, having fired on 8 of 5,682 published stories | `config/idhazh.json`, `config/taxonomy.json` |
...
| Rail markers today | `rail_group_minutes` **60**; recorded 2026-09-02 over 12 days and 4,713 stories: **907 markers**, so 80.8 percent of stories carry no label. **Re-derived 2026-09-12 by row #5 over 22 days and 8,922 stories: 1,218 markers, 86.3 percent carrying no label.** Both readings are right on their own day; the share moved because the newer days group more tightly | `frontend/src/lib/day-shape.ts` docstring; `config/appearance.json` |
...
| 7 | A desk floor and a desk ceiling | 2 | D | DONE #673 | p25-7 | #673 | worker |
| 8 | A story cross-files to a second desk | 7 | E | DONE #682 | p25-r8 | #682 | worker |
| 8a | The second desk reaches the reader | 8 | J | PENDING | - | - | - |
...
| D | 4, 7 | `backend/idhazh/rank.py`, `config/idhazh.json`, `backend/tests/test_rank.py`, `docs/architecture/sources/discovery.md`, `docs/architecture/publishing/layout.md` | `backend/idhazh/placement.py`, `backend/idhazh/contracts/taxonomy.py`, `schemas/taxonomy.schema.json`, `config/taxonomy.json`, ``backend/tests/placement/`, `backend/tests/contracts/``, `docs/concepts/placement.md` | Both write a file in `config/`. Row #4 writes `idhazh.json` and row #7 writes `taxonomy.json`. Row #4 adds no contract field, so it regenerates no schema |
| E | 8, 9a | `backend/idhazh/placement.py`, `backend/idhazh/contracts/{article,digest_day,digest_view}.py`, `schemas/{article,digest-day,digest-view}.schema.json`, `frontend/src/lib/payload/types.ts`, ``backend/tests/placement/`, `backend/tests/contracts/``, `docs/concepts/placement.md` | `backend/idhazh/{rank,cli,ledger}.py`, `backend/idhazh/contracts/counterfactual_score.py`, `schemas/counterfactual-score.schema.json`, `backend/tests/test_ledger.py`, `docs/concepts/growing-reads.md` | Both edit a contract and regenerate a schema. Three named schema files against one, all four distinct. Row #8 opens no ledger and row #9a opens no published payload model. **Row #9a's contract and schema are plan 23 row #21's, extended rather than minted** (section 10), and it dropped `backend/tests/test_marks.py` with the module it no longer creates |
| F | 10 | singleton at 16 files - row #9b retired on 2026-09-11 and left it alone | - | - |
| G | 11, 14 | `frontend/src/lib/components/ConsoleNav.svelte`, `frontend/src/lib/console/band.ts`, `backend/idhazh/publish_console_band.py`, `backend/idhazh/contracts/console_band.py`, `schemas/console-band.schema.json`, `tests/fixtures/contracts/console-band/newest-day.json`, `backend/tests/test_console_payloads_producer.py`, `frontend/tests/{console-nav,console-title,console-band}.spec.ts`, `docs/architecture/publishing/console.md` | `backend/idhazh/diversity.py`, `backend/idhazh/assemble.py`, `backend/idhazh/contracts/{app_config,day_metrics}.py`, `schemas/{app-config,day-metrics}.schema.json`, `config/idhazh.json`, ``backend/tests/test_diversity.py`, `backend/tests/contracts/`, `backend/tests/test_marks.py``, `docs/concepts/placement.md` | Both regenerate a schema. **Two different contract modules, two different schema files, two different fixtures**, so the drift gate sees two disjoint diffs. The one file that could bring them together is `backend/tests/contracts/`: row #14 lists it and row #11 does not, so row #11 re-checks this group before it opens a pull request if it finds it needs one |
...
- **Files touched:** `backend/idhazh/assemble.py`, `backend/idhazh/cli.py`, `backend/tests/pipeline/`, `docs/architecture/publishing/visuals.md`, `docs/architecture/sources/discovery.md`
- **Acceptance gates:** `GATE-PY` with `backend/tests/pipeline/`, `GATE-SUITE`. Plus, in this row:
...
- **Files touched:** `backend/idhazh/placement.py`, `backend/idhazh/assemble.py`, `backend/idhazh/contracts/app_config.py`, `schemas/app-config.schema.json`, `config/idhazh.json`, `backend/tests/placement/`, `backend/tests/test_marks.py`, `docs/concepts/placement.md`, `docs/architecture/publishing/layout.md`
- **Acceptance gates:** `GATE-PY` with ``backend/tests/placement/`, `backend/tests/test_marks.py``, `GATE-SCHEMA`, `GATE-SUITE`, `GATE-DAYS`. Plus, in this row:
...
**Five things in this row were corrected on execution, 2026-09-13, per section 0.1.** **Decision 4 says every relative size is stamped "in `config/idhazh.json`'s own comment block", and there is no such block** - the file is JSON and JSON has no comments. A config key's comment in this project is the `Field(description=...)` on its model, which the drift gate carries into `schemas/app-config.schema.json`, so that is where the stamps went. **The oracle as written could not fail against the base tree**: all four named terms already moved the order there and `source_form`, `title` and `item_id` already moved nothing, so it was run on the base tree, passed, and was narrowed on Editor's ruling to the half that can fail - *no unnamed term with no owner moves the order*. **No weight moved**; what moved is that a fifth term, unnamed by this row and owned by nobody, was removed. **Decision 4 says "every one" and two of the four were not stamped**: `collect.tier_weights` carried no description at all and `collect.reliability_floor` carried a reason with no measurement, so both were measured and stamped here and `discovery.md` stopped claiming only two of the four were. And **the number the removal was first argued from was measured under the wrong reading of "a head slot moved"**: head membership is unchanged on all 13 days, but the slot order changes on 2 of them and the **lead story** changes on both, so "nothing a reader has already seen would have been different" was false and is corrected everywhere it was written. Editor re-ruled on the corrected number and the term still goes. The measurements are in the pull request and on [`../docs/concepts/placement.md`](../docs/concepts/placement.md).
...
- **Files touched:** `backend/idhazh/rank.py`, `backend/idhazh/contracts/app_config.py`, `schemas/app-config.schema.json`, `config/idhazh.json`, `backend/tests/test_rank.py`, `docs/architecture/sources/discovery.md`. **Widened on execution 2026-09-13 by the no-prisoners rule, and every addition is one file the removed term reached**: `backend/idhazh/contracts/sources.py` and `schemas/sources.schema.json` (a salience feed's description said what a vote was worth), `backend/tests/test_discover.py` (the helper and the formula pin), `backend/tests/test_order_of_the_day.py` and `tests/fixtures/rank/worked-example.json` (the example fired the term), `tests/fixtures/contracts/app-config/every-knob-differs-from-the-committed-config.json` (a knob that no longer exists), `docs/concepts/placement.md` (prose, term table, diagram and worked example) and `docs/concepts/config.md`. **Re-checked against row #6's list and group C still holds**: row #6 opens `appearance_config.py`, `appearance-config.schema.json`, `config/appearance.json`, two frontend files, two frontend specs and `frontend.md`, and this row opens none of them.
...
- **Files touched:** `frontend/src/lib/components/DigestItem.svelte`, `frontend/src/lib/components/DigestList.svelte`, `frontend/src/lib/components/TimeRail.svelte` (deleted), `frontend/src/lib/day-shape.ts`, `frontend/src/lib/format.ts`, `frontend/src/lib/server/config.ts`, `frontend/src/styles/tokens.css`, `frontend/src/styles/frame.generated.css`, `frontend/scripts/build-frame-css.mjs`, `frontend/scripts/test-groups.ts`, `backend/idhazh/contracts/appearance_config.py`, `backend/idhazh/contracts/app_config.py`, `backend/tests/test_appearance_config.py`, `schemas/appearance-config.schema.json`, `schemas/app-config.schema.json`, `config/appearance.json`, `tests/fixtures/contracts/appearance-config/{committed,defaults}.json`, `tests/fixtures/contracts/app-config/every-knob-differs-from-the-committed-config.json`, `frontend/tests/time-rail.spec.ts` (renamed to `item-time.spec.ts`), `frontend/tests/item-card.spec.ts`, `frontend/tests/item-meta.spec.ts`, `frontend/tests/item-zones.spec.ts`, `frontend/tests/reading-page.spec.ts`, `docs/concepts/ui-shell.md`, `docs/concepts/design-system.md`, `docs/architecture/publishing/layout.md`, `docs/architecture/publishing/frontend.md`, `docs/reference/measurements-site.md`, `docs/how-to/run-the-gates.md`. **Widened on execution 2026-09-12 from the fourteen the row first named, per section 0.1; the bullet below says what each addition is for. Re-checked against row #1's list, and group A still holds - row #1 opens no frontend file, no schema and neither of these contract modules.**
- **`frontend/tests/reading-page.spec.ts` is in the list and was not before.** `git grep -n data-rail-note -- frontend/tests` returns **two** specs, not one: `time-rail.spec.ts`, which this row deletes with the component, and **`reading-page.spec.ts:296`, which it does not**. **Corrected 2026-09-12 on execution: the two hits in `time-rail.spec.ts` are at lines 298 and 299, not `:299` and `:312`.** Deleting `TimeRail.svelte` takes that locator's target with it, so the row's own no-prisoners rule reaches a second file. **Row #10 also names `reading-page.spec.ts` and it is in group F**, so group A still holds.
- **What the row's file list missed, found on execution 2026-09-12 and shipped with it.** Nine more files, and each one is the no-prisoners rule reaching further than the list did: `frontend/tests/item-meta.spec.ts` is where the eyebrow cap is asserted, so it is where the oracle belongs; `frontend/scripts/test-groups.ts` refuses an unregistered spec file, so the rename goes through it; `frame.zone_time_rem` is the rail's column width and only the rail read it, so it goes too - taking `backend/tests/test_appearance_config.py`, `frontend/scripts/build-frame-css.mjs`, `frontend/src/styles/tokens.css`, `frontend/src/styles/frame.generated.css`, `frontend/src/lib/server/config.ts` and three fixtures under `tests/fixtures/contracts/` with it. `ui.rail_group_minutes` lives on `UiConfig`, which `AppConfig` and `AppearanceConfig` **share**, so `schemas/app-config.schema.json` and `backend/idhazh/contracts/app_config.py` moved as well - the row's list named only the appearance schema. And `time-rail.spec.ts` is **renamed** to `item-time.spec.ts` rather than deleted: most of what it asserts is about `format.ts`, which survives the rail, and deleting those cases would delete coverage of shipping code.
...
- **Oracle:** **The eyebrow still holds four children at every width and the time is the fourth.** Driven from the canary day, which plants a story of every `time_source` state on purpose - including `unknown`, which has **never happened in 8,922 committed items** (re-confirmed 2026-09-12) and which no archive-driven test can reach. The assertion is on the count and on the fourth child's identity, at each breakpoint the spec already drives. It fails against the base tree, **where the last eyebrow child is the source name and nothing carries `data-item-time`**. **Corrected 2026-09-12 on execution: the row said the base tree's fourth child is the day link, and it is not.** The day link is drawn only in a search result, so on a day page the base tree has **three** eyebrow children and **zero** day links - probed on the base build at 1536px: widest eyebrow 3, `[data-item-time]` 0, `[data-item-day]` 0, last child `span.source`. Both halves of the oracle still fail there, which is what the check was for.
...
| An item with no publisher time | The stamp is drawn unattributed - no mark, no word. That is the render `layout.md` already rules for the 3,733 items whose `time_source` is absent, and it is the honest one: printing it as a feed time is a claim the run never recorded, and refusing to print it deletes a fact from **41.8 percent** of the archive (re-measured 2026-09-12 over 8,922 items; the row said 44 percent against 8,550, and the count is frozen while the share falls every run) |
...
- **Files touched:** `frontend/src/lib/day-shape.ts`, `frontend/src/lib/payload/desks.ts`, `frontend/src/lib/components/FilterBar.svelte`, `frontend/src/lib/components/DigestList.svelte`, `frontend/src/lib/components/ArchiveSearch.svelte`, `frontend/src/routes/archive/+page.svelte`, `frontend/src/lib/server/config.ts`, `backend/idhazh/contracts/app_config.py`, `backend/idhazh/contracts/appearance_config.py`, `schemas/app-config.schema.json`, `schemas/appearance-config.schema.json`, `config/appearance.json`, `frontend/tests/topics.spec.ts`, `frontend/tests/layout-overflow.spec.ts`, `tests/fixtures/contracts/appearance-config/committed.json`, `tests/fixtures/contracts/appearance-config/defaults.json`, `tests/fixtures/contracts/app-config/every-knob-differs-from-the-committed-config.json`, `docs/architecture/publishing/frontend.md`, `docs/concepts/design-system.md`. **Widened by four paths on 2026-09-13, before dispatch**, and by nine more on execution. `layout-overflow.spec.ts` imports `splitPills` and drives it at four call sites, so a behaviour change reaches it; and a new `UiConfig` field breaks `test_fixture_round_trips_byte_identically` unless every fixture carrying that block gains the key in sorted order.
- **Two premises moved on execution, 2026-09-13, and the tree won (section 0.1).** **`topic_pills_max` is declared in `app_config.py`, not `appearance_config.py`** - it sits on `UiConfig`, which both documents share, so the row edits `app_config.py` and both schemas move together. And **`filter-bar.spec.ts` and `backend/tests/test_appearance_config.py` needed no edit**: the first drives the panel through a real page rather than through `splitPills`, and the second reads the schema stamp generically. `docs/concepts/design-system.md` was added instead: it carried a THIRD copy of the stale prerender sentence, in the paragraph that owns the fold-into-a-disclosure pattern.
...
- **What this row does not do:** it does not build the pill row, the disclosure or the count-based cut. **All three already exist**: `FilterBar.svelte` draws the pills, `splitPills` already cuts by story count, and the overflow already folds into a disclosure whose summary says how many. What this row changes is the **order**, the **threshold**, and the rule that bounds why a desk moves.
- **"Stops the order twitching" was wrong, and the measurement is what corrected it (2026-09-13, on execution).** Measured over the 23 committed days carrying all five desks, a margin of 2 moves 31 desk-days between consecutive days against 28 at a margin of 1 - **a bigger margin moves the row MORE**, because it pins some days to the alphabetical order and lets others go. What the margin actually buys is a bound on the REASON a desk moves: no desk overtakes another on a lead smaller than the margin, and the counts on the pills are what make that readable. **Nobody may later cite steadiness across days as grounds to raise the number.** The twitch the row is really about is between the five publishes within one day, and no committed payload records per-desk counts per run, so that half stays unmeasured.
- **After this row, nothing folds on any real day, and that is expected.** `config/taxonomy.json` declares five verticals and the threshold is five, so the fold goes from unreachable at eight to unreachable at five. The only change a reader sees today is the **order**. The fold rule is driven by fixtures and by `frontend/src/lib/payload/desks.ts`, which is empty until plan 23 row #16 promotes a vertical.
...
- **Files touched:** `backend/idhazh/placement.py`, `backend/idhazh/contracts/taxonomy.py`, `schemas/taxonomy.schema.json`, `config/taxonomy.json`, `backend/tests/placement/`, `backend/tests/contracts/`, `docs/concepts/placement.md`, `tests/fixtures/contracts/taxonomy/with-tombstones.json`. **Widened by one path on 2026-09-13, before dispatch:** a new field on `TaxonomyConfig` fails `test_fixture_round_trips_byte_identically` unless that fixture gains the key in sorted order in the same commit, and the row's own list did not name it. **Widened by one more on execution, 2026-09-13:** `backend/idhazh/assemble.py`, which is the only caller of `placement.place` and has to hand it the two things the rules read - the desk bounds off the taxonomy it already holds, and the desks this run will not render off the plan it already holds. Group D's other row is #4 and it had landed, so the widening collided with nothing (section 0.1). **Two premises moved too, both in this row's favour.** The three taxonomy fixtures this row worries about - `definitions-a.json`, `definitions-b.json`, `retired-event.json` - are committed already, so the optional-key rule is testable today rather than a promise about plan 23. And `schemas/taxonomy.schema.json` already carries `2026-09-13`, so "today's version" here means the minute form `2026-09-13T<HH>:<MM>`; a bare repeat raises `TypeError` at import from `contracts/base.py`. One more thing the Scope's shorthand hides: the desks live under `verticals` in `config/taxonomy.json`, so the keys are `verticals.<desk>.floor` and `verticals.<desk>.ceiling`.
- **Acceptance gates:** `GATE-PY` with ``backend/tests/placement/`, `backend/tests/contracts/``, `GATE-SCHEMA`, `GATE-SUITE`, `GATE-DAYS`. Plus:
...
| 6 | **The overflow goes to the desk whose feed carried the story, and where a story has no such desk it stays where it is.** Decided on execution, 2026-09-13, because decision 1 says this row does not land usefully without row #8 and row #8 is PENDING. Two things follow and both are asserted. A ceiling with nowhere to send a story **leaves it where it is** rather than dropping it, so the day is never shorter - which is what row #2 decision 1 forbids and the only thing this rule may not get wrong. And the valve opens one plan earlier than row #8: the moment a model reads the article and names a desk other than the feed's, the feed's desk becomes a second desk the overflow can reach. So this row goes live with **plan 23 row #6**, not with row #8, and row #8 replaces the fallback with a second desk of the article's own reading rather than supplying the first one. Measured 2026-09-13: 0 of 9,353 committed items across 24 days carry a read desk, so the rules move no day the site has published and `docs/concepts/placement.md` says so in those words | Editor; row #2 decision 1 |
| 7 | **The numbers, ruled 2026-09-13 by Editor over three rounds.** Floor 6 on all five desks; ceilings `india` 0.4, `world` 0.4, `ai` 0.35, `energy` 0.25, `business-economy` 0.25. Every one sits above the largest share supply has produced, because ESCALATE trigger 2 is about a desk over a third **when supply does not put it there** - a ceiling that cut a real day would be a quota. The Python defaults are the identity, floor 0 and ceiling 1.0, because a desk nobody configured has no rule rather than a strict one. A sixth number - a day size below which the ceiling switches off - was offered and refused: a desk's allowance is the larger of its floor and its share, which is `rank.day_source_ceiling`'s own arithmetic, and it switches the ceiling off at the exact point where it would have cut a desk under the breadth the floor guarantees | Editor |
...
- **Files touched:** `backend/idhazh/placement.py`, `backend/idhazh/contracts/article.py`, `backend/idhazh/contracts/digest_day.py`, `schemas/article.schema.json`, `schemas/digest-day.schema.json`, `frontend/src/lib/payload/types.ts`, `backend/tests/placement/`, `backend/tests/contracts/`, `docs/concepts/placement.md`, `docs/concepts/taxonomy.md`, `backend/idhazh/assemble.py`, `tests/fixtures/contracts/article/{brief,fetch-failed,ok,truncated}.json`, `tests/fixtures/contracts/digest-day/two-runs.json`. **Widened by seven paths on 2026-09-13, before dispatch, and the row's own list named none of them.** `assemble.py` is the only caller of `placement.place`, so a signature that now carries a second desk changes it - row #7 was widened for exactly this reason on execution and this row inherits it. The six contract fixtures fail `test_fixture_round_trips_byte_identically` the moment a field lands on `Article`, `DigestDay` or `DigestView`, because that test writes the model back out and compares bytes; each fixture gains the key in sorted order in the same commit. Group E's other rows have landed, so the widening collides with nothing. **Corrected again on execution, 2026-09-13, in three places.** `place`'s signature did not change after all - the second desk rides on the `DigestItem`, so what changed in `assemble.py` is `to_digest_item`, which copies the field, and `build_day`, which lists every name any story holds. **`backend/idhazh/contracts/digest_view.py`, `schemas/digest-view.schema.json` and `tests/fixtures/contracts/digest-view/one-day.json` are dropped**, because the field earns no place on the served payload under decision 6 below: `DigestViewItem`'s own rule is that a field without a named renderer does not earn the wire, and nothing renders this one yet. The served schema regenerated byte-identical, which is the proof. **`docs/concepts/taxonomy.md` is added**: it is the page that owns the desk vocabulary, it said in so many words that the desk is two fields and one vocabulary, and it is now three. **One page is owed and was not written, because a second worker held it**: `docs/architecture/publishing/layout.md`'s "Two words, and a count for each" section says "a day lists every name either word uses", and it is now three words.
- **Acceptance gates:** `GATE-PY` with ``backend/tests/placement/`, `backend/tests/contracts/``, `GATE-SCHEMA`, `GATE-WEB`, `GATE-SUITE`, `GATE-DAYS`. Plus:
  - all three schemas carry today's `version` and a `changelog` entry. **Corrected on execution: two schemas, not three** - `digest-view` does not change (above). `article` and `digest-day` both carried `2026-09-12T18:40` and neither carried today's date, so a bare `2026-09-13` was free on both and the minute form was not needed;
  ...
  - `frontend/src/lib/payload/types.ts` is edited by hand to mirror the new field, and the mirror is checked by eye against `schemas/digest-day.schema.json`. **Corrected 2026-09-13, before dispatch:** this gate read `regenerates byte-identical`, and nothing regenerates it. There is no TypeScript generator and no `frontend/src/contracts/` directory - the file's own header says it mirrors the schema, and `frontend/scripts/` contains no emitter for it. The drift gate covers `schemas/` only. A worker who runs an export and sees no diff has proved nothing about this file.
- **Oracle:** **A story is in the stream once and on at most two desks, and every desk's `count` equals the stories that name it either way.** Asserted on a built day where three stories cross-file: `len(stream) == len(set(item_id))`, no item names more than two desks, and each `DigestVerticalRef.count` reconciles. **The count reconciliation is the half that catches the real defect**, which is a desk page showing 40 and listing 37. **Corrected on execution, 2026-09-13, because the tree moved under it.** `DigestVerticalRef` gained `desk_count` beside `count` on 2026-09-12, so "every desk's `count`" is now two numbers with two meanings: `count` answers for the feed's declared word and `desk_count` for the desk the day filed the story under. Under decision 6 neither counts a second desk, so the reconciliation asserted is that both agree with the items beside them and both sum to the length of the day. It is asserted by building the day through `DigestDay` itself rather than by re-counting in the test, so the contract is what reconciles. Both halves of the change were proved load-bearing by neutering them: ignore the article's reading and the oracle fails on the desks a story names; overwrite the first desk instead of swapping, and the payload refuses to validate.
- **What this row does not do:** it does not draw a second chip on the item. The eyebrow's cap is four and it is full (row #5). A story's secondary desk is why it appears on that desk's page; it is not a fact the item's own line prints. **Corrected on execution: it does not put the story on that page either** (decision 6).
...
| 4 | **The second desk comes from the same source the first does.** Today that is the feed's declared vertical, so the secondary is empty on almost every item and the ceiling's overflow has nowhere to go - **which is the honest state of this row until a model actually writes a desk**, and the row says so in `placement.md` rather than shipping a valve that is closed and looks open. **Corrected 2026-09-13, before dispatch.** This decision read `until plan 23 row #6 lands`. That row landed as #620 and `Article.desk` exists, so the sentence as written says the valve is open and it is not: 0 of 9,353 committed items across 24 days carry a read desk, because nothing populates the field until plan 23 row #8 ships the labelling call. The gate is a populated `desk`, not a declared one. Row #7 decision 6 already shipped the fallback that keeps the day whole in the meantime, so this row replaces a fallback rather than supplying the first valve | Verified 2026-09-11; corrected 2026-09-13 |
...
| 6 | **The count and the desk page widen together or neither does, and in this row neither does.** Decided on execution, 2026-09-13. Decision 5's rule is the one being kept - the pill is a promise about what its page lists - and keeping it means the number and the list are the same set on both sides of this change. Three things decided it. Nothing cross-files today, so widening both would describe behaviour that fires on no day: 0 of 9,353 committed items across 24 days carry a read desk. Widening only the count ships the exact defect the Oracle names, a pill reading 40 over a page listing 37. And widening both would need a render test in `frontend/tests/`, which a second worker held for the length of this dispatch, so the page half would ship untested - which is how that defect gets in. So this row ships the mechanism: the article's own second reading replaces the feed's word inside `placement.second_desk`, and the field is published on the committed day. It is not a dead field - it is what stops a crowded desk sitting over its ceiling the day labelling arrives | Editor, 2026-09-13 |
| 7 | **A move swaps the two desks; it does not consume the second one.** The story goes to its second desk and the desk it left becomes its second. A ceiling says a desk is busy today and nothing about the story changed, so erasing what the story was read as would hand a crowding rule a second job nobody gave it. Consuming was rejected on what it costs a reader: on a heavy AI day, AI stories would fall off the AI page **because** the desk was busy, which is the inverted failure and the reader cannot see the reason for it. The swap is self-limiting - a story that has moved has its old desk as its second, so neither pass can send it back | Editor, 2026-09-13 |
| 8 | **A second desk this run will not render is dropped rather than published.** `rank.desk_of` already refuses to file a story on a desk found below its feed floor, because a page drawing stories under a name the same day's payload says planned nothing is the page and the operator surface disagreeing about one word. The second name is owed the same rule, and `placement.secondary_desk_of` is where it is applied at write time. `refile` keeps its own `closed` check as well, because the day also holds items an earlier run published against a different set of closed desks | `backend/idhazh/rank.py`; Fowler |
...
- **Files touched:** `backend/idhazh/contracts/digest_day.py` and `schemas/digest-day.schema.json` (`desk_count`'s meaning and its validator), `backend/idhazh/assemble.py` (the count it writes), `frontend/src/lib/day-shape.ts`, `frontend/src/lib/server/payload.ts`, `frontend/src/lib/components/DigestList.svelte`, `frontend/src/routes/+page.svelte`, `frontend/src/routes/archive/+page.server.ts`, `frontend/src/lib/payload/types.ts`, `backend/idhazh/contracts/digest_view.py`, `schemas/digest-view.schema.json`, `tests/fixtures/contracts/digest-view/one-day.json`, `frontend/src/lib/payload/project.ts`, ``backend/tests/placement/`, `backend/tests/contracts/``, `frontend/tests/` (the render test this row exists to be able to write), `docs/concepts/placement.md`, `docs/architecture/publishing/layout.md`. Derived on 2026-09-13 by reading the callers; it is a start and not a fence (section 0.1).
- **Acceptance gates:** `GATE-PY`, `GATE-SCHEMA`, `GATE-WEB`, `GATE-BROWSER`, `GATE-SUITE`, `GATE-DAYS`.
- **Oracle:** **The pill and the page are the same set on a day where a story cross-files, and the day's own total counts that story once.** A built day, asserting the desk page lists the cross-filed story under both names, `desk_count` equals what each page lists, and the day total does not double-count it. **The third assertion is the one that catches the regression this row can cause**: the three day totals sum `desk_count` today, and a widened `desk_count` makes that sum larger than the day.
- **What it must also carry:** the three day totals switch from summing `desk_count` to summing `count`, which sums exactly to the day's item count whatever cross-files; and the ceiling sentence in [`../docs/concepts/placement.md`](../docs/concepts/placement.md) is rewritten to say it bounds where the day **files** a story rather than how many a reader can reach under that name.
- **Pull it forward the moment anything starts populating `Article.desk` on a real day.** From that day on a cross-filed story is reachable under one name where it should be reachable under two, and the reader who follows the desk it left cannot see that it is missing. Editor, 2026-09-13.
...
- **Files touched:** `backend/idhazh/assemble.py`, `backend/idhazh/cli.py`, `frontend/src/lib/server/payload.ts`, `frontend/src/lib/components/DigestList.svelte`, `frontend/src/lib/day-shape.ts`, `backend/tests/pipeline/`, `backend/tests/test_same_story.py`, `backend/tests/test_leading_stories.py`, `frontend/tests/dated-day.spec.ts`, `frontend/tests/day-list.spec.ts`, `frontend/tests/day-seam.spec.ts`, `frontend/tests/reading-page.spec.ts`, `frontend/tests/readstate.spec.ts`, `frontend/tests/topic-day.spec.ts`, `frontend/tests/whole-day.spec.ts`, `docs/architecture/publishing/layout.md`
...
- **Acceptance gates:** `GATE-PY` with ``backend/tests/pipeline/`, `backend/tests/test_same_story.py`, `backend/tests/test_leading_stories.py``, `GATE-WEB`, `GATE-BROWSER`, `GATE-SUITE`, `GATE-DAYS`, and the section 12 browser smoke. Plus:
...
- **Files touched:** `frontend/src/routes/console/judgement/+page.svelte`, `frontend/src/routes/console/judgement/+page.server.ts`, `frontend/src/lib/console/judgement.ts`, `backend/idhazh/publish_console_judgement.py`, `backend/idhazh/contracts/console_judgement.py`, `schemas/console-judgement.schema.json`, `tests/fixtures/contracts/console-judgement/newest-day.json`, `backend/idhazh/contracts/app_config.py`, `schemas/app-config.schema.json`, `config/idhazh.json`, `backend/tests/test_console_judgement.py`, `backend/tests/contracts/`, `backend/tests/test_marks.py`, `frontend/tests/console-judgement.spec.ts`, `docs/architecture/publishing/console.md`, `docs/architecture/publishing/console-payloads.md`, `docs/architecture/publishing/console-charts.md`, `docs/concepts/console-design.md`
- **Depends on:** row #11, and **plan 23 row #14**. See the section below.
- **Acceptance gates:** `GATE-PY` with ``backend/tests/test_console_judgement.py`, `backend/tests/contracts/`, `backend/tests/test_marks.py``, `GATE-SCHEMA`, `GATE-WEB`, `GATE-BROWSER`, `GATE-SUITE`, and the section 12 browser smoke. Plus, in this row:
...
- **Files touched:** `frontend/src/routes/console/voices/+page.svelte`, `frontend/src/routes/console/voices/+page.server.ts`, `frontend/src/routes/console/+page.svelte`, `frontend/src/routes/console/+page.server.ts`, `frontend/src/lib/console/band.ts`, `backend/idhazh/publish_console_band.py`, `backend/idhazh/publish_source_health.py`, `backend/idhazh/contracts/source_health_view.py`, `schemas/source-health-view.schema.json`, `tests/fixtures/contracts/source-health-view/four-facts.json`, `backend/tests/test_publish_source_health.py`, `frontend/tests/console-voices-sources.spec.ts`, `frontend/tests/console-voices-feeds.spec.ts`, `frontend/tests/console-voices-cuts.spec.ts` (the three renamed from `console-sources`, `console-feeds` and `console-source-cuts`), `frontend/tests/console-voices.spec.ts`, `frontend/tests/console.spec.ts`, `frontend/tests/console-window.spec.ts`, `frontend/tests/console-nav.spec.ts`, `frontend/tests/console-axis.spec.ts`, `frontend/tests/console-mark-parity.spec.ts`, `docs/architecture/publishing/console.md`, `docs/architecture/publishing/console-payloads.md`, `docs/architecture/publishing/console-charts.md`, `docs/concepts/console-design.md`, `docs/architecture/sources/health.md`
- **Acceptance gates:** `GATE-PY` with `backend/tests/test_publish_source_health.py`, `GATE-SCHEMA`, `GATE-WEB`, `GATE-BROWSER`, `GATE-SUITE`, and the section 12 browser smoke. Plus, in this row:
  ...
  - **corrected from** "`config/idhazh.json` gains a `payload_ceilings_bytes` entry for the tab's payload", **2026-09-14**, because no instrument in the repository can hold that payload and the clause asked for one that would fail the build. `payload_ceilings_bytes` keys resolve against the BUILD directory, and `frontend/public/source-health.json` never reaches it: `copy-visuals.mjs` stages seven named console directories plus `digest/`, `telemetry/` and `assist/index/`, and that file is in none of them - the page reads it at build time through `SOURCE_HEALTH_PATH` and no browser ever fetches it. `bundle-gate.mjs` fails any key that matches nothing, so the clause as written turns the gate red on the commit that satisfies it. `page_weight.ceilings_bytes` is not a fallback either: every console route left it on 2026-09-10 in `a4f6a285` ("Delete the four page numbers that move when a run publishes"), because those numbers move when a run publishes and a number somebody has to raise on every publish teaches an operator to raise numbers. Voices inlines feed rows, a census and four source-cut tables, so it is exactly that kind of page. Nothing this row adds is fetched over the network - the only console payload Voices asks for is `console/band.json`, which the layout already fetched on every route and which `payload_ceilings_bytes` already caps at 2,000 bytes.
...
**`Pipelines` answers three questions today** - did the runs work, which feeds broke, and what each stage cost - and its own route description says exactly that. **The feed half is already four panels**: `Sources we may ask, and what they yield`, `Sources cut short most often`, the failure list, and the clean-read count under it. Moving them leaves `Pipelines` answering one question. (**corrected from** `Four facts about every source we may ask`, **2026-09-14**: that heading was renamed before this row was written and the row was never updated. The phrase survives as the panel's *description* in `docs/architecture/publishing/console.md` and `docs/architecture/sources/health.md`, which is a different thing from its heading.)
...
- **Files touched:** `backend/idhazh/diversity.py`, `backend/idhazh/publish_day_metrics.py`, `backend/idhazh/contracts/day_metrics.py`, `schemas/day-metrics.schema.json`, `tests/fixtures/contracts/day-metrics/full.json`, `backend/idhazh/contracts/app_config.py`, `schemas/app-config.schema.json`, `config/idhazh.json`, `backend/tests/test_diversity.py`, `backend/tests/test_day_metrics_producer.py`, `backend/tests/contracts/`, `backend/tests/test_marks.py`, `frontend/src/routes/console/+page.svelte`, `frontend/src/routes/console/+page.server.ts`, `frontend/tests/console-desk-mix.spec.ts`, `docs/concepts/placement.md`
- **Acceptance gates:** `GATE-PY` with ``backend/tests/test_diversity.py`, `backend/tests/test_day_metrics_producer.py`, `backend/tests/contracts/`, `backend/tests/test_marks.py``, `GATE-SCHEMA`, `GATE-WEB`, `GATE-BROWSER`, `GATE-SUITE`, `GATE-DAYS`. Plus, in this row:
...
| **An organisation announcing its own product, at the lead** | The lead slot is held by an `institution` feed declaring its kind as `announcement` or `research` on **8 of the 13 committed days** that carry `rank_score`, up from 4 of 13 before row #4 turned carriage into a tie-break. Both figures are against `config/sources.json` as it stands today. Nothing in the score separates an institution reporting a fact about the world from an institution announcing its own product: the institution tier is 1.0 because one institution saying a thing makes it true, which is an argument about whether a fact is reliable, and it is being spent on whether a story is important | **Not row #7.** Its floor and ceiling bound a desk's share of the **day**, and row #4 measured that the day's desk mix does not move at all - 5 desks, a median 16 feeds in the head, the biggest desk still a median quarter of it. A ceiling on the day cannot reach one slot. It belongs beside `ui.lead_max_yesterday`, which is already a bound on the lead slot keyed off something other than the score, and no row of this plan owns that block. **The step size is not the lever either**: every value in its legal window leaves all 8 of those leads exactly where they are. Named by Editor, 2026-09-13, who accepted it for a bounded period on the condition that it is written down with its count - which it is, on [`../docs/concepts/placement.md`](../docs/concepts/placement.md) |
| **Two feeds of one organisation are two feeds** | `placement.head_no_repeat` stops one `source_id` repeating in the first ten stories, and `nvidia-newsroom` and `nvidia-technical-blog` are two `source_id`s. A reader reads brand names rather than feed ids, so a head carrying three NVIDIA stories in eight reads as one voice however many feeds supplied it | It is the same missing relation as the wire-syndication row above: `config/sources.json` carries no field saying two feeds belong to one organisation. Adding one is a source-contract change, and the frame rule that would read it is row #7's neighbourhood rather than row #4's. Found 2026-09-13 by Reader, on row #4's own output |
...
| **The recency half-life** | `collect.recency_half_life_hours` is 18 over a 24-hour admission window, and that makes the term nearly a constant. Measured 2026-09-13 by row #3 over the 5,442 of 5,682 published stories whose bonus can be recovered from the published score: it fires on every one of them, paying a median of 0.5656 against a 0.6 ceiling - 94 percent of the most it can pay - and the middle half of the stream spans 0.5053 to 0.5874, **a spread of 0.08, a quarter of the smallest tier step**. The median story is 1.5 hours old when it is scored. So the term the editor ranked second separates almost nothing | Row #3's own scope names an 18-hour half-life, so the row that found this could not touch it. Raising `recency_weight` does not fix it either - it scales a flat term. **Only the half-life would**, and nothing owns it. Named by Editor, 2026-09-13 |
| **An owner for the lens term** | `LensDef.weight` in `config/taxonomy.json` is a term of the selection score - 11.4 percent of the stream, 20 percent of head slots, heaviest weight 0.3 - and no row of this plan names it as one. Section 0.3 already measures that four live lenses sit at 0.0 and fire 3.6 times as often as the two that pay | Row #3 does not name it, and it is the term plan 23 row #17's per-run loop moves, so retiring or re-pricing it here would descope another plan's row. Row #7 writes `config/taxonomy.json` and rules on desks rather than lenses. Named by Editor, 2026-09-13 |
| **The two salience feeds** | `hn-frontpage` and `hn-best` cost one request each per run and supply a fact that reached 8 of 5,682 published stories. [`../docs/architecture/sources/discovery.md`](../docs/architecture/sources/discovery.md) set the condition on 2026-08-30 - retire both if the vote fires on under 1 percent - and row #3 measured 0.14 percent on 2026-09-13 | Row #3 honoured the half of that sentence about the weight and left the half about the feeds. Retiring a feed is a `config/sources.json` decision outside row #3's scope, and Editor ruled the published fact stays so a later row can restore the term if supply changes. **The measurement is over published rather than planned items**, which is the population the condition names. **The question that settles it, set by Editor on 2026-09-13: how many stories over those 13 days entered the pool ONLY through `hn-frontpage` or `hn-best`.** At zero the two feeds are pure duplication and go with the term; above zero they are a discovery source and the vote's retirement says nothing about them |
