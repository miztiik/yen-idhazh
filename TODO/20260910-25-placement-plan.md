...
- **Oracle:** **Five tabs stand on no more than two rows at 1440px and no more than three at 360px, and no tab's box overlaps another's.** Geometric, read off the built page rather than off the rule. **Corrected 2026-09-12: this row said "at the widths `console-nav.spec.ts` already drives" and that spec drives none** - it had no `setViewportSize` call at all, so the two widths are added by this row rather than borrowed. **It fails against the base tree in both directions**: three tabs cannot produce five boxes, and five tabs at the committed basis produce five rows on a phone rather than three.
