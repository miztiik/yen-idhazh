...
| The Status Reckoner: rows 11 to 19 and 21 are BLOCKED on the console-signal plan's row 17 | That plan closed as #298 on 2026-08-30 and every blocked row has since merged - #306 (row 11), #301 (row 12), #308 (rows 13, 14, 15 and 16), #307 (row 17), #310 (rows 18 and 19). The table was never updated because a row cannot edit the table that dispatches it, which is a defect of this plan's shape rather than of any row. |
| Row #21 re-derives the first-load JavaScript ratchet from CI, and its file list names `frontend/bundle-baseline.json` | **Both are gone.** PR #299 deleted the per-route first-load ratchet and the baseline file on 2026-08-30: nothing required it, a local build could not reproduce CI's inside its own 64-byte tolerance, and its record was one file every branch had to rewrite. Closure re-derives the prerendered-document ceilings and nothing else. Row #21 decision 2 describes a gate that no longer exists. |
| Row #16 cannot start until a rate is named, and the rate is UNRESOLVED | Named and shipped with #308. `observability.cost_currency`, `cost_input_per_million` and `cost_output_per_million` are config knobs with committed defaults, and the panel prints the rate it used and whether it came from the config or from the operator. |
| Finding D - `score_ms` is still on the Pipelines critical-path timing table | Closed by #307. `frontend/src/lib/server/model-work.ts` reads it for the Model route, and `frontend/src/routes/console/+page.server.ts` carries a comment recording that it used to be a fourth entry there. |
| Row #7's file list names `backend/idhazh/contracts/item_health_row.py` | The file is `backend/idhazh/contracts/item_health.py`, and the widening touched 13 files rather than the 6 listed - the row's own two test modules were both missing from the list. |
| `state/runtime-counters.csv` holds 29 data rows over 6 runs | **52 rows over 12 runs**, on 2026-08-31 after #309 settled 81 duplicate rows out of three ledgers. Every number this plan derived from that file was taken over a ledger that held rows twice. |
| The lazy chart chunk stands at 197,561 B and one new echarts registration crosses the 200,000 B trigger | The premise held and the trigger never fired. Twenty-one rows later the chunk is **byte for byte 197,561 gzipped, 585,481 raw, and the same content hash** - every panel this plan added is hand-written markup or a type `core.ts` already registers. |
| Finding A - `/console/` crosses its 301,580 B ceiling on published day 16 | **Superseded by the split rather than fixed, and the shape did not change.** That figure was taken on a page that still carried the model panels. On 2026-08-31 `/console/` measures 116,153 gzipped bytes on ten published days at 19,301 bytes a day, against a re-derived 251,324, so the crossing moved out and the page is still linear in items. Both facts the finding rests on survive: `state/scores.csv` is unbounded on purpose (Finding B), and the answer when the gate fires is to re-measure and raise it (the owner's byte ruling). |
| Row #21 measures the console page height against an 8,794px baseline | That baseline is a page that no longer exists - the split replaced it with three. Measured per route on 2026-08-31 rather than against it. |
