...
| B | **`state/scores.csv` (24.9 percent of `state/`) and `state/seen/` (54.5 percent) are still unbounded.** Row #3 bounded `state/item-health/` only, correctly: only item-health rows carry a `stage`, which is what the owner's `(date, stage)` aggregate is keyed on, and `state/seen/`'s honest retention is deletion rather than a fold - a different decision the owner has not taken. | A separate retention decision, not a bug in Row #3. |
