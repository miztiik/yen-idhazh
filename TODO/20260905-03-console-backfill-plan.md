...
| 4 | Faithfulness and lead coverage join them | 3 | D | DONE #428 | yi-c04 | #428 | Susan - agent file read directly, no runSubagent. Andre, Jony, Carmack, Reader applied from the docs they own, not from their agent files |
| 5 | The ceilings are re-recorded by the plan that grew them | 2, 4 | E | DONE #429 | yi-c05 | #429 | Carmack - agent file read directly, no runSubagent. `/console/` and `/console/machine/` re-derived to 335,051 and 44,706; `/console/model/` left at 56,385 because a fresh derivation lands 73 bytes above it and the committed number already carries 6.99 publishes. Seven publishes taken where the 2x margin rule would have allowed nine |
