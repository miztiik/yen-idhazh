...
| 14 | The concurrency group goes | I | 7, 9, 13, 15 | 6 | DONE | 4 | p46i | - | GitHub Copilot |
...
| 11.4 | Table C is the authority on every path. **Two corrections an earlier draft got wrong**: span-rollup moves from month to day grain, so `month_partition.month_files` keeps only its `frontend/public/` callers, and a trace carries no `<ledger>` level. **Corrected 2026-09-22, both measured against the tree.** The draft's third correction was itself half right, and the half that is wrong is the useful half: no `state/validation/` tree exists at the repository root, but `SegmentLedger.VALIDATION` is live - `stages/decide.py` and `stages/qualify_decide.py` both write it - and it lands under whatever `common.STATE_ROOT` names, which a pipeline-tests dispatch points at `state/pipeline-tests/`. So the ledger is real and migrates with the rest; only the root path is absent. The draft also described the trace name wrongly: the committed name is `<DD>-<ordinal>-<shard>.jsonl`, which carries the run's **ordinal** rather than its whole id, and carries neither `attempt` nor `job`. That is what `ledger.PRE_IDENTITY_TRACE` spells and what the 104 committed traces measured on 2026-09-22 match. | Carmack |
...
| 11.8 | **This is the one PR that lands alone, and it asked for a quiet window.** A digest run in flight when it merges pushes an old-shape head the migration missed. Measured: the longest gap in the schedule is **4 h 48 min**, between the 18:20 run finishing and 02:20. **Owner ruling 2026-09-22: the schedule stays on, and losing one run to this merge is acceptable.** The cost is one run's rows, which the next run re-derives, against holding a 292-file migration for a window somebody has to be awake for. ESCALATE trigger 1 is answered and closed. | Carmack, ruled by the owner |
...
| 11.14 | **Eight paths leave the rebuild list in this row.** Seven because they are written once - item-health, host-fingerprint, scores, score-index, span-rollup, traces and segments - and `state/published` beside them because it is union-safe and the hand-back was **discarding this run's appended rows before the union driver could ever fire**, leaving the publication record dependent on a rebuild succeeding. **Corrected 2026-09-22, measured against `idhazh.paths.DERIVED`:** `hand_back` still touches two `state/` paths, not none. `state/day-metrics` stays, because it is derived - assemble rewrites one whole JSON per day from that day's rows, so two runs of one day do land on one path, and a text merge of two JSON objects is not JSON. The closed-day fold stays for the same reason and is the one entry the rebuild leaves out, because `idhazh assemble` re-emits no fold and a job that handed it back would delete it. **What the eight buy is correctness, not time**: against a 5-second rebuild in an 11-second step there is no time to win. | Fowler |
