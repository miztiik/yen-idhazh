...
4. **One declared locale, applied at build time, identical for every reader.** The separator hazard is real - `1.234` is one thousand two hundred and thirty-four in Berlin and slightly more than one in London - and the answer is **not** to read the reader's browser. A runtime locale read makes two readers see different bytes, which breaks the byte-identity oracle, and it is behaviour that varies by reader, which Rule #1 refuses. The locale is a config value.
