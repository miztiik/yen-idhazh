...
| C3 | The renderer is a hard-coded bar | True, and worse: `chart_spec()` hard-codes `"color": "#4c6ef5"` and `"labelFont": "sans-serif"`, which is a live Guardrail #6 violation | [backend/idhazh/route.py](backend/idhazh/route.py#L437) |
...
| C16 | **This document** re-baselines `frontend/bundle-baseline.json` and the first-load JS ratchet (doc B, section 14.1) | **Both were deleted on 2026-08-30.** The file does not exist and `bundle-gate` now does exactly two things: the encoder grep and the page ceilings read from `config/idhazh.json`. Doc B's byte half shrinks to re-recording `page_weight.ceilings_bytes`, and every ratchet caveat it inherited is dead | [frontend/scripts/bundle-gate.mjs](frontend/scripts/bundle-gate.mjs) |
| C17 | **This document** (O26, section 14.1) asks for parallel test execution | **Already shipped.** `[tool.pytest.ini_options]` reads `addopts = "-q -n auto"`. What is still missing is only the `markers` list, so 14.1's test-filtering half stands and its parallelism half is done | [pyproject.toml](pyproject.toml) |
| C18 | **This document** (row 25) retires `visuals.canvas_width` and `canvas_height` "as a fixed pair" | **There is no `canvas_height`.** The block holds `canvas_width` 800 alone, plus four knobs no row names: `max_chart_points` 8, `max_facts` 16, `max_diagram_steps` 6, `min_diagram_steps` 3. Each has to move or die with the renderer, and a row that names a knob which does not exist will not find the four that do | [config/idhazh.json](config/idhazh.json) |
| C19 | **This document** (section 11.2a) presents `--flash-attn` as a flag to add | **The config key already exists**: `models.inference.flash_attention`, set to `null`. O22 is a value change, not a new knob, and the qualification gate already reads it | [config/idhazh.json](config/idhazh.json) |
| C20 | **This document** (O8) sets `retention.image_months` to 13 | **The committed value is -1**, which means no age prune at all. So O8 is not a tightening from one window to another; it is the first time an age window exists. The `dry_run` half is unchanged and still `true` | [config/idhazh.json](config/idhazh.json) |
| C21 | **This document** (section 7) buys d3 as a new dependency and prices it | **`d3-array` and `d3-scale` are already dependencies**, beside `echarts` ^5.6.0. O5's install cost is therefore lower than section 7 implies; what section 7.2 actually adds is the shape, axis and layout modules | [frontend/package.json](frontend/package.json) |
| C22 | **This document** (row 68, doc L) puts `LabelRow` in `backend/idhazh/evals/labels.py` | **`LabelRow` is in `backend/idhazh/contracts/label_row.py`**; `evals/labels.py` imports it. It carries `ConfidenceBand` and **no `label_source` and no `model_id`**, so both of O41's stamping guardrails are new fields on a persisted contract rather than existing columns - a schema stamp, a changelog entry and a fixture round trip each | [backend/idhazh/contracts/label_row.py](backend/idhazh/contracts/label_row.py) |
| C23 | **This document** (O29, section 14.4b) widens `publish_telemetry.PUBLIC_COLUMNS` "by eight already-committed columns" | **`PUBLIC_COLUMNS` is `PublicTelemetryRow.csv_columns()`.** Widening it is a change to a persisted contract - schema stamp, changelog entry, fixture round trip, and the canary writer that restates the header in JavaScript - not an edit to a tuple literal. The work is real and it is bigger than the row implies | [backend/idhazh/publish_telemetry.py](backend/idhazh/publish_telemetry.py) |
| C24 | **This document** (section 14.1) prices `/console/` at 276,828 B and names three console ceilings | **`page_weight.ceilings_bytes` holds six keys**: `/404` 1,706, `/archive/` 7,553, `/console/` 277,195, `/console/machine/` 39,743, `/console/model/` 37,979, `/evals/` 3,279. The `/console/` figure moved inside a day of being written, which is the recorded behaviour of a ceiling measured against a tree the pipeline rewrites several times an hour | [config/idhazh.json](config/idhazh.json) |
| C25 | **This document** (O35, section 3) asks for the visual below the summary and says `ui.visual_side` is `above` today | **There is no `ui.visual_side`, and O35 is already satisfied.** `config/idhazh.json` dropped its copy on 2026-09-05, so `config/appearance.json` `digest.visual_side` is the only declaration and it reads `trailing`; the contract default moved to `TRAILING` in the same commit to match. And the placement O35 asked for already ships: `DigestItem.svelte` renders the title, the summary, the reader note, then the figure. So O35 is a row to close, not a row to run | [config/appearance.json](config/appearance.json), [frontend/src/lib/components/DigestItem.svelte](frontend/src/lib/components/DigestItem.svelte) |

**C16 to C25 correct this document rather than the proposal, and were verified against the tree on 2026-09-05.** They are in the same numbered sequence deliberately: a reader checking a claim should not have to know which document made it. Eight of the ten make a row cheaper or already-done; two (C22, C23) make a row more expensive, because a tuple edit turns out to be a persisted-contract change. **The pattern is the one section 12.15 already named** - a claim about the current tree rots in days on this repository, and none of these was older than a fortnight.
...
| O44 | **The "route" rename is paid up front, as the first structural change, and not late in the build.** O2 scrubbed the word and X4 costed it, but both left it inside plan-doc H - so every phase before H writes the wrong name and every phase after H pays a rename tax on code it just wrote. Owner decision 2026-09-05: it lands as a preliminary, behaviour-free change before any new visual work begins. "Route" names a dispatch decision and the thing being built is a planning decision, so leaving it in place is technical debt that compounds with every file the programme adds. **Two breaks X4 did not name ship with it.** `config/idhazh.json` is a persisted surface, so `models.route`, `run.route_budget_minutes` and `finetune.student` are breaking key renames that need a read side accepting the old key. And `StageName.ROUTE` reaches **published** run manifests, so the Python member renames while the wire value stays `"route"`, or every committed manifest stops validating. The cost is stated rather than hidden: the rename touches code later phases substantially rewrite, and that is accepted - a mechanical rename is cheap, while the config keys and the published wire values are the half that must only ever be paid once | O2, X4, C1 |
| O45 | **A derived value is not an invented number, and the objection to the composition family is overruled.** Ruled 2026-09-05, after Carmack proposed dropping `pie`, `bubble`, `histogram` and the Derived Value contract on the grounds that they "let the planner state a number the article never did". **The planner states nothing.** Every value is arithmetic that **code** performs over Tier 1 elements **code** extracted, through a **closed** three-function allow-list - `count`, `sum`, `share_of_declared_whole` - with a provenance chain back to each input element. The model picks the type and labels the axes; it cannot name a function, cannot supply an operand, and has no numeric field anywhere in its schema (section 10.1a). The metric the objection reached for, new-fact rate, measures whether a **summary key point** states something the summary prose does not - a summariser instrument about prose, unrelated to a chart's arithmetic. **One real item survived the objection, and O47 has since closed it**: `convert` was not on the allow-list, so `4,200 tonnes` beside `4.2 kilotonnes` was a rejection. O47 makes `convert` the **fourth** function, with a versioned unit table and a provenance chain recording source and target units, so the list is one longer and still closed. M7 is worth taking because it sizes the `pie` template's value, and it gates nothing | P.2.3, P.L22, row 45, 12.13 G29, O47 |
| O46 | **The potential classifier ships in two pieces, and the cheap piece ships first.** Amends O42. Fowler would not build it at all - the element table answers `chartable` and `narrative` as a query over data already committed, at no new contract and no new config cost. Andre showed it cannot be built where O42 put it, because four of its seven signals read call-1 output that does not exist until the two-call flow lands. Both are right about a different half. Ruled 2026-09-05: **the query ships with the element table**, giving `chartable` and `narrative` a real denominator immediately; **the contrastive and sequencing lexicons ship with the diagram family**, which is the only surface that needs `processual` and `comparative` and is also where 12.15 G46's edge decision procedure is authored - the same linguistic artefact seen from the other end. A whole plan-doc becomes one function now and one word-list pass where it bites. **What is lost, stated:** `comparative` and `processual` have no denominator until the diagram family lands, so any rate reported per class before then names three classes rather than five, and must say so | O42, 12.11 G16, 12.15 G46 |
| O47 | **Every number a reader sees is normalised for reading, and the two acts inside that sentence are governed differently.** Ruled 2026-09-05, and written out in section 7.5. **Formatting** - `2000000` drawn as `2M` - moves no quantity and is the compiler's job, needing no provenance. **Conversion** - `4200 tonnes` drawn as `4.2 kt` - is a different number against a different unit, so it is a derived value and `convert` becomes the **fourth** function on O45's allow-list, reading a versioned unit table and recording source and target units. SI prefixes and metric units wherever the quantity has an SI form; a unit with no SI form keeps its own; a non-SI source unit is converted with the original recorded rather than replaced, which is safe because an element's Tier 1 `surface` is never overwritten. **One declared locale applied at build time**, because `1.234` names two different numbers in Berlin and London and reading the reader's browser would make two readers see different bytes - breaking the byte-identity oracle, and being exactly the reader-varying behaviour Guardrail #1 refuses. Precision is declared, not inherited. A shortened number still faces the legibility floor. One formatter serves the compiler and the console, because two formatters is how `2M` and `2,000,000` reach one page | O45, 12.13 G29, section 7.5 |
...
| 48 | Template order follows observed frequency | Jony | Not the fixed wave order in P.4.3.1. P.D2 makes the order derivable from `planned_type`, so guessing it is an unmeasured number justifying a design (Guardrail #10) | P.4.3.1, P.L2 |
...
| 60 | `PAGES_HARD_CAP_MB` to config with `le=1024` | Carmack, Fowler, Owner O7 | Config can lower it and cannot raise it, so Guardrail #2's "the budget is the platform, not a preference" stays enforceable | P.6.2 |
...
| 64 | Wire in the existing faithfulness scorer | Fowler, Andre | The eval ledger, `ConfidenceBand` and `BandReason` already exist and `LabelRow` already imports `ConfidenceBand`. Do not build a second scorer (Guardrail #8) | P.L25, P.Q8 |
...
| 66 | Call it offline paired evaluation | Andre | "A/B testing" names a mechanism Guardrail #1 forbids, and the numbers would eventually be quoted as if readers produced them | P.L26, P.6.3.4 |
...
4. Run exactly N ticks in a synchronous loop, N from `config/` (Guardrail #6). N = 300 matches the default schedule, where alpha reaches `alphaMin` 0.001.
...
### 7.5 Number and unit normalisation

Owner decision O47, 2026-09-05: every number a reader sees is normalised for reading. **Two different acts hide inside that sentence and the contract governs them differently.** Running them together is how a formatting rule quietly becomes permission to state a figure the article never gave.

| Act | Example | A new value? | Who does it | Governed by |
|---|---|---|---|---|
| **Format** | `2000000` drawn as `2M` | **No.** Same quantity, different glyphs | The compiler | The formatter, versioned |
| **Convert** | `4200 tonnes` drawn as `4.2 kt` | **Yes.** A different number against a different unit | Code, never the model | `convert`, the fourth allow-list function |

**Formatting needs no provenance chain**, because nothing about the quantity moved. **Conversion does**, so O45's closed allow-list grows from three functions to four - `count`, `sum`, `share_of_declared_whole`, `convert` - and `convert` reads a versioned unit table, records the source unit and the target unit, and is counted by `derived_value_rate` like any other derived value. The list is still closed; it is one longer.

**Nothing is lost, and that is what makes conversion safe here.** An element's Tier 1 `surface` is the article's own characters and is never overwritten (section 10.1b). The axis draws the readable form, the provenance carries the original, and the accessible description can carry both.

#### The rules

1. **SI, and metric, wherever the quantity has an SI form.** A prefix rather than a run of zeros: `2M tokens` not `2,000,000 tokens`; `4.2 kt` not `4,200 tonnes`; `380 GW` not `380,000,000,000 watts`. **The prefix set is the SI set and nothing else** - no `bn`, no `lakh`, no `crore` on an axis, because each of those is read differently by a different part of the audience.
2. **A unit with no SI form keeps its own.** A currency, a headcount, a percentage. `share_of_declared_whole` already yields a percentage and stays one.
3. **A non-SI source unit is converted, and the original is recorded rather than replaced.** An article's `gallons` draws as `litres` with `gallons` in the provenance chain and in the accessible description.
4. **One declared locale, applied at build time, identical for every reader.** The separator hazard is real - `1.234` is one thousand two hundred and thirty-four in Berlin and slightly more than one in London - and the answer is **not** to read the reader's browser. A runtime locale read makes two readers see different bytes, which breaks the byte-identity oracle, and it is behaviour that varies by reader, which Guardrail #1 refuses. The locale is a config value.
5. **Precision is declared, not inherited.** Significant digits come from `config/`, so the choice between `2M` and `2.4M` is a decision rather than an accident of the input.
6. **A shortened number is still a drawn label.** It faces the legibility floor unchanged. Making a number shorter is not a licence to set it smaller.
7. **One formatter, one module, both surfaces.** The compiler and the console call the same code. Two formatters is how `2M` and `2,000,000` end up on one page.

#### What it costs

Less than it reads. `d3-format` and `d3-time-format` are already named in section 7.2, and `d3-format`'s SI notation does rules 1 and 5 with no code of ours. What is genuinely ours is the unit table for rule 3, its version stamp, and the two provenance columns.

#### Config

`visuals.number_format` gains `locale`, `si_prefix_from` (the magnitude at which a prefix replaces digits) and `significant_digits`; `visuals.unit_table_version` stamps the conversion table. Section 15.4's rule binds - the `visuals` block belongs to exactly one plan-doc.

#### Where this has to land in `docs/`

The formatting half is a published-surface rule and belongs in [`../docs/concepts/design-system.md`](../docs/concepts/design-system.md), beside the other things a drawn label must satisfy. The conversion half is a contract rule and belongs with the Derived Value contract. **Neither may live only here**, because this document dies at distill.
...
At 20 items the base work roughly halves - median about 39 minutes, worst about 68 - which leaves the margin the second call spends. The margin is real but it is not luxurious: sizing from the worst case rather than the median is the whole of Guardrail #2 here, and the worst case is 135.4 minutes, not 70.
...
|---|---|---|
| **Config-driven** | Every window, fuse and cutoff is a knob in `config/idhazh.json` behind a Pydantic model. No literal dates, no literal counts | Guardrail #6 |
...
flowchart TD
  ...
  C1["<b>CALL 1</b> - model reads the same article<br/><br/>CANNOT EMIT a span, a character offset, a value,<br/>a unit or a count. No field of the schema accepts<br/>one, so the grammar refuses it at decode time and<br/>nothing downstream has to check for it  (Guardrail #11)"]
...
| Coreference chains | Within-article entity merging | ~40 | 3.6 min | **NO.** Exact and prefix matching over spans code already holds does most of it, and the model's version cannot be span-validated (Guardrail #8) |
...
1. **Any llama-server setting quoted anywhere carries the model entry it was derived against, the runner and the date.** That is Guardrail #10 applied to a setting rather than to a throughput figure. A bare `n_ctx: 16384` in a plan-doc is an unmeasured number justifying a design.
...
It buys nothing. The largest prompt ever seen is 5,516 tokens and `extract.truncation_cap_tokens` is 5,000, so a 32K window is more than five times what this pipeline can put into it - and it would spend 1 GiB of a 1.61 GiB margin doing so. Guardrail #2: the budget is the platform, not a preference.
...
| L2 | **AMEND, disambiguated 2026-09-03.** The vocabulary **lives in `config/`** rather than in a Python literal (Guardrail #6). It is still the **full** vocabulary - config locates the list, it does not shorten it. `wasted_decode_rate` reported | Carmack, Fowler | row 48, section 12.8 X1 |
...
| R12 | **NOT closed. Reopened 2026-09-04 - see 12.14 G40.** Row 55 gives the store a CSV, and R12 asks for the full input tuple: plan, element ids, all versions. A CSV cell cannot hold a Visual Plan, so row 35's whole-day re-render, P.6.4.2's corpus pass and P.6.4.3's blast-radius query all have nothing to read |
...
| 6.1 Telemetry event | Ships as the canonical schema; `state/visuals/` is its store (row 55). **Two of its fields did not land**: `visual_id`, which is the only join between a label and a telemetry row, and the full input tuple R12 requires - 12.14 G41 and G40 |
| 6.2 Console | Ships, plus every existing eval metric not yet on the console (row 70). **The never-blend rule that constrains one of its panels was carried as a field and not as a rule** - 12.12 G23 - and `visual_repetition_rate` is absent, so R18 has a rule and no instrument (12.14 G43) |
| 6.3 Feedback without a server | Ships, amended by O12. **Row 68 reversed the review surface and three of P.6.3.1's mechanisms were written for the old one** - 12.14 G44. `select_exemplars.py` has two consumers here and no producer - 12.14 G42 |
| 6.4 Versioning and migration | Ships; **versions are date-stamps, not integers** (P.L15). **P.6.4.2 is missing steps 1 and 2 with no note saying whether that is a judgement** - 12.14 G45 - and P.6.4.3's blast-radius query has no store to run against (12.14 G40) |
...
| # | Defect | Intent it breaks | Fix | Owner |
|---|---|---|---|---|
| G12 | **`state/visuals/` folds to "one row per group" and nothing says what the group is.** Section 9.2 sets the policy and names no key; 9.3 gave the store `ledger fold` and a window knob that no longer exists | G7 made this store one row per **attempt** and G1 added `none_reason` so the majority outcome has a cause breakdown. A fold keyed on date alone deletes both at the window edge: the full-grain shard is gone and the aggregate never carried the cause. **This is the half that decides whether G1 was worth building** | Declare the key in the schema, the way `state/telemetry-aggregate/` already declares "one `(date, stage)` pair" in its own description. `(date, decision, none_reason, rejection_reason)` is the **minimum** that survives the fold with G1 intact, and **12.13 G37 adds `potential_primary`, `family` and an element-count band to it, in the same commit** - the fold is irreversible, so a key settled without the stratum terms cannot be widened later. Ruled together with the named window 9.2's correction already owes this store, and before the row that writes it lands | **K**, with **E** |
...
| # | Gap | Why it is load-bearing | Owner |
|---|---|---|---|
| G29 | **"Convertible or identical" lost a word, and the word was the check.** P.4.1.2 is deliberately precise: units must be "convertible or identical - **not merely equal strings**". Every restatement here drops it - section 1a's validator box says "units compatible", section 14.5 gate 3 says "units are compatible" | Convertibility is not a validator refinement, it is **a new class of displayed value**. Drawing `4,200 tonnes` beside `4.2 kilotonnes` on one axis renders a number the article never wrote, which lands inside P.2.3's Derived Value contract - **whose function list is a closed allow-list of `count`, `sum` and `share_of_declared_whole`, adopted verbatim by row 45. `convert` is not on it.** Section 10.6 refuses the *model* doing unit normalisation and says "code does arithmetic", which reads as settled and answers **who**, never **whether**. Two consistent positions exist and this document holds neither: units must be identical, and `tonnes` beside `kilotonnes` is a rejection (cheapest, and consistent with row 8's prefer-rejection rule); or `convert` becomes a fourth derived function with a versioned unit table, a provenance chain, and `derived_value_rate` counting it. **CLOSED 2026-09-05 by O47**, which takes the second position and widens it past convertibility: `convert` is the fourth allow-list function, and the same ruling settles formatting - SI prefixes, one declared locale applied at build time, and declared precision. Section 7.5 | **H**, ruled with G21 |
...
| G31 | **Two of the compiler's three accessibility duties are unassigned, and the architecture creates the question the proposal warns about.** P.4.3.2 gives the compiler contrast, **focus order** and **screen-reader structure**. Row 28 covers contrast properly. The other two are zero hits. P.4.4.4's fifth question is the pointed one: does accessibility live baked into the static output or re-derived client-side - "splitting it across both is how it silently regresses" | Progressive enhancement (row 22) **is** that split. Rows 31 and 33 push toward baked without ruling on it: row 31 is about pixels, row 33 is a design constraint | **CLOSED 2026-09-04, on a test doc I already owes.** Accessibility is **baked**, and 12.10's byte-for-byte hydration comparison is what enforces it: if the post-hydrate DOM equals the build-time emit byte for byte, focus order and the accessible tree match **by construction**, because both are functions of that DOM. P.4.4.4's fifth question is therefore answered by an obligation that already exists rather than a new one, and the two unassigned duties become properties of a test rather than duties somebody can forget. **What I owes on top is one sentence, not a workstream**: the hydration test's failure message names accessibility among the things it guards, or the next person to relax that test will not know what they are relaxing | **I**, closed |
...
| G37 | **The four comparability rules are absent, and G12 needs them in the room.** P.5.2.4: report percentile rank within the corpus rather than raw score; stratify by potential class, family and element-count band, then z-score within stratum; publish a rolling 28-day median as the headline; track the distribution rather than the mean, because "a bimodal distribution is the interesting finding and a mean hides it" | 12.9 G12 is about to settle a fold key for `state/visuals/`, so the cause breakdown survives the window edge. **P.5.2.4 says the quality scores need stratum and shape preserved too**, and a fold is irreversible - 9.2 deletes the full-grain shard - so a key that drops those terms cannot be widened later against data that no longer exists | **K, and this has a deadline: it is ruled in the same commit as G12, or not at all.** Concretely: G12's key gains `potential_primary`, `family` and an element-count band, and the row carries a distribution rather than a mean - at minimum a count, a median and the two outer quartiles, because a mean cannot show the bimodality P.5.2.4 calls the interesting finding |
...
### 12.14 Six more, from the sixth review

Sections 12.6 to 12.13 walked the diagram, the tail, the element record, the plan contract and the measurement chapter. This pass, run 2026-09-04, walks **section 6 of the proposal** - the telemetry event, the console, the feedback surface and the migration plan. **Two of the six are risks this document marked closed against the wrong thing**, which is a worse failure than an omission: a closed row stops anyone looking.

| # | Gap | Why it is load-bearing | Owner |
|---|---|---|---|
| G40 | **R12 is marked closed by a store that cannot hold what R12 asks for.** 12.3 reads "R12 - Closed by row 55", and row 55 is `state/visuals/<YYYY-MM>.csv`. R12 requires that **every published visual stores its full input tuple - plan, element ids, all versions**. A CSV cell cannot hold a Visual Plan. `compiler_wave`, the field P.6.4.3's blast-radius query is written in terms of, is zero hits | **G10 found this exact hole and fixed the other half of it.** G10 ruled that a rejected plan's body "does not fit a CSV cell" and gave it a home - a short rolling window, lookup semantics, a 9.3 register row - and scoped itself to rejects. The published plan needs the identical treatment, and three things already depend on it: **row 35** requires a renderer bump to re-render whole days or none, and from a CSV row nothing can be re-rendered; **P.6.4.2 step 4** is the corpus re-render pass; **P.6.4.3** is the blast-radius query verbatim, "every visual rendered by `compiler_wave` 2 using `pie`". **A second-order gap surfaces with it**: 9.3 claims to name every artefact this project writes and registers **no element store**, while 15.2 says doc G delivers "a queryable fact table over every article". If elements live in the day payload - registered, and never deleted - that answers it, and nothing says so. **Re-rendering needs the elements as much as it needs the plan** | **K**, with **G** and **E**, ruled alongside G10 |
| G41 | **`visual_id` is zero hits, and it is the join key.** P.6.3.8's whole agreement mechanism rests on it: "overlap is manufactured by issuing the same `visual_id` to two reviewers across a defined fraction of the queue". P.5.6's claim that the human loop eventually validates the machine metrics themselves needs labels to join to telemetry rows | **12.4 Q5 already commits to the consequence** - "`inter_reviewer_agreement` is published beside every keep rate or the keep rate is not quotable" - while the field that makes agreement computable is unnamed. Without it there is no overlap to compute agreement from, so by Q5's own rule no keep rate is quotable, and the human loop's output cannot be joined to anything the machine loop wrote. One field on the G7 ledger row | **K** |
| G42 | **`select_exemplars.py` has two committed consumers here and no producer.** P.6.3.3's diagram carries three fitters. G22 recovered `fit_priors.py` and G34 found `fit_weights.py` shipped as an intention. `select_exemplars.py` producing `exemplars.json` is the third, and it is zero hits - **while this document has already twice committed to using its output** | Section 10.5's fourth fix is "one worked good-or-bad pair in the system prompt - about 30 tokens, prefix-cached across the shard, so free after the first item". That pair **is** an exemplar, and nothing says who picks it. Section 16.2's prompt loop runs against the committed canary corpus and O28 commits the winning prompt, the rubric, every candidate score and the seed - exemplar selection is the missing input to that. **And an exemplar carries G22's static-within-a-shard constraint for the identical reason**: rendered into the system prompt once it is prefix-cached and free; selected per item it changes the prompt per item and destroys the KV reuse | **H**, with **F** |
| G43 | **R18 is closed by a rule about a different axis.** R18 is **cross-day**: a running story yields the same visual daily and "the reader sees the same visual five days running", mitigated by fingerprinting the `(element set, type)` pair across a rolling window and reporting `visual_repetition_rate`. Row 49 is **within-day**: a day publishing a single rendered type is a recorded defect. Both are real and they are orthogonal | 12.3 pairs row 49 with Editor's rule - "a running story's visual repeats only when its numbers moved" - which **is** the right axis, and it is a rule with no instrument. `visual_repetition_rate` is zero hits and so is the fingerprint. Without a fingerprint over a rolling window, "only when its numbers moved" is unenforceable, because **nothing computes whether they moved** | **K** |
| G44 | **The review surface was reversed and three of its mechanisms were not followed through.** Row 68 and 9.3 tightened `review/` from shipped-with-the-site to a build artifact - never committed, never under `frontend/public/` - and 12.7 records it as a deliberate tightening. It is the right call: the proposal's only reason for shipping it was "the cheapest distribution" | Three of P.6.3.1's mechanisms were written for the shipped model and none has been restated. **`noindex`, unlinked, excluded from sitemaps and feeds** is now moot - there is nothing to exclude - and saying so is what stops a plan-doc implementing it. **Transport** is unchanged in intent but the reviewer now has to obtain the artifact first, and nothing says how: artifact download, local build, or a manual dispatch. **Ingestion** - a scheduled job validating and appending to the append-only ledger - still needs a source, and the source moved | **L**, with one line in 15.4a |
| G45 | **P.6.4.2's migration is missing two of its five steps, with no note saying why.** Step 1 is freeze-baseline and step 2 is dual-run, old and new pipelines over the same set, compared per potential class | Section 10.3 refuses comparability for **summaries** outright and states the trade - "the current summaries are unsatisfactory and comparability with them is not worth protecting" - which is a fair judgement, honestly made. It is silent on everything else, and **the honest answer is probably that there is nothing to baseline**: no visual metric exists today, so a freeze-baseline over visuals would freeze an empty set. Say that. **A migration section missing two of five steps with no note reads as an oversight rather than a judgement**, and the next reader spends the time working out which it was | **H** |
...
Guardrail #10: an unmeasured number may not justify a design. Each row blocks something named.
...
1. **Actions minutes are free and unmetered on a public repository** (Guardrail #2 states it outright), so a longer ceiling costs no money.
...
| T2 | The **file sink stays the only sink CI runs** | No host, no key, no third party. Guardrail #1 untouched. Langfuse stays available to a developer who names a host |
...
Widening the projection and republishing every existing month from `state/` **backfills the console for every past run, in one commit, with no spans, no package and no new stage.** Every added cell is an integer, so `FORBIDDEN_COLUMNS` and Guardrail #11 do not move.
...
| Frozen set | The committed canary corpus, so the comparison is reproducible and touches no network (Guardrail #7) |
...
Two beyond the settings table in 11.2a. Both are **candidates, not rulings** - neither has been verified against the pinned build b10598, and `server_argv` is the only place a flag may be spelled (Guardrail #6), so each needs a config field before it can be tried.
