...
**20 of 29 rows are on `main`** (PRs 989, 990, 992, 993, 994, 995, 996, 997, 999, 1000, 1004, 1007, 1011, 1012, 1013, 1014, 1015). Nine are left: 13, 14, 15b, 19, 20, 21a, 21b, 22, 23. No pull request is open and no worktree is held.

The council can now be built, tested and run with no judge in the repository: it has its own package, its own knobs, its own run identity, its own verbs over a config-declared tenant list, and its own shard-outcome record. The content-similarity judge's stores now sit under its own slug. What is left is the judge side's stamps, the night plan, and the closing checks.

**Row #18 landed 2026-09-21 (PR 1015).** Two things it turned up are worth carrying: the council workflow no longer spells any store path, because row #24 replaced that list with what a tenant hands back - so a later row that moves a store does not touch the workflow. And the row's oracle is the pattern to copy for row #13's: bounded to the six trees a person edits through `git ls-files`, anchored so it does not match the published contract keys, and written with its own search string in a character class so the test file does not find itself.
...
| Row #24's report | `idhazh.ledger` pulls three of the content-similarity judge's contracts into `idhazh.council.tenancy`'s closure. Not a judge's code, but a judge's published shape inside the venue's closure. Row #18 moved those stores and did not cut the import; it stays open. **Measured 2026-09-21: it is three contracts by two routes.** `council.tenancy -> idhazh.ledger` reaches `story_similarity_pair` and `fitted_similarity_threshold`; `council.registry -> contracts.app_config -> contracts.knobs.placement` reaches `story_similarity_distribution`. No judge *code* is in the closure. **Row #23 froze that list in a check that fails when it grows.** Cutting the two routes is the follow-up: it refactors `ledger.py` and `knobs/placement.py`, which every stage and the digest pipeline read. |
| Row #22's report | **Row #13's scrub replaced `leg` with `unit`, which is a second name for `shard`.** It survives in about 13 test names and forty comments across `backend/tests/workflows/test_llm_council_workflow.py` and `.github/workflows/llm-council.yml` - `test_one_server_per_unit`, `Run this unit of the tenant's work`. Row #22 renamed the guard only. Section 0b says a borrowed second name is deleted rather than replaced, so this is a scrub somebody still owes. |
| Row #15b's report | `STORES_NOTHING_FILLS_YET` in `test_ledger_staging.py` still excuses `state/content-similarity-judge/metrics`, which now has a writer. The test derives writers from `ledger.append_*` and `ledger.write_*` only, and this store is filled through the council's shipping capability - so removing the entry turns the test red for the opposite reason. The reason string was corrected rather than left reading as a lie. The structural fix is for the derivation to follow `ledger.extend_ledger_file` callers too. |
| Row #14's report | `judged_by_run_id` on the pair row is still empty and this row could not fill it: `stage_judge_item_pairs` takes no `run_id`. Row #15b gives every member that does work a `run_id`, so it is the row that fills the cell. |
| Row #14's report | `ShardReport` carries `owned`, `judged` and `usable` but not `unreadable` or `refused`, so the judge's metrics identity - read plus refused plus unreadable plus abandoned equals dealt - cannot be built from it. Nothing builds that row yet. Row #15b owns both files and widens the report when it does. |
| Row #14's report | The row's own oracle asked for an unreachable world. It said "a pair whose first call failed asserts the call count is 1, not 2", and decision 4 says both calls always run - so that count can never read 1. The row now asks for a pair behind a reasoning span reading 4, which proves the same property. **An oracle describing a world the design forbids is a test that can only be made to pass by breaking the design.** |
| Row #13, measured before dispatch | The telemetry compaction verb is also called a fold - `telemetry/rollup.py`, `retention.py`, `prune_state.py`, the digest workflow's own step and their tests, about 65 files. Row #13 leaves it, because it is the digest pipeline's word and no row of this plan names it. Row #13 decision 7 puts the plain-meaning test into `CLAUDE.md` section 0b, which is what makes this a question somebody now has to answer: either the telemetry fold earns its word or a later plan deletes it too. |
...
| 15b | The content-similarity judge fills its metrics and registers as a tenant | 6, 14, 15a | L | DONE | - | 1025 | worker |
...
| 20 | The plan pointer | 19, 21b, 23 | V | DONE | - | - | owner |
...
- **Files touched:** every tracked file carrying `leg` as a word - **counted by the row itself, with the exact pattern its oracle will use, as the first step**; `backend/idhazh/similarity/fold.py` -> `counting.py`; `backend/idhazh/similarity/draw.py` -> `selection.py`; `backend/idhazh/similarity/judge.py`, `stamps.py`; `backend/idhazh/contracts/story_similarity_distribution.py` (the field), `fitted_similarity_threshold.py`, `story_similarity_pair.py`, `knobs/placement.py`, `knobs/council.py` (field descriptions); `backend/idhazh/stages/set_merge_line.py`, `count_verdicts.py`, `judge_item_pairs.py`, `pick_item_pairs.py`, `common.py`; `backend/idhazh/cli.py`; `state/content-similarity-judge/score-distribution.json` and its archived sibling; `tests/fixtures/contracts/story-similarity-distribution/a-narrow-band-with-one-day-folded.json` (**the fixture's own filename carries the word**); `schemas/` (generated); `frontend/src/contracts/` (generated); `frontend/src/lib/console/merge-line.ts`; `frontend/src/lib/server/similarity-ledger.ts`; `frontend/src/routes/console/judgement/`; `CLAUDE.md` section 0b; `docs/architecture/publishing/llm-council.md`
- **Corrected 2026-09-21, before dispatch:** the draft named `backend/idhazh/stages/judge_fit.py`, which row #12 renamed to `set_merge_line.py`. It named `state/story-similarity/score-distribution.json`, which row #18 moved under the judge slug. Both are written above as they stand.
...
| 2a | **Which `fold` - measured 2026-09-21, before dispatch.** The word appears in two subsystems. This row deletes it where it names the **judging** council's collecting job: `similarity/fold.py`, `folded_dates`, `daysFolded`, `test_similarity_fold.py` and the prose around them. It leaves the telemetry compaction verb alone - `telemetry/rollup.py`, `retention.py`, `prune_state.py`, the digest workflow's own fold step and their tests. That is a different subsystem with its own word, no row of this plan names it, and section 1a's table was written about the shards of the judging work. Taking both would turn a 12-file scrub into a 77-file one across the digest pipeline. **The telemetry `fold` is recorded as a follow-up in section 0e rather than dropped.** | Owner, 2026-09-21 |
...
| 4a | **Where `leg` names something that is not a judging shard, it stays.** Measured 2026-09-21: `visual_validator.py` calls a two-part rule's first clause a leg, which is decision 4 above; `.github/workflows/measure.yml` calls one arm of a measurement gate a leg. Neither names a shard, so decision 1's rule does not reach them. Deleting a word wherever it appears is how a scrub turns into churn. | Fowler |
...
| 6 | **The two reply types gain the counts the council's record has no other producer for - round seven found the gap and it was round six's defect repeated one level down.** `Reading` carries `prompt_tokens` and no completion count; `Judged` carries neither and collapses `decode_seconds` to the pair's sum, so the judging stage never sees a per-call figure. `Reading` gains `completion_tokens`; `Judged` gains `prompt_tokens`, `completion_tokens`, `calls` and `decode_seconds_max`. **Three of `ShardResult`'s four cost fields and the judge's own `decode_seconds_max` all die without this**, and until round seven no row named the file. | Carmack |
| 6a | **Where those three things are, checked 2026-09-21 before dispatch.** The draft cited `judge.py:47`, `judge.py:56` and `judge_shard.py:87`. Row #12 renamed the stage, so `judge_shard.py` no longer exists - it is `backend/idhazh/stages/judge_item_pairs.py`, and the line that collapses the figure is its `"decode_seconds": verdicts.decode_seconds`. `Reading` and `Judged` are both in `backend/idhazh/similarity/judge.py`. **Find them by name, not by line number** - a line number in a plan rots the moment anything above it moves. | Owner, 2026-09-21 |
...
- **Files touched:** `backend/idhazh/similarity/tenant.py` (new), `backend/idhazh/stages/judge_item_pairs.py`, `backend/idhazh/stages/count_verdicts.py`, `config/idhazh.json`, `tests/fixtures/contracts/app-config/every-knob-differs-from-the-committed-config.json`, `backend/tests/test_similarity_judge.py`, `backend/tests/test_similarity_counting.py`
- **Corrected 2026-09-21, before dispatch:** the draft named `backend/tests/test_similarity_fold.py`; row #13 renamed it to `test_similarity_counting.py`. This row also inherits two things row #14 could not do: fill `judged_by_run_id` on the pair row, which needs the `run_id` this row gives every member that does work, and widen `ShardReport` with the unreadable and refused counts, without which the funnel identity in the oracle cannot be built.
...
- **Scope:** the committed same-story tree moves under the judge slug that produced it, and every reader, workflow path and document naming the old tree moves with it. **The store path moves; no contract does** - `story_similarity_pair.py`, `StorySimilarityPair`, `story-similarity-pair.schema.json` and the generated frontend contract all keep their names. They are published keys, and a key is renamed by the row that has a reason to rename it.
- **Files touched:** `backend/idhazh/ledger.py`, `backend/idhazh/telemetry/prune.py` (the prune ledger words are joined from the constants), `backend/utilities/sample_sheet.py`, `backend/utilities/widen_ledger_header.py`, `state/story-similarity/` -> `state/content-similarity-judge/`, `.gitattributes`, `frontend/src/lib/server/similarity-holdout.ts`, `frontend/src/lib/server/similarity-ledger.ts`, `docs/reference/repository-layout.md`, `docs/architecture/publishing/autotune-content-similarity.md`, `docs/architecture/contracts/schemas.md`, `docs/concepts/partitions.md`, `docs/concepts/growing-reads.md`, `docs/architecture/publishing/retention.md`, `docs/how-to/label-the-similarity-holdout.md`, `docs/reference/benchmarks/what-a-judge-pair-costs.md`, `TODO/20260920-a-second-judge-in-the-council-handover.md`, `backend/tests/test_ledger.py`, `backend/tests/retention/test_prune_range.py`, `backend/tests/workflows/test_ledger_staging.py`, `backend/tests/test_widen_ledger_header.py`
- **Not touched, checked 2026-09-21:** `.github/workflows/llm-council.yml` and `backend/tests/workflows/test_llm_council_workflow.py` - see decision 3. `frontend/src/routes/console/judgement/+page.server.ts` reaches the store through the two server libraries and names no path. `TODO/20260918-35-search-eval-key-points-plan.md` and `TODO/20260921-39-delete-the-scaffolding-plan.md` each name the old tree once; they belong to other plans and are left, so an agent running one of them finds a path that moved.
- **Acceptance gates:** local - the ledger and similarity test modules, the retention and staging tests, the frontend build, `doc_load.py --changed`. CI - full suite, plus the published-site smoke.
...
| 3 | **Superseded on 2026-09-21, after row #24 landed.** The draft said the council workflow names the old paths in its commit step. It no longer does: the commit step stages `needs.draw.outputs.committed_paths`, which a tenant returns, so the workflow spells no store path at all. The list a tenant returns is built from the constants decision 4 moves, so the move still reaches the workflow - through the tenant, not through the file. | Carmack |
| 4 | **`STORY_SIMILARITY_DIRNAME` is deleted**, not left holding the new string. `CONTENT_SIMILARITY_JUDGE_DIRNAME` already exists in `ledger.py`, filled by rows #6 and #7, so the five path helpers point at it. Two constants carrying one value re-opens the drift the closed sets exist to close, and the prune vocabulary is built by joining those constants - so the prune words move with them. | Fowler |
...
| 6a | **Most of the date axis already exists - measured 2026-09-21, before dispatch.** Row #24 built the plumbing: `backend/utilities/council_matrix.py` takes `--date` repeatably, `cells()` loops the dates it is given, and `_parallel` caps the product at the 20-job ceiling. What is missing is the thing that decides WHICH dates: the planning step passes one date it worked out itself. **This row supplies the list, it does not rebuild the matrix.** Decision 8 is in the same state - the artifact names already carry tenant, date and shard, and the collecting job already downloads on a pattern that spans them. Check both before writing either. | Owner, 2026-09-21 |
...
- **Files touched:** `backend/idhazh/similarity/tenant.py`, `backend/idhazh/contracts/story_similarity_distribution.py`, `backend/idhazh/similarity/counting.py`, `backend/idhazh/stages/count_verdicts.py`, `backend/tests/test_similarity_counting.py`, `backend/tests/contracts/test_story_similarity.py`, `schemas/story-similarity-distribution.schema.json` (generated)
- **Corrected 2026-09-21, before dispatch:** the draft named `similarity/fold.py` and `test_similarity_fold.py`; row #13 renamed them to `counting.py` and `test_similarity_counting.py`. **This row shares three files with row #15b, so it runs after it** - `tenant.py`, `count_verdicts.py` and `test_similarity_counting.py`. Row #15b creates `tenant.py`; this row extends it.
...
- **Oracle:** three arms, and the second is the one round six had to add.
  ...
  - **The closure arm** walks the **whole static import closure of the council's entry point** - `cli.py` and `AppConfig` included - and fails on a judge module reached at import time from any council verb. A directory arm alone would never have seen either of the two real couplings: `cli.py` imported four judge stages at module scope, and `app_config.py` imported the judge's knob module, so every council module that read config carried a judge in its closure. **Both of those are now cut** (rows #24 and #25).
  - **What the closure still holds, and what the arm does about it. Owner ruling, 2026-09-21.** Walking the graph on the day says three judge *contracts* remain reachable, by two routes: `council.tenancy -> idhazh.ledger` reaches `story_similarity_pair` and `fitted_similarity_threshold`, and `council.registry -> contracts.app_config -> contracts.knobs.placement` reaches `story_similarity_distribution`. No judge code is in the closure - these are published shapes pulled in by two modules both sides share. **The arm fails on any judge module in the closure except exactly those three, and a second assertion fails when that list grows.** Cutting the two routes is a refactor of `ledger.py` and `knobs/placement.py`, which every stage and the digest pipeline read; it is a follow-up row, priced and not taken here. The frozen list is what makes the exception self-limiting: a doc note does not stop a fourth contract arriving, and this does. **Take the list from a fresh walk on the day rather than from this paragraph** - if it has already changed, that is the finding.
