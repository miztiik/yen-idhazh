...
Five of the original 37 are gone, and that is the other half of the question.
A re-export nobody outside the package ever reached is a second name for a thing
only the package uses, so plan 32 row 10 cut those five and this file is where
the cut is recorded.

**What this cannot settle: whether the seam is in the right place.** It would
pass just as green with all 32 names left in one file, and it would pass with
...
#: What the package re-exports today: the pre-split surface, less the five.
RE_EXPORTED: Final[frozenset[str]] = BEFORE_THE_SPLIT - CUT_BY_THE_DISPATCHER_ROW
...
def test_the_flat_module_left_no_shim_behind() -> None:
    ...
    The ten publisher modules row 10 moved in owe the same, and a `publish_*.py`
    left at the top of `idhazh/` would read as the live one.
    """
