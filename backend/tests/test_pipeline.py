...
def isolate_ledgers(tmp_path: Path, monkeypatch: MonkeyPatch) -> None:
    ...
    The month index needs no line of its own: `cli._index_root()` derives it from
    `PUBLIC_ROOT`, so redirecting the day directory carries it. It used to be a
    constant, and left unpatched it rebuilt the committed index from an empty
    fixture tree and truncated the served vectors to zero bytes - a change
    `git status` shows and a test never asserts on.
    """
    monkeypatch.setattr(cli, "VAR_ROOT", tmp_path / "run")
