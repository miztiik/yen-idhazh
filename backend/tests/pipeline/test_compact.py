...
def test_two_dates_of_one_month_fold_into_one_head_carrying_both(tmp_path: Path) -> None:
    ...
    for shard in (0, 1):
        _write(
            ...
            _rollup(yesterday, overnight, shard=shard, span=RollupSpan.ITEM, total_ms=90 + shard),
        )
    _write(
        state,
        ledger.SegmentLedger.SPAN_ROLLUP,
...
def test_a_fold_left_over_a_month_boundary_reaches_the_month_its_own_rows_name(
    ...
    _write(
        ...
        _rollup(last_of_august, before_midnight, shard=0, span=RollupSpan.ITEM, total_ms=61),
    )
    _write(
        state,
        ledger.SegmentLedger.SPAN_ROLLUP,
        f"{after_midnight}-1-{ServerJob.WORK.value}-00.csv",
        _rollup(first_of_september, after_midnight, shard=0, span=RollupSpan.ITEM, total_ms=59),
    )
