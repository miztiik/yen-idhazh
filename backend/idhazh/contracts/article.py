...
class Article(Contract):
    ...
    __changelog__: ClassVar[tuple[ChangelogEntry, ...]] = (
        ...
        ChangelogEntry(
            ...
            why=(
                "The entry below said both titles pass the same cleaner, so the field "
                "is provenance and never a level of trust. The system already "
                "disagreed: the feed is read first BECAUSE it is the less "
                "attacker-controlled string, and order is a control. The two paths now "
                "carry different bounds as well - a page headline past 200 characters "
                "is refused rather than cut - so feed and page are not interchangeable "
                "origins. No field moved and no payload needs migrating; the two "
                "descriptions move, so the generated bytes move, and the reason is "
                "stamped here rather than left to the drift gate to announce "
                "(section 11)."
            ),
        ),
        ChangelogEntry(
