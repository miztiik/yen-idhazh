...
class DerivedValueError(ValueError):
    """The plan cannot be turned into figures, and the message says which part.

    A `ValueError` so a caller written before this class still catches it, and
    its own type so a caller can tell it from a shape refusal. It is raised
    rather than returned because every case it covers is one the validator
    refuses first: reaching here means the caller drew a plan it never checked,
    which is this process's own arithmetic being wrong rather than one article's
    problem. The read-side and write-side split `ElementTable.span_drift` draws
    is the same split, and this is the write side of it.
    """
...
def _quantity(element: Element) -> Decimal:
    ...
    if element.value is None:
        raise DerivedValueError(f"{element.element_id} states no value and cannot be drawn")
    try:
        return Decimal(element.value)
    except InvalidOperation as unreadable:  # pragma: no cover - the pattern cannot produce one
        raise DerivedValueError(f"{element.element_id} does not read as a number") from unreadable
...
def count(inputs: Sequence[Element], *, lower: Decimal, upper: Decimal) -> DerivedValue:
    ...
    """
    if not inputs:
        raise DerivedValueError("a count with no elements counted has no chain")
    return DerivedValue(
...
def sum_of(inputs: Sequence[Element]) -> DerivedValue:
    ...
    """
    if len(inputs) < 2:
        raise DerivedValueError("a total of one element is that element")
    units = {element.unit for element in inputs}
    if len(units) != 1:
        stated = sorted(unit or "(no unit)" for unit in units)
        raise DerivedValueError(f"a total needs one unit and these state {stated}")
    return DerivedValue(
...
def share_of_declared_whole(part: Element, whole: Sequence[Element]) -> DerivedValue:
    ...
    """
    if len(whole) < 2:
        raise DerivedValueError("a whole of one part is that part, and its share is all of it")
    units = {element.unit for element in whole} | {part.unit}
    if len(units) != 1:
        stated = sorted(unit or "(no unit)" for unit in units)
        raise DerivedValueError(f"a share needs one unit and these state {stated}")
    with localcontext() as context:
        ...
        try:
            ...
            raise DerivedValueError("a whole of nothing has no parts to take a share of") from empty
    return DerivedValue(
...
def convert(element: Element, *, target: str) -> DerivedValue:
    ...
    **A ratio that will not state exactly is refused rather than rounded.** The
    ...
    source = element.unit
    if source is None:
        raise DerivedValueError(f"{element.element_id} states no unit, so there is nothing to move")
    if not commensurable(source, target):
        raise DerivedValueError(f"{source} and {target} do not measure the same thing")
    if source == target:
        raise DerivedValueError(f"{source} and {target} are one unit, and moving it moves nothing")
...
def bin_edges(values: Sequence[Decimal], *, bins: int) -> list[Decimal]:
    ...
    """
    if bins < 2:
        raise DerivedValueError(f"{bins} bins is not a distribution")
    if not values:
        raise DerivedValueError("a histogram of nothing has no range to bin")
    low, high = min(values), max(values)
    if low == high:
        raise DerivedValueError("these values have no spread, so there is no distribution to draw")
    with localcontext() as context:
        context.prec = _SHARE_PRECISION
        width = (high - low) / bins
...
def _bars(elements: Sequence[Element], *, bins: int) -> list[DisplayedValue]:
    ...
    **A bin nothing falls into refuses the drawing.** Its chain would name no
    ...
    for step in range(bins):
        ...
        if not inside:
            raise DerivedValueError(
                f"nothing falls between {_plain(low)} and {_plain(high)}, so {bins} bins is "
                f"more than these {len(elements)} values have spread to fill"
            )
        drawn.append(
            DisplayedValue(
...
def resolve_displayed_values(
    ...
    """Every figure this plan will draw, each one resolved to an element or to a chain.

    This is what the row's oracle walks. A plan that declines draws nothing and
    resolves to nothing, which is a fact rather than a failure.

    **It runs on a plan the validator has already passed.** Every refusal below is
    one of the nine checks restated, and reaching it means a caller drew a plan it
    never checked - so it raises rather than degrades, for the reason
    `DerivedValueError` gives.
    """
    ...
    if missing:
        raise DerivedValueError(f"{missing[0]} is not in the element table, so nothing draws it")
