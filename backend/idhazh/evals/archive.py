...
def read_rows(day: Path) -> list[dict[str, str]]:
    """One day of the ledger, as the CSV spells it."""
...
def _cohort(rows: Sequence[Mapping[str, str]]) -> ScoreCohort:
    """One group's numbers, computed from the rows the month held for it."""
    first = rows[0]
    deciles = [0] * DECILES
...
def reconcile(
    stored: ScoreArchive, days: Sequence[Path], *, month: str, observation_key: Sequence[str]
) -> None:
    """Prove the file that was read back describes the days, or raise saying which part does not.
    ...
    """
    fresh = summarise(days, month=month, observation_key=observation_key)
    where = archive_relpath(month)
    for name in ("month", "source_rows", "source_sha256"):
        _same(getattr(stored, name), getattr(fresh, name), where, f"the month's {name}")
    _same(
    ...
    for mine, theirs in zip(stored.cohorts, fresh.cohorts, strict=True):
        cohort = f"cohort {'/'.join(theirs.key)}"
        _same(mine.rows, theirs.rows, where, f"{cohort}: the row count")
        _same(mine.hhem_deciles, theirs.hhem_deciles, where, f"{cohort}: the deciles")
        _same(mine.bands, theirs.bands, where, f"{cohort}: the bands")
        _same(mine.signals, theirs.signals, where, f"{cohort}: the signal counts")
        _same(
        ...
        _same(
            ...
            where,
            f"{cohort}: the premise digests",
        )
        _same(mine.measurements, theirs.measurements, where, f"{cohort}: the moments")


def _same(stored: object, fresh: object, where: str, what: str) -> None:
    if stored != fresh:
        raise ValueError(
            f"{where} does not reconcile with the day files beside it: "
            f"{what} reads back as {stored!r} and recomputes as {fresh!r}, so they stay"
        )
