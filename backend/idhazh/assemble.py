...
def build_day(
    *,
    ...
    The leading block is chosen here rather than by a caller because it is
    decided over the finished day: a later run adds stories the first one could
    not weigh, so the block is recomputed on what the day now holds. It moves
    nothing - `items` is still the published order, appended to and never
    re-sorted.
    """
