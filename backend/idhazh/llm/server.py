...
#: What a decode stamp leaves out. Everything else a caller posts is in it, so a
#: key added to a request body next year is stamped without anybody remembering
#: to come back here - which is the property a fixed list of stamped keys does
#: not have, and the reason this list is the short one. It is public because it
#: is the whole of the contract: the stamp is defined by what it excludes.
UNSTAMPED_REQUEST_KEYS: Final[frozenset[str]] = frozenset(
    {
        # The item's own bytes, which `prompt_sha256` already carries. In here
        # every item would carry a different decode stamp, and a stamp that
        # never repeats says nothing about how two items were decoded.
        "prompt",
        # Generated by the caller from a vocabulary the caller already stamps.
        # In here, respelling one literal would move the stamp although the
        # sampler was asked for nothing different.
        "grammar",
        # Which weights, not how they decode - and a single-model server ignores
        # it. The bytes are stamped as `model_sha256`, which a renamed entry
        # does not move.
        "model",
    }
)
...
def decode_digest(payload: Mapping[str, Any]) -> str:
    """One digest over what a request asked the decoder to do.

    Every posted key is in it except the three in `UNSTAMPED_REQUEST_KEYS`, and
    that direction is the whole design: a caller that starts sending a new
    sampler field gets it stamped, where a list of stamped keys would have gone
    on reporting the old decode under the old digest.

    It is taken over the body rather than over a config block because a config
    block is what somebody wrote down and a body is what went out. The two
    disagree exactly when a builder drops a key, which is the case worth seeing.
    """
    stamped = {name: value for name, value in payload.items() if name not in UNSTAMPED_REQUEST_KEYS}
    return derive_text_digest(canonical_json(stamped))
...
class Completion:
    ...
    #: The digest of what this call posted, minus the prompt, the grammar and
    #: the model name. Empty where the reply was parsed without its request in
    #: hand, which is every path that reads a recorded body off disk.
    decode_sha256: str = ""
...
def post(
    ...
    *,
    ...
    The decode stamp is taken here because this is the one place that has both
    halves: the body that went out and the reply that came back. A caller that
    digested its own payload would be stamping what it meant to send.
    """
    ...
    with request.urlopen(outbound, timeout=timeout) as response:
        completion = parse_completion(response.read().decode("utf-8"), answer_at=answer_at)
    return replace(completion, decode_sha256=decode_digest(payload))
