...
- [`TODO/`](TODO/) - the plan-docs, one per piece of work, each carrying its own Status Reckoner of rows and what has landed. **[`TODO/STATUS.md`](TODO/STATUS.md) is the list of them**, generated from those Reckoners by one job after a merge; it finds a plan by looking rather than by being told, so a plan cannot be missing from it. `python backend/utilities/plan_status.py` prints the same answer about the tree in front of you and writes nothing. There is no second list here: the one that used to sit on this line named six plans against fifty on disk, because a list somebody retypes is a list that rots.
